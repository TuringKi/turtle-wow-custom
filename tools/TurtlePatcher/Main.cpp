...
bool IsOurMessage(UINT MsgID) { return MsgID == G_WM_INCREMENT_PROGRESS || MsgID == G_WM_PATCHING_DONE || MsgID == G_WM_SET_PROGRESS || MsgID == G_WM_SET_STAGE || MsgID == G_WM_SET_ERROR; }
...
inline void SetProgress(int Progress) { PostThreadMessageA(gMainThreadID, G_WM_SET_PROGRESS, Progress, 0); }

inline void SetErrorState() { PostThreadMessageA(gMainThreadID, G_WM_SET_ERROR, 0, 0); }

inline void SetPatcherStage(EPatcherStage Stage) { PostThreadMessageA(gMainThreadID, G_WM_SET_STAGE, (WPARAM)Stage, 0); }
...
void PatchVisualVersion(FILE* hWoW, const std::string& VersionString, const std::string& BuildString, const std::string& DateString, const std::string& WebsiteFilter, const std::string& WebsiteFilter2)
{
...
void PatchBinary(FILE* hWoW)
{
    ...
    // main code
    unsigned char patch_27[] = {};
...
struct StormFile
{
    ...
    StormFile(HANDLE InFile) : hFile(InFile)
    {
        assert(hFile != NULL);
    ...
    ~StormFile() { SFileCloseFile(hFile); }
};
...
struct StormArchive
{
    ...
    bool IsValid() const { return mpq != NULL; }
    ...
    struct FileIterator
    {
        ...
        FileIterator(HANDLE hMPQ) : ParentMPQ(hMPQ)
        {
        ...
        operator bool() { return !bReachEnd; }
        ...
        ~FileIterator() { SFileFindClose(hFind); }
    };
};
...
[[deprecated("Strongly recommended to use ErrorBox(DWORD TxtID), since we need support localization")]] inline void ErrorBox(const TCHAR* errorTxt)
{
    TCHAR ErrLabel[32] = {0};
...
DWORD FindWoWProcess()
{
    ...
    if (Process32First(snapshot, &entry) == TRUE)
    {
        while (Process32Next(snapshot, &entry) == TRUE)
        {
            ...
            if (CurrentProcess == TargetProcessName || CurrentProcess == TargetProcessName2)
            {
...
DWORD DoPatcherMainWork()
{
    ...
    if (bDownloadPatchFromInternet /*!bPatchExist*/)
    {
        ...
        Downloader->OnDownloadComplete = [&bShouldWait]() { InterlockedExchange(&bShouldWait, FALSE); };
