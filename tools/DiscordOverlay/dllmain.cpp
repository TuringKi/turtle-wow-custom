...
unsigned __stdcall DiscordThreadStart(void*)
{
    ...
    gState.Core->SetLogHook(discord::LogLevel::Debug, [](discord::LogLevel level, const char* message) { PrintConsole(L"Discord %S: %S", LogLevel_ToString(level), message); });
    ...
    ActivityMgr.UpdateActivity(DefaultAct, [](discord::Result result) { PrintConsole(L"Set activity - %S", result == discord::Result::Ok ? "Ok" : "FAILED"); });
