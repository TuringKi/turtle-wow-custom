...
float DistanceSq(Vector3 v, float x, float y) { return (x - v.x) * (x - v.x) + (y - v.y) * (y - v.y); }
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        rotation = G3D::Matrix3::fromEulerAnglesXYZ(-1 * G3D::pi() * instance.iRot.z / 180.0f, -1 * G3D::pi() * instance.iRot.x / 180.0f, -1 * G3D::pi() * instance.iRot.y / 180.0f);
