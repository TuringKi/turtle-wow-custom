...
{
public:
    void RegisterCommands(httplib::Server* server) override { RegisterEndpoint<HttpApi::HttpMethod::Get>("/manifest", &GetManifestAction); }
};
...
public:
    void Start()
    {
        ...
        Server.set_logger([](const httplib::Request& req, const httplib::Response& res) { sLog.out(LOG_API, "Handling request from %s.\nRoute:%s\nBody:%s", req.remote_addr.c_str(), req.path.c_str(), req.body.c_str()); });

        HttpApi::BaseController::RegisterAll(&Server);
    }

    void Tick() { Server.listen(bind_ip, Port); }
