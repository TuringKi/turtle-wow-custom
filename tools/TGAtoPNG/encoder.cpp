...
namespace tga
{
    ...
    Encoder::Encoder(FileInterface* file) : m_file(file) {}
    ...
    void Encoder::writeHeader(const Header& header)
    {
        ...
        m_hasAlpha = (header.bitsPerPixel == 16 || header.bitsPerPixel == 32);
    ...
    void Encoder::writeImage(const Header& header, const Image& image, Delegate* delegate)
    {
        ...
        case tga::RleRgb:
            for (int y = 0; y < h; ++y)
            {
                ...
                case 15:
                case 16:
                    writeRleScanline<uint32_t>(w, image, &Encoder::write16Rgb);
                    break;
                case 24:
                    writeRleScanline<uint32_t>(w, image, &Encoder::write24Rgb);
                    break;
                case 32:
                    writeRleScanline<uint32_t>(w, image, &Encoder::write32Rgb);
                    break;
                default:
                    assert(false);
                    return;
    ...
    template <typename T>
    void Encoder::writeRleScanline(const int w, const Image& image, void (Encoder::*writePixel)(color_t))
    {
        int x = 0;
        while (x < w)
        {
    ...
    template <typename T>
    void Encoder::countRepeatedPixels(const int w, const Image& image, int x0, int& offset, int& count)
    {
        auto it = m_iterator;
    ...
    void Encoder::write8(uint8_t value) { m_file->write8(value); }
    ...
    void Encoder::write16Rgb(color_t c)
    {
        ...
        const uint16_t v = ((r >> 3) << 10) | ((g >> 3) << 5) | ((b >> 3)) | (m_hasAlpha && a >= 128 ? 0x8000 : 0); // TODO configurable threshold
        write16(v);
    }
