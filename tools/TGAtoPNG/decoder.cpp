...
namespace tga
{
    ...
    Decoder::Decoder(FileInterface* file) : m_file(file) {}
  ...
  m_hasAlpha =
    ...
    (header.bitsPerPixel == 16 && alphaBits == 1);
        ...
        m_hasAlpha = (header.bitsPerPixel == 32) || (header.bitsPerPixel == 16);
#endif
        ...
        return (header.validColormapType() && header.valid());
    }
            ...
            case 16:
                {
                    ...
                    header.colormap[i] = rgba(scale_5bits_to_8bits((c >> 10) & 0x1F), scale_5bits_to_8bits((c >> 5) & 0x1F), scale_5bits_to_8bits(c & 0x1F));
                    break;
                }
    ...
    bool Decoder::readImage(const Header& header, Image& image, Delegate* delegate)
    {
        ...
        for (int y = 0; y < header.height; ++y)
        {
            ...
            if (delegate && !delegate->notifyProgress(float(y) / float(header.height)))
            {
                break;
            }
        }
    ...
    void Decoder::postProcessImage(const Header& header, Image& image)
    {
        ...
        for (int y = 0; y < header.height; ++y)
        {
            ...
            {
                color_t c = *p;
                if (transparentImage && geta(c) != 0)
                {
                    transparentImage = false;
                }
                if (blackImage && (getr(c) != 0 || getg(c) != 0 || getb(c) != 0))
                {
                    blackImage = false;
                }
            }
        ...
        if (transparentImage && !blackImage)
        {
            for (int y = 0; y < header.height; ++y)
            {
                ...
                {
                    color_t c = *p;
                    *p = rgba(getr(c), getg(c), getb(c), 255);
                }
            }
        }
    }
    ...
    uint8_t Decoder::read8() { return m_file->read8(); }
    ...
    color_t Decoder::read16AsRgb()
    {
        ...
        return rgba(scale_5bits_to_8bits((v >> 10) & 0x1F), scale_5bits_to_8bits((v >> 5) & 0x1F), scale_5bits_to_8bits(v & 0x1F), a);
    }
