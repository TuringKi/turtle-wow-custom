...
#define SUB(a, b) (m[2][a] * m[3][b] - m[3][a] * m[2][b])
        return m[0][0] * (m[1][1] * SUB(2, 3) - m[1][2] * SUB(1, 3) + m[1][3] * SUB(1, 2)) - m[0][1] * (m[1][0] * SUB(2, 3) - m[1][2] * SUB(0, 3) + m[1][3] * SUB(0, 2)) + m[0][2] * (m[1][0] * SUB(1, 3) - m[1][1] * SUB(0, 3) + m[1][3] * SUB(0, 1)) - m[0][3] * (m[1][0] * SUB(1, 2) - m[1][1] * SUB(0, 2) + m[1][2] * SUB(0, 1));
#undef SUB
    }
...
#undef SUB
    ...
    Matrix& operator*=(const Matrix& p) { return *this = this->operator*(p); }
    ...
    Vec3D GetHPB()
    {
        ...
        // First get RX and RY
        bool zzero[3] = {(fabs(z[0]) <= std::numeric_limits<float>::epsilon()), (fabs(z[1]) <= std::numeric_limits<float>::epsilon()), (fabs(z[2]) <= std::numeric_limits<float>::epsilon())};
        ...
        bool rot_yzero[3] = {(fabs(rot_y[0]) <= std::numeric_limits<float>::epsilon()), (fabs(rot_y[1]) <= std::numeric_limits<float>::epsilon()), (fabs(rot_y[2]) <= std::numeric_limits<float>::epsilon())};
