...
public:
    ...
    friend Vec4D operator*(float d, const Vec4D& v) { return v * d; }
    ...
    float operator*(const Vec4D& v) const { return x * v.x + y * v.y + z * v.z + w * v.w; }
...
public:
    ...
    static const Quaternion lerp(const float r, const Quaternion& v1, const Quaternion& v2) { return v1 * (1.0f - r) + v2 * r; }
