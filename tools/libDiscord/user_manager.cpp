...
namespace discord
{
    ...
    Result UserManager::GetCurrentUser(User* currentUser)
    {
        ...
        auto result = internal_->get_current_user(internal_, reinterpret_cast<DiscordUser*>(currentUser));
    ...
    void UserManager::GetUser(UserId userId, std::function<void(Result, User const&)> callback)
    {
        ...
        {
            std::unique_ptr<std::function<void(Result, User const&)>> cb(reinterpret_cast<std::function<void(Result, User const&)>*>(callbackData));
            if (!cb || !(*cb))
            {
                return;
            }
    ...
    Result UserManager::GetCurrentUserPremiumType(PremiumType* premiumType)
    {
        ...
        auto result = internal_->get_current_user_premium_type(internal_, reinterpret_cast<EDiscordPremiumType*>(premiumType));
        return static_cast<Result>(result);
    }
    ...
    Result UserManager::CurrentUserHasFlag(UserFlag flag, bool* hasFlag)
    {
        ...
        auto result = internal_->current_user_has_flag(internal_, static_cast<EDiscordUserFlag>(flag), reinterpret_cast<bool*>(hasFlag));
        return static_cast<Result>(result);
    }
