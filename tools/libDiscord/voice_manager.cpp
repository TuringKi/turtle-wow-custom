...
namespace discord
{
    ...
    Result VoiceManager::GetInputMode(InputMode* inputMode)
    {
        ...
        auto result = internal_->get_input_mode(internal_, reinterpret_cast<DiscordInputMode*>(inputMode));
    ...
    void VoiceManager::SetInputMode(InputMode inputMode, std::function<void(Result)> callback)
    {
        ...
        internal_->set_input_mode(internal_, *reinterpret_cast<DiscordInputMode const*>(&inputMode), cb.release(), wrapper);
    }
    ...
    Result VoiceManager::GetLocalVolume(Snowflake userId, std::uint8_t* volume)
    {
        ...
        auto result = internal_->get_local_volume(internal_, userId, reinterpret_cast<uint8_t*>(volume));
        return static_cast<Result>(result);
    }
