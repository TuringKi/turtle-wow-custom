...
namespace discord
{
    ...
    Result AchievementManager::GetUserAchievement(Snowflake userAchievementId, UserAchievement* userAchievement)
    {
        if (!userAchievement)
        {
        ...
        auto result = internal_->get_user_achievement(internal_, userAchievementId, reinterpret_cast<DiscordUserAchievement*>(userAchievement));
        return static_cast<Result>(result);
    }

    Result AchievementManager::GetUserAchievementAt(std::int32_t index, UserAchievement* userAchievement)
    {
        if (!userAchievement)
        {
            return Result::InternalError;
        ...
        auto result = internal_->get_user_achievement_at(internal_, index, reinterpret_cast<DiscordUserAchievement*>(userAchievement));
        return static_cast<Result>(result);
    }
