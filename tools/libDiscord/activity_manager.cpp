...
namespace discord
{
    ...
    public:
        ...
        static void OnActivityInvite(void* callbackData, EDiscordActivityActionType type, DiscordUser* user, DiscordActivity* activity)
        {
            ...
            auto& module = core->ActivityManager();
            module.OnActivityInvite(static_cast<ActivityActionType>(type), *reinterpret_cast<User const*>(user), *reinterpret_cast<Activity const*>(activity));
        }
    };
    ...
    void ActivityManager::SendRequestReply(UserId userId, ActivityJoinRequestReply reply, std::function<void(Result)> callback)
    {
        static auto wrapper = [](void* callbackData, EDiscordResult result) -> void
        {
            std::unique_ptr<std::function<void(Result)>> cb(reinterpret_cast<std::function<void(Result)>*>(callbackData));
