...
namespace benchmark
{
    ...
    public:
        explicit State(int64_t iters) : iters_(iters), arg_(0), has_arg_(false) {}

        State(int64_t iters, int64_t arg) : iters_(iters), arg_(arg), has_arg_(true) {}
...
namespace testing
{
    ...
    public:
        Benchmark(const char* name, void (*func)(benchmark::State&)) :
            name_(name), func_(
                             [func](int iters, int arg)
                             {
                                 benchmark::State state(iters);
                                 func(state);
                             }),
            lo_(0), hi_(0), has_arg_(false)
        {
            Register();
        }
...
#define BENCHMARK_RANGE(f, lo, hi) ::testing::Benchmark* _benchmark_##f = (new ::testing::Benchmark(#f, f, lo, hi))
