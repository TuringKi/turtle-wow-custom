...
#define TreeType PointKDTree<T, PositionFunc, HashFunc, EqualsFunc>
        ...
        public:
            ...
            inline Handle() {}
            inline Handle(const T& v) : value(v) { PositionFunc::getPosition(v, m_position); }
        ...
        public:
            ...
            */
            ...
            void getIntersectingMembers(const AABox& box, const Sphere& sphere, Array<T>& members, bool useSphere) const
            {
                ...
                {
                    if ((useSphere && sphere.contains(valueArray[v].position())) || (!useSphere && box.contains(valueArray[v].position())))
                    {
        ...
        Node* makeNode(Array<Handle>& source, Array<Handle>& temp, int valuesPerNode, int numMeanSplits)
        {
            ...
            else
            {
                ...
                if (numMeanSplits <= 0)
                {
                    ...
                    if ((node->valueArray.size() > source.size() / 2) && (source.size() > 10))
                    {
    ...
    public:
        ...
        PointKDTree(const PointKDTree& src) : root(NULL) { *this = src; }
        ...
        size_t size() const { return memberTable.size(); }
         ...
         */
        bool contains(const T& value) { return memberTable.containsKey(value); }
    ...
    public:
         ...
         */
        void getMembers(Array<T>& members) const { memberTable.getKeys(members); }
        ...
        */
        ...
        public:
            ...
            const T& operator*() const { return it->key; }

            T* operator->() const { return &(it->key); }

            operator T*() const { return &(it->key); }
        };
