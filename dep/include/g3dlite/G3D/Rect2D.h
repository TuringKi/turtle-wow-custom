...
namespace G3D
{
    ...
    public:
        ...
        inline bool isEmpty() const { return min.isNaN() && max.isNaN(); }
        ...
        Point2 randomPoint() const { return Point2(uniformRandom(0, max.x - min.x) + min.x, uniformRandom(0, max.y - min.y) + min.y); }
        ...
        float x0() const { return min.x; }

        float x1() const { return max.x; }

        float y0() const { return min.y; }

        float y1() const { return max.y; }

        /** Min, min corner */
        Point2 x0y0() const { return min; }

        Point2 x1y0() const { return Point2(max.x, min.y); }

        Point2 x0y1() const { return Point2(min.x, max.y); }

        /** Max,max corner */
        Point2 x1y1() const { return max; }
        ...
        Point2 center() const { return (max + min) * 0.5; }
        ...
        static Rect2D xywh(float x, float y, float w, float h) { return xyxy(x, y, x + w, y + h); }

        static Rect2D xywh(const Point2& v, const Vector2& w) { return xyxy(v.x, v.y, v.x + w.x, v.y + w.y); }

        /** Constructs a Rect2D with infinite boundaries.
            Use isFinite() to test either min or max.
         */
        static Rect2D inf() { return xyxy(Vector2::inf(), Vector2::inf()); }
        ...
        Rect2D operator*(float s) const { return xyxy(min.x * s, min.y * s, max.x * s, max.y * s); }

        Rect2D operator*(const Vector2& s) const { return xyxy(min * s, max * s); }

        Rect2D operator/(float s) const { return xyxy(min / s, max / s); }

        Rect2D operator/(const Vector2& s) const { return xyxy(min / s, max / s); }

        Rect2D operator+(const Vector2& v) const { return xyxy(min + v, max + v); }

        Rect2D operator-(const Vector2& v) const { return xyxy(min - v, max - v); }

        bool operator==(const Rect2D& other) const { return (min == other.min) && (max == other.max); }
        ...
        /** @deprecated
         @sa expand() */
        Rect2D border(float delta) const { return Rect2D::xywh(x0() + delta, y0() + delta, width() - 2.0f * delta, height() - 2.0f * delta); }
        ...
        void clip(const Array<T>& inPoly, Array<T>& outPoly) const
        {
            ...
            bool entirelyClipped = clipSide2D(x0(), lessThan, X, inPoly, temp) || clipSide2D(x1(), greaterThan, X, temp, outPoly) || clipSide2D(y0(), lessThan, Y, outPoly, temp) || clipSide2D(y1(), greaterThan, Y, temp, outPoly);
