...
namespace G3D
{
    ...
    public:
         ...
         */
        inline Vector2 lerp(const Vector2& v, float alpha) const { return (*this) + (v - *this) * alpha; }

        inline Vector2 clamp(const Vector2& low, const Vector2& high) const { return Vector2(G3D::clamp(x, low.x, high.x), G3D::clamp(y, low.y, high.y)); }

        inline Vector2 clamp(float low, float high) const { return Vector2((float)G3D::clamp(x, low, high), (float)G3D::clamp(y, low, high)); }
        ...
        /** Component-wise argmax(abs(), v.abs()).
            ...
            \sa max
         */
        Vector2 maxAbs(const Vector2& v) const { return Vector2(::fabsf(x) > ::fabsf(v.x) ? x : v.x, ::fabsf(y) > ::fabsf(v.y) ? y : v.y); }
        ...
        /** Component-wise argmin(abs(), v.abs()).
            ...
            \sa max
         */
        Vector2 minAbs(const Vector2& v) const { return Vector2(::fabsf(x) < ::fabsf(v.x) ? x : v.x, ::fabsf(y) < ::fabsf(v.y) ? y : v.y); }
    ...
    inline const float& Vector2::operator[](int i) const { return ((float*)this)[i]; }
    ...
    inline bool Vector2::operator==(const Vector2& rkVector) const { return (x == rkVector.x && y == rkVector.y); }


    inline bool Vector2::operator!=(const Vector2& rkVector) const { return (x != rkVector.x || y != rkVector.y); }


    inline Vector2 Vector2::operator+(const Vector2& rkVector) const { return Vector2(x + rkVector.x, y + rkVector.y); }


    inline Vector2 Vector2::operator-(const Vector2& rkVector) const { return Vector2(x - rkVector.x, y - rkVector.y); }


    inline Vector2 Vector2::operator*(float fScalar) const { return Vector2(fScalar * x, fScalar * y); }
    ...
    inline float Vector2::dot(const Vector2& rkVector) const { return x * rkVector.x + y * rkVector.y; }


    inline Vector2 Vector2::min(const Vector2& v) const { return Vector2(G3D::min(v.x, x), G3D::min(v.y, y)); }


    inline Vector2 Vector2::max(const Vector2& v) const { return Vector2(G3D::max(v.x, x), G3D::max(v.y, y)); }


    inline bool Vector2::fuzzyEq(const Vector2& other) const { return G3D::fuzzyEq((*this - other).squaredLength(), 0); }


    inline bool Vector2::fuzzyNe(const Vector2& other) const { return G3D::fuzzyNe((*this - other).squaredLength(), 0); }


    inline bool Vector2::isFinite() const { return G3D::isFinite(x) && G3D::isFinite(y); }


    inline bool Vector2::isZero() const { return G3D::fuzzyEq(fabsf(x) + fabsf(y), 0.0f); }


    inline bool Vector2::isUnit() const { return G3D::fuzzyEq(squaredLength(), 1.0f); }
...
inline G3D::Vector2 operator*(double s, const G3D::Vector2& v) { return v * (float)s; }
inline G3D::Vector2 operator*(int s, const G3D::Vector2& v) { return v * (float)s; }
