...
namespace G3D
{
    ...
    public:
         ...
         */
        explicit AABox(const Point3& v) { lo = hi = v; }
        ...
        */
        ...
        bool isEmpty() const { return lo.isNaN(); }
        ...
        */
        AABox(const Point3& low, const Point3& high) { set(low, high); }
        ...
        AABox operator/(float f) const { return *this * (1.0f / f); }
        ...
        inline bool isFinite() const { return isEmpty() || (lo.isFinite() && hi.isFinite()); }

        /** Returns not-a-number if empty */
        inline const Point3& low() const { return lo; }

        /** Returns not-a-number if empty */
        inline const Point3& high() const { return hi; }
         ...
         */
        inline Point3 center() const { return (lo + hi) * 0.5; }
        ...
        inline bool contains(const AABox& other) const { return (other.hi.x <= hi.x) && (other.hi.y <= hi.y) && (other.hi.z <= hi.z) && (other.lo.x >= lo.x) && (other.lo.y >= lo.y) && (other.lo.z >= lo.z); }

        inline bool contains(const Point3& point) const { return (point.x >= lo.x) && (point.y >= lo.y) && (point.z >= lo.z) && (point.x <= hi.x) && (point.y <= hi.y) && (point.z <= hi.z); }
        ...
        inline size_t hashCode() const { return lo.hashCode() + hi.hashCode(); }
        ...
        void getBounds(AABox& out) const { out = *this; }
