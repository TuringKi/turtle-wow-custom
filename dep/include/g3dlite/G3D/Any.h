...
namespace G3D
{
    ...
    */
    ...
    private:
        ...
        */
        ...
        bool isPlaceholder() const { return !m_placeholderName.empty(); }
    ...
    public:
        ...
        */
        ...
        const Any& last() const { return (*this)[size() - 1]; }

        Any& last() { return (*this)[size() - 1]; }
        ...
        // string literals
        const Any& operator[](const char* key) const { return operator[](std::string(key)); }
        ...
        inline Any& operator[](const char* key) { return operator[](std::string(key)); }
        ...
        bool operator==(const std::string& s) const { return *this == Any(s); }

        bool operator==(const double& v) const { return *this == Any(v); }

        bool operator==(int v) const { return *this == Any(v); }

        bool operator==(bool v) const { return *this == Any(v); }
        ...
        operator uint8() const { return uint8(int(*this)); }

        operator int16() const { return int16(int(*this)); }

        operator uint16() const { return uint16(int(*this)); }
    ...
    */
    ...
    public:
        ...
        bool hasMore() const { return m_any.size() > m_alreadyRead.size(); }

        /** Verifies that all keys have been read. */
        void verifyDone() const;

        /** Return the underlying Any. */
        const Any& any() const { return m_any; }
    ...
    AnyKeyIterator& operator++();
        ...
        bool containsUnread(const std::string& s) const { return m_any.containsKey(s) && !m_alreadyRead.contains(s); }
    };
