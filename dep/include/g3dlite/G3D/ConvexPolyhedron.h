...
namespace G3D
{
    ...
    public:
         ...
         */
        inline const Vector3& vertex(int i) const { return _vertex[i]; }

        inline void setVertex(int i, const Vector3& v) { _vertex[i] = v; }

        /**
           Zero vertices indicates an empty polygon (zero area).
        */
        inline int numVertices() const { return _vertex.size(); }

        inline void setNumVertices(int n) { _vertex.resize(n); }
    ...
    public:
        ...
        inline int numVertices() const { return m_vertex.size(); }
