    ...
    namespace _internal
    {
        ...
        */
        ...
        public:
            ...
            inline bool operator==(const Indirector& m) const { return *handle == *(m.handle); }

            inline bool operator==(const Type& m) const { return *handle == m; }

            inline size_t hashCode() const { return handle->hashCode(); }
        };
    } // namespace _internal
} // namespace G3D
...
#define TreeType KDTree<T, BoundsFunc, HashFunc, EqualsFunc>
        ...
        public:
            ...
            inline bool operator==(const Handle& other) const { return EqualsFunc::equals(value, other.value); }

            inline size_t hashCode() const { return HashFunc::hashCode(value); }
        };
        ...
        public:
            ...
            */
            ...
            inline bool isLeaf() const { return (child[0] == NULL) && (child[1] == NULL); }
            ...
            void verifyNode(const Vector3& lo, const Vector3& hi)
            {
                ...
                debugAssertM(lo == splitBounds.low(), format("lo = %s, splitBounds.lo = %s", lo.toString().c_str(), splitBounds.low().toString().c_str()));
            ...
            bool intersects(const Ray& ray, float distance) const
            {
                ...
                bool rayWillHitBounds = CollisionDetection::collisionLocationForMovingPointFixedAABox(ray.origin(), ray.direction(), splitBounds, location, alreadyInsideBounds);

                bool canHitThisNode = (alreadyInsideBounds || (rayWillHitBounds && ((location - ray.origin()).squaredLength() < square(distance))));
            ...
            void intersectRay(const Ray& ray, RayCallback& intersectCallback, float& distance, bool intersectCallbackIsFast) const
            {
                ...
                for (int v = 0; v < valueArray.size(); ++v)
                {
                    ...
                    if (!intersectCallbackIsFast)
                    {
                        ...
                        const AABox& bounds = boundsArray[v];
                        bool alreadyInsideBounds = false;
                        bool rayWillHitBounds = CollisionDetection::collisionLocationForMovingPointFixedAABox(ray.origin(), ray.direction(), bounds, location, alreadyInsideBounds);

                        canHitThisObject = (alreadyInsideBounds || (rayWillHitBounds && ((location - ray.origin()).squaredLength() < square(distance))));
                    }
        ...
        Node* makeNode(Array<Handle*>& source, int valuesPerNode, int numMeanSplits, Array<Handle*>& temp)
        {
            ...
            else
            {
                ...
                if (numMeanSplits <= 0)
                {
                    ...
                    if ((node->valueArray.size() > (source.size() / 2)) && (source.size() > 6))
                    {
                ...
                if (numMeanSplits > 0)
                {
                    ...
                    splitLocation = bounds.high()[splitAxis] * 0.5f + bounds.low()[splitAxis] * 0.5f;

                    debugAssertM(isFinite(splitLocation), "Internal error: split location must be finite.");
    ...
    public:
        ...
        KDTree(const KDTree& src) : root(NULL) { *this = src; }
    ...
    public:
        ...
        private:
            ...
            BoxIntersectionIterator(const AABox& b, const Node* root) : isEnd(root == NULL), box(b), node(const_cast<Node*>(root)), nextValueArrayIndex(-1)
            {
        ...
        public:
            inline bool operator!=(const BoxIntersectionIterator& other) const { return !(*this == other); }
            ...
            bool operator==(const BoxIntersectionIterator& other) const
            {
                ...
                else
                {
                    ...
                    if ((box != other.box) || (node != other.node) || (nextValueArrayIndex != other.nextValueArrayIndex) || (stack.length() != other.stack.length()))
                    {
                        return false;
                    }
         ...
         */
        BoxIntersectionIterator beginBoxIntersection(const AABox& box) const { return BoxIntersectionIterator(box, root); }
        ...
        /**
          ...
          calling balance.
         */
        void serializeStructure(BinaryOutput& bo) const { Node::serializeStructure(root, bo); }
        ...
        */
        ...
        public:
            const T& operator*() const { return it->key.handle->value; }

            T* operator->() const { return &(it->key.handle->value); }

            operator T*() const { return &(it->key.handle->value); }
        };
         ...
         */
        Iterator begin() const { return Iterator(memberTable.begin()); }
        ...
        /**
         ...
         element.
         */
        Iterator end() const { return Iterator(memberTable.end()); }
#undef TreeType
    };
