...
namespace G3D
{
    ...
    public:
        ...
        uint8& operator[](int i) { return value[i]; }

        const uint8& operator[](int i) const { return value[i]; }
        ...
        inline bool operator!=(const MD5Hash& other) const { return !(*this == other); }
        ...
        static size_t hashCode(const MD5Hash& key)
        {
            ...
            {
                const int x = i * 4;
                h ^= (((uint32)key.value[x + 0]) << 24) | (((uint32)key.value[x + 1]) << 16) | (((uint32)key.value[x + 2]) << 8) | ((uint32)key.value[x + 3]);
            }
            return h;
        }
    };
