...
namespace G3D
{
    ...
    private:
        ...
        private:
            ...
            Node(const Key& k, const Value& v, size_t h, Node* n) : entry(k, v), hashCode(h), next(n) { debugAssert((next == NULL) || isValidHeapPointer(next)); }

            Node(const Key& k, size_t h, Node* n) : entry(k), hashCode(h), next(n) { debugAssert((next == NULL) || isValidHeapPointer(next)); }
        ...
        public:
            ...
            */
            Node* clone(MemoryManager::Ref& mm) { return create(this->entry.key, this->entry.value, hashCode, (next == NULL) ? NULL : next->clone(mm), mm); }
        };
        ...
        void* alloc(size_t s) const { return m_memoryManager->alloc(s); }

        void free(void* p) const { return m_memoryManager->free(p); }
    ...
    public:
        ...
        */
         ...
         */
        double debugGetLoad() const { return (double)size() / m_numBuckets; }

        /**
         Returns the number of buckets.
         */
        size_t debugGetNumBuckets() const { return m_numBuckets; }
        ...
        private:
             ...
             */
            Iterator() : index(0), node(NULL), m_bucket(NULL) { isDone = true; }

            Iterator(size_t numBuckets, Node** m_bucket) : index(0), node(NULL), m_numBuckets(numBuckets), m_bucket(m_bucket)
            {
        ...
        public:
            ...
            const Entry& operator*() const { return node->entry; }

            const Value& value() const { return node->entry.value; }
         ...
         */
        void set(const Key& key, const Value& value) { getCreateEntry(key).value = value; }
    ...
    public:
        ...
        bool getRemove(const Key& key, Key& removedKey, Value& removedValue) { return remove(key, removedKey, removedValue, true); }
    ...
    public:
        ...
        */
        Entry& getCreateEntry(const Key& key, bool& created)
        {
            ...
            const int bucketsPerElement = (m_size > 50000) ? 3 : ((m_size > 10000) ? 5 : ((m_size > 5000) ? 10 : 15));
            ...
            if ((bucketLength > maxBucketLength) && !allSameCode && (m_numBuckets < m_size * bucketsPerElement))
            {
        ...
        Value& getCreate(const Key& key) { return getCreateEntry(key).value; }

        /** \param created True if the element was created. */
        Value& getCreate(const Key& key, bool& created) { return getCreateEntry(key, created).value; }
         ...
         */
        inline Value& operator[](const Key& key) const { return get(key); }
