...
namespace G3D
{
    ...
    public:
        ...
        template <typename T>
        inline static void multisend(const Array<ReliableConduitRef>& array, uint32 type, const T& m)
        {
    ...
    private:
        ...
        void serializeMessage(uint32 type, const T& m, BinaryOutput& b) const
        {
            ...
            if (b.size() >= MTU)
            {
                throw LightweightConduit::PacketSizeException(format("This LightweightConduit is limited to messages of "
                                                                     "%d bytes (Ethernet hardware limit; this is the "
                                                                     "'UDP MTU')",
                                                                     maxMessageSize()),
    ...
    public:
        ...
        public:
            ...
            inline PacketSizeException(const std::string& m, int s, int b) : message(m), serializedPacketSize(s), maxMessageSize(b) {}
        };
    ...
    public:
        ...
        /** Returns the available ethernet adapters for the current
            ...
            for localhost.*/
        inline const Array<EthernetAdapter>& adapterArray() const { return m_adapterArray; }

        /** Returns the (unique) IP addresses for UDP broadcasting
            extracted from adapterArray(). All are in host byte order. */
        inline const Array<uint32>& broadcastAddressArray() const { return m_broadcastAddresses; }
