...
namespace G3D
{
    ...
    private:
        ...
        /**
         ...
         Used by append.
         */
        inline bool inArray(const T* address) { return (address >= data) && (address < data + num); }


        /** Only compiled if you use the sort procedure. */
        static bool __cdecl compareGT(const T& a, const T& b) { return a > b; }
    ...
    public:
        ...
        /**
         ...
         element.
         */
        ConstIterator end() const { return data + num; }
        ...
        /**
         ...
         the Array is deallocated.
         */
        const T* getCArray() const { return data; }

        /** Creates a zero length array (no heap allocation occurs until resize). */
        Array() : num(0) { init(0, MemoryManager::create()); }
        ...
        Array(const Array& other) : num(0) { _copy(other); }

        explicit Array(const std::vector<T>& other) : num(0), data(NULL) { *this = other; }
         ...
         */
        void clear(bool shrink = true) { resize(0, shrink); }
        ...
        inline MemoryManager::Ref memoryManager() const { return m_memoryManager; }

        /**
         Number of elements in the array.
         */
        inline int size() const { return (int)num; }
        ...
        */
         ...
         */
        inline void push(const T& value) { append(value); }

        inline void push(const Array<T>& array) { append(array); }

        /** Alias to provide std::vector compatibility */
        inline void push_back(const T& v) { push(v); }
        ...
        /**
           "The member function returns the storage currently allocated to hold the controlled
            ...
            For compatibility with std::vector.
        */
        int capacity() const { return (int)numAllocated; }
        ...
        */
        ...
        {
            debugAssertM((n >= 0) && (n < (int)num), format("Array index out of bounds. n = %d, size() = %d", (int)n, (int)num));
            debugAssert(data != NULL);
            return data[n];
        }
        ...
        {
            debugAssertM(n < (uint32)num, format("Array index out of bounds. n = %d, size() = %d", (int)n, (int)num));
            return data[n];
        }
        ...
        */
        ...
        /** \deprecated */
        void G3D_DEPRECATED deleteAll() { invokeDeleteOnAllElements(); }
    ...
    \endcode
        ...
        void sortSubArray(int beginIndex, int endIndex, bool(__cdecl* lessThan)(const T& elem1, const T& elem2)) { std::sort(data + beginIndex, data + endIndex + 1, lessThan); }
         ...
         */
        void partition(const T& partitionElement, Array<T>& ltArray, Array<T>& eqArray, Array<T>& gtArray) const { partition(partitionElement, ltArray, eqArray, gtArray, typename Array<T>::DefaultComparator()); }
        ...
        size_t sizeInMemory() const { return sizeof(Array<T>) + (sizeof(T) * numAllocated); }
