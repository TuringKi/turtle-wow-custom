...
#define ThisType PointHashGrid<Value, PosFunc, EqualsFunc>
    ...
    public:
        ...
        */
        ...
        float radiusHint() const { return m_cellWidth; }
        ...
        void clearAndSetRadiusHint(float radiusHint) { return clear(radiusHint); }
        ...
        */
        PointHashGrid(const Array<Value>& init, float radiusHint = -1.0f, const MemoryManager::Ref& m = MemoryManager::create()) : m_size(0), m_memoryManager(m)
        {
            ...
            if (radiusHint <= 0)
            {
                ...
                float numPerCell = 5;
                radiusHint = (float)pow(numPerCell * m_bounds.volume() / init.size(), 1.0 / 3.0);
        ...
        const AABox& conservativeBoxBounds() const { return m_bounds; }
        ...
        */
        ...
        private:
            ...
            Iterator() : m_isEnd(true), m_grid(NULL), m_tableIterator(CellTable().end()), m_arrayIndex(0), m_epoch(0) {}

            Iterator(const ThisType* grid) : m_isEnd(grid->size() == 0), m_grid(grid), m_tableIterator(grid->m_data.begin()), m_arrayIndex(0), m_epoch(grid->m_epoch) {}

        private:
            const Value& value() const
            {
                debugAssert(!m_isEnd);
                debugAssertM(m_tableIterator->value.size() > m_arrayIndex, "No more elements");
        ...
        public:
            inline bool operator!=(const Iterator& other) const
            {
                ...
                {
                    return (m_isEnd != other.m_isEnd) || (m_tableIterator != other.m_tableIterator) || (m_arrayIndex != other.m_arrayIndex);
                }
            }

            bool isValid() const { return !m_isEnd; }

            /** @deprecated  Use isValid */
            bool hasMore() const { return !m_isEnd; }
        ...
        */
        ...
        private:
            ...
            /** Begin iterator */
            BoxIterator(const ThisType* grid, bool exact, const AABox& box) : m_isEnd(false), m_grid(grid), m_exact(exact), m_box(box), m_current(-1, 0, 0), m_cell(NULL), m_arrayIndex(0), m_epoch(grid->m_epoch)
            {
        ...
        public:
            inline bool operator!=(const BoxIterator& other) const
            {
                ...
                {
                    return (m_isEnd != other.m_isEnd) || (m_cell != other.m_cell) || (m_arrayIndex != other.m_arrayIndex);
                }
            }
        ...
        */
        BoxIterator beginBoxIntersection(const AABox& box, bool exact = true) const { return BoxIterator(this, exact, box); }
        ...
        private:
            ...
            SphereIterator(const ThisType* grid, const Sphere& sphere) : m_isEnd(false), m_sphere(sphere), m_boxIterator(grid, false, getBoundingBox(sphere))
            {
            ...
            const Value& value() const { return *m_boxIterator; }
        ...
        public:
            inline bool operator!=(const SphereIterator& other) const
            {
                ...
                {
                    return (m_isEnd != other.m_isEnd) || (m_sphere != other.m_sphere) || (m_boxIterator != other.m_boxIterator);
                }
            }

            bool operator==(const SphereIterator& other) const { return !(*this != other); }
            ...
            /** \deprecated use isValid */
            bool G3D_DEPRECATED hasMore() const { return !m_isEnd; }
        ...
        /** Finds all values whose positions are within \a sphere.  It is
            ...
            it. */
        SphereIterator begin(const Sphere& sphere) const { return SphereIterator(this, sphere); }


        /** \deprecated \sa beginIntersection */
        SphereIterator G3D_DEPRECATED beginSphereIntersection(const Sphere& sphere) const { return SphereIterator(this, sphere); }
        ...
        */
        ...
        private:
            ...
            Cell& cell() { return m_tableIterator->value; }
        ...
        public:
            ...
            public:
                ...
                AABox bounds() const
                {
                    ...
                    return AABox(Vector3(k) * m_parent->m_grid->m_cellWidth, Vector3(k + Vector3int32(1, 1, 1)) * m_parent->m_grid->m_cellWidth);
                }
        ...
        private:
            ...
            /** End iterator. Note that the m_tableIterator is initialized to the end iterator
                of a temporary value!  This is ok because we'll never look at the value of the
                m_tableIterator, since we're initializing the "end" Iterator.*/
            CellIterator() : m_isEnd(true), m_grid(NULL), m_tableIterator(CellTable().end()), m_epoch(0) {}

            CellIterator(const ThisType* grid) : m_isEnd(false), m_grid(grid), m_tableIterator(grid->m_data.begin()), m_epoch(grid->m_epoch)
            {
        ...
        public:
            ...
            inline bool operator!=(const CellIterator& other) const
            {
                ...
                return !((m_isEnd && other.m_isEnd) || ((m_isEnd == other.m_isEnd) && (m_tableIterator != other.m_tableIterator)));
            }
        ...
        CellIterator beginCells() const { return CellIterator(this); }
        ...
        */
        ...
        int debugGetDeepestBucketSize() const { return (int)m_data.debugGetDeepestBucketSize(); }

        float debugGetAverageBucketSize() const { return m_data.debugGetAverageBucketSize(); }
#undef ThisType
    };
