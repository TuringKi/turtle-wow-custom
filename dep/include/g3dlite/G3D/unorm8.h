...
namespace G3D
{
    ...
    */
    ...
    public:
        ...
        static unorm8 fromBits(uint8 b) { return unorm8(b); }

        /** \copydoc fromBits */
        static unorm8 reinterpretFrom(uint8 b) { return unorm8(b); }
        ...
        explicit unorm8(float f) { m_bits = (uint8)(clamp(f, 0.0f, 1.0f) * 255.0f + 0.5f); }

        explicit unorm8(double f) { m_bits = iClamp(int(f * 255.0f + 0.5f), 0, 255); }

        /** Returns a number on [0.0f, 1.0f] */
        operator float() const { return float(m_bits) * (1.0f / 255.0f); }

        operator double() const { return double(m_bits) * (1.0 / 255.0); }

        static unorm8 one() { return fromBits(255); }

        static unorm8 zero() { return fromBits(0); }

        /**\brief Returns the underlying bits in this representation.
         Equivalent to:
        \code uint8 i = reinterpret_cast<const uint8&>(u); \endcode */
        uint8 bits() const { return m_bits; }

        /** \copydoc bits */
        uint8 reinterpretAsUInt8() const { return m_bits; }

        bool operator>(const unorm8 other) const { return m_bits > other.m_bits; }
        ...
        unorm8 operator+(const unorm8 other) const { return unorm8(uint8(m_bits + other.m_bits)); }
        ...
        unorm8 operator*(const int i) const { return unorm8(uint8(m_bits * i)); }
