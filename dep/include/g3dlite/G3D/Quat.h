...
namespace G3D
{
    ...
    public:
        ...
        bool fuzzyEq(const Quat& q) { return G3D::fuzzyEq(x, q.x) && G3D::fuzzyEq(y, q.y) && G3D::fuzzyEq(z, q.z) && G3D::fuzzyEq(w, q.w); }

        /** True if these quaternions represent the same rotation (note that every rotation is
            represented by two values; q and -q).
          */
        bool sameRotation(const Quat& q) { return fuzzyEq(q) || fuzzyEq(-q); }
    ...
    public:
         ...
         */
        Quat slerp(const Quat& other, float alpha, float threshold = 0.05f) const { return slerp(other, alpha, threshold, finf()); }

        /** Rotates towards \a other by at most \a maxAngle. */
        Quat movedTowards(const Quat& other, float maxAngle) const { return slerp(other, 1.0f, 0.05f, maxAngle); }

        /** Rotates towards \a other by at most \a maxAngle. */
        void moveTowards(const Quat& other, float maxAngle) { *this = movedTowards(other, maxAngle); }
        ...
        /** Note that q<SUP>-1</SUP> = q.conj() for a unit quaternion.
            @cite Dam99 page 13 */
        inline Quat inverse() const { return conj() / dot(*this); }
        ...
        /* (*this) * other.inverse() */
        Quat operator/(const Quat& other) const { return (*this) * other.inverse(); }

        /** Is the magnitude nearly 1.0? */
        bool isUnit(float tolerance = 1e-5) const { return abs(dot(*this) - 1.0f) < tolerance; }
        ...
        /**
         ...
         @cite Dam98 pg 21
         */
        inline Quat pow(float r) const { return (log() * r).exp(); }
    ...
    inline Quat exp(const Quat& q) { return q.exp(); }

    inline Quat log(const Quat& q) { return q.log(); }

    inline G3D::Quat operator*(double s, const G3D::Quat& q) { return q * (float)s; }

    inline G3D::Quat operator*(float s, const G3D::Quat& q) { return q * s; }
...
inline G3D::Quat pow(const G3D::Quat& q, double x) { return q.pow((float)x); }
