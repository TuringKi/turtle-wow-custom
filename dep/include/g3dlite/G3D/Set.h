...
namespace G3D
{
    ...
    public:
        void clearAndSetMemoryManager(const MemoryManager::Ref& m) { memberTable.clearAndSetMemoryManager(m); }

        virtual ~Set() {}

        int size() const { return (int)memberTable.size(); }

        bool contains(const T& member) const { return memberTable.containsKey(member); }
         ...
         */
        bool remove(const T& member) { return memberTable.remove(member); }
         ...
         */
        const T* getPointer(const T& member) const { return memberTable.getKeyPointer(member); }

        Array<T> getMembers() const { return memberTable.getKeys(); }

        void getMembers(Array<T>& keyArray) const { memberTable.getKeys(keyArray); }

        void clear() { memberTable.clear(); }
        ...
        public:
            ...
            bool hasMore() const { return it.isValid(); }
        ...
        /**
         ...
         element.
         */
        const Iterator end() const { return Iterator(memberTable.end()); }
    };
