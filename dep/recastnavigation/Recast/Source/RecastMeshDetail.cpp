...
inline float vdot2(const float* a, const float* b) { return a[0] * b[0] + a[2] * b[2]; }
...
inline float vdist2(const float* p, const float* q) { return sqrtf(vdistSq2(p, q)); }
...
static unsigned short getHeight(const float fx, const float fy, const float fz, const float /*cs*/, const float ics, const float ch, const int radius, const rcHeightPatch& hp)
{
...
inline float getJitterX(const int i) { return (((i * 0x8da6b343) & 0xffff) / 65535.0f * 2.0f) - 1.0f; }

inline float getJitterY(const int i) { return (((i * 0xd8163841) & 0xffff) / 65535.0f * 2.0f) - 1.0f; }
...
static unsigned char getEdgeFlags(const float* va, const float* vb, const float* vpoly, const int npoly)
{
    ...
    {
        if (distancePtSeg2d(va, &vpoly[j * 3], &vpoly[i * 3]) < thrSqr && distancePtSeg2d(vb, &vpoly[j * 3], &vpoly[i * 3]) < thrSqr)
            return 1;
    }
    return 0;
}
...
bool rcBuildPolyMeshDetail(rcContext* ctx, const rcPolyMesh& mesh, const rcCompactHeightfield& chf, const float sampleDist, const float sampleMaxError, rcPolyMeshDetail& dmesh)
{
    ...
    for (int i = 0; i < mesh.npolys; ++i)
    {
        ...
        // Build detail mesh.
        int nverts = 0;
        if (!buildPolyDetail(ctx, poly, npoly, sampleDist, sampleMaxError, heightSearchRadius, chf, hp, verts, nverts, tris, edges, samples))
        {
            return false;
        }
