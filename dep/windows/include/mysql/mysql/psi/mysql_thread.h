...
#ifdef SAFE_MUTEX
#define mysql_mutex_init(K, M, A) inline_mysql_mutex_init(K, M, A, __FILE__, __LINE__)
#else
#define mysql_mutex_init(K, M, A) inline_mysql_mutex_init(K, M, A)
#endif
#else
#ifdef SAFE_MUTEX
#define mysql_mutex_init(K, M, A) inline_mysql_mutex_init(M, A, __FILE__, __LINE__)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_rwlock_rdlock(RW) inline_mysql_rwlock_rdlock(RW, __FILE__, __LINE__)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_prlock_rdlock(RW) inline_mysql_prlock_rdlock(RW, __FILE__, __LINE__)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_rwlock_wrlock(RW) inline_mysql_rwlock_wrlock(RW, __FILE__, __LINE__)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_prlock_wrlock(RW) inline_mysql_prlock_wrlock(RW, __FILE__, __LINE__)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_rwlock_tryrdlock(RW) inline_mysql_rwlock_tryrdlock(RW, __FILE__, __LINE__)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_rwlock_trywrlock(RW) inline_mysql_rwlock_trywrlock(RW, __FILE__, __LINE__)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_cond_timedwait(C, M, W) inline_mysql_cond_timedwait(C, M, W, __FILE__, __LINE__)
#else
...
static inline int inline_mysql_mutex_init(
    ...
    PSI_mutex_key key,
#endif
    mysql_mutex_t* that, const pthread_mutexattr_t* attr
#ifdef SAFE_MUTEX
    ,
...
{
#ifdef HAVE_PSI_INTERFACE
    that->m_psi = (PSI_server ? PSI_server->init_rwlock(key, &that->m_prlock) : NULL);
#else
    that->m_psi = NULL;
#endif
...
)
{
    ...
    if (likely(PSI_server && that->m_psi))
    {
        locker = PSI_server->get_thread_rwlock_locker(&state, that->m_psi, PSI_RWLOCK_READLOCK);
        if (likely(locker != NULL))
...
)
{
    ...
    if (likely(PSI_server && that->m_psi))
    {
        locker = PSI_server->get_thread_rwlock_locker(&state, that->m_psi, PSI_RWLOCK_WRITELOCK);
        if (likely(locker != NULL))
...
)
{
    ...
    if (likely(PSI_server && that->m_psi))
    {
        locker = PSI_server->get_thread_rwlock_locker(&state, that->m_psi, PSI_RWLOCK_TRYREADLOCK);
        if (likely(locker != NULL))
            PSI_server->start_rwlock_rdwait(locker, src_file, src_line);
    }
#endif
...
)
{
    ...
    if (likely(PSI_server && that->m_psi))
    {
        locker = PSI_server->get_thread_rwlock_locker(&state, that->m_psi, PSI_RWLOCK_TRYWRITELOCK);
        if (likely(locker != NULL))
            PSI_server->start_rwlock_wrwait(locker, src_file, src_line);
    }
#endif
...
static inline int inline_mysql_cond_init(
    ...
    PSI_cond_key key,
#endif
    mysql_cond_t* that, const pthread_condattr_t* attr)
{
#ifdef HAVE_PSI_INTERFACE
...
static inline int inline_mysql_cond_wait(mysql_cond_t* that, mysql_mutex_t* mutex
#ifdef HAVE_PSI_INTERFACE
                                         ,
                                         const char* src_file, uint src_line
#endif
)
{
    ...
    if (likely(PSI_server && that->m_psi))
    {
        locker = PSI_server->get_thread_cond_locker(&state, that->m_psi, mutex->m_psi, PSI_COND_WAIT);
        if (likely(locker != NULL))
...
static inline int inline_mysql_cond_timedwait(mysql_cond_t* that, mysql_mutex_t* mutex, const struct timespec* abstime
#ifdef HAVE_PSI_INTERFACE
                                              ,
                                              const char* src_file, uint src_line
#endif
)
{
    ...
    if (likely(PSI_server && that->m_psi))
    {
        locker = PSI_server->get_thread_cond_locker(&state, that->m_psi, mutex->m_psi, PSI_COND_TIMEDWAIT);
        if (likely(locker != NULL))
            PSI_server->start_cond_wait(locker, src_file, src_line);
    }
#endif
...
#ifdef HAVE_PSI_INTERFACE
static inline int inline_mysql_thread_create(PSI_thread_key key, pthread_t* thread, const pthread_attr_t* attr, void* (*start_routine)(void*), void* arg)
{
    int result;
