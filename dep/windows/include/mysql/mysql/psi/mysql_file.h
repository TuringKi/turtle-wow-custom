...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_file_fstat(FN, S, FL) inline_mysql_file_fstat(__FILE__, __LINE__, FN, S, FL)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_file_fseek(FD, P, W, F) inline_mysql_file_fseek(__FILE__, __LINE__, FD, P, W, F)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_file_ftell(FD, F) inline_mysql_file_ftell(__FILE__, __LINE__, FD, F)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_file_create_temp(K, T, D, P, M, F) inline_mysql_file_create_temp(K, T, D, P, M, F)
#else
...
*/
#ifdef HAVE_PSI_INTERFACE
#define mysql_file_create_with_symlink(K, P1, P2, P3, P4, P5) inline_mysql_file_create_with_symlink(K, __FILE__, __LINE__, P1, P2, P3, P4, P5)
#else
    ...
    char* str, int size, MYSQL_FILE* file)
{
    ...
    {
        locker = PSI_server->get_thread_file_stream_locker(&state, file->m_psi, PSI_FILE_READ);
    ...
    const char* str, MYSQL_FILE* file)
{
    ...
    if (likely(PSI_server && file->m_psi))
    {
        locker = PSI_server->get_thread_file_stream_locker(&state, file->m_psi, PSI_FILE_WRITE);
        if (likely(locker != NULL))
        {
...
static inline int inline_mysql_file_fprintf(MYSQL_FILE* file, const char* format, ...)
{
    /*
    ...
    MYSQL_FILE* file)
{
    ...
    if (likely(PSI_server && file->m_psi))
    {
        locker = PSI_server->get_thread_file_stream_locker(&state, file->m_psi, PSI_FILE_FLUSH);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(locker, (size_t)0, src_file, src_line);
    }
#endif
    ...
    int filenr, MY_STAT* stat_area, myf flags)
{
    ...
    {
        locker = PSI_server->get_thread_file_descriptor_locker(&state, filenr, PSI_FILE_FSTAT);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(locker, (size_t)0, src_file, src_line);
    }
#endif
    ...
    const char* path, MY_STAT* stat_area, myf flags)
{
    ...
    if (likely(PSI_server != NULL))
    {
        locker = PSI_server->get_thread_file_name_locker(&state, key, PSI_FILE_STAT, path, &locker);
        if (likely(locker != NULL))
    ...
    File file, my_off_t newlength, int filler, myf flags)
{
    ...
    if (likely(PSI_server != NULL))
    {
        locker = PSI_server->get_thread_file_descriptor_locker(&state, file, PSI_FILE_CHSIZE);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(locker, (size_t)newlength, src_file, src_line);
    }
#endif
    ...
    const char* filename, int flags, myf myFlags)
{
    ...
    if (likely(that != NULL))
    {
        that->m_psi = NULL;
        {
            ...
            if (likely(PSI_server != NULL))
            {
                locker = PSI_server->get_thread_file_name_locker(&state, key, PSI_FILE_STREAM_OPEN, filename, that);
                if (likely(locker != NULL))
                    that->m_psi = PSI_server->start_file_open_wait(locker, src_file, src_line);
            }
#endif
    ...
    MYSQL_FILE* file, myf flags)
{
    ...
    if (likely(file != NULL))
    {
        ...
        if (likely(PSI_server && file->m_psi))
        {
            locker = PSI_server->get_thread_file_stream_locker(&state, file->m_psi, PSI_FILE_STREAM_CLOSE);
            if (likely(locker != NULL))
                PSI_server->start_file_wait(locker, (size_t)0, src_file, src_line);
        }
#endif
    ...
    MYSQL_FILE* file, my_off_t pos, int whence, myf flags)
{
    ...
    if (likely(PSI_server && file->m_psi))
    {
        locker = PSI_server->get_thread_file_stream_locker(&state, file->m_psi, PSI_FILE_SEEK);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(locker, (size_t)0, src_file, src_line);
    }
#endif
    ...
    MYSQL_FILE* file, myf flags)
{
    ...
    if (likely(PSI_server && file->m_psi))
    {
        locker = PSI_server->get_thread_file_stream_locker(&state, file->m_psi, PSI_FILE_TELL);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(locker, (size_t)0, src_file, src_line);
    }
#endif
    ...
    const char* filename, int create_flags, int access_flags, myf myFlags)
{
    ...
    if (likely(PSI_server != NULL))
    {
        locker = PSI_server->get_thread_file_name_locker(&state, key, PSI_FILE_CREATE, filename, &locker);
        if (likely(locker != NULL))
            PSI_server->start_file_open_wait(locker, src_file, src_line);
    }
#endif
    ...
    const char* filename, int flags, myf myFlags)
{
    ...
    if (likely(PSI_server != NULL))
    {
        locker = PSI_server->get_thread_file_name_locker(&state, key, PSI_FILE_OPEN, filename, &locker);
        if (likely(locker != NULL))
            PSI_server->start_file_open_wait(locker, src_file, src_line);
    }
#endif
    ...
    File file, myf flags)
{
    ...
    if (likely(PSI_server != NULL))
    {
        locker = PSI_server->get_thread_file_descriptor_locker(&state, file, PSI_FILE_CLOSE);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(locker, (size_t)0, src_file, src_line);
    }
#endif
    ...
    File file, const uchar* buffer, size_t count, myf flags)
{
    ...
    if (likely(PSI_server != NULL))
    {
        locker = PSI_server->get_thread_file_descriptor_locker(&state, file, PSI_FILE_WRITE);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(locker, count, src_file, src_line);
    }
#endif
    ...
    const char* name, myf flags)
{
    ...
    if (likely(PSI_server != NULL))
    {
        locker = PSI_server->get_thread_file_name_locker(&state, key, PSI_FILE_DELETE, name, &locker);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(locker, (size_t)0, src_file, src_line);
    }
#endif
    ...
    const char* from, const char* to, myf flags)
{
    ...
    if (likely(PSI_server != NULL))
    {
        locker = PSI_server->get_thread_file_name_locker(&state, key, PSI_FILE_RENAME, to, &locker);
        if (likely(locker != NULL))
            PSI_server->start_file_wait(locker, (size_t)0, src_file, src_line);
    }
#endif
...
static inline File inline_mysql_file_create_with_symlink(
    ...
    PSI_file_key key, const char* src_file, uint src_line,
#endif
    const char* linkname, const char* filename, int create_flags, int access_flags, myf flags)
{
    File file;
#ifdef HAVE_PSI_INTERFACE
    struct PSI_file_locker* locker = NULL;
...
#endif
    file = my_create_with_symlink(linkname, filename, create_flags, access_flags, flags);
#ifdef HAVE_PSI_INTERFACE
    if (likely(locker != NULL))
        PSI_server->end_file_open_wait_and_bind_to_descriptor(locker, file);
#endif
