...
typedef void (*my_error_reporter)(enum loglevel level, const char* format, ...) ATTRIBUTE_FORMAT_FPTR(printf, 2, 3);
...
*/
typedef void* (*my_getopt_value)(const char*, uint, const struct my_option*, int*);
...
extern int handle_options(int* argc, char*** argv, const struct my_option* longopts, my_get_one_option);
...
double getopt_double_limit_value(double num, const struct my_option* optp, my_bool* fix);
