...
namespace detail
{
    ...
    public:
        static constexpr const bool value = is_string<T>::value || std::is_convertible<T, std_string_view<char>>::value || !std::is_void<decltype(check<T>(nullptr))>::value;
    };
...
#define FMT_DECLTYPE_RETURN(val)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
    ...
    template <typename T>
    struct has_const_begin_end<T, void_t<decltype(detail::range_begin(std::declval<const remove_cvref_t<T>&>())), decltype(detail::range_end(std::declval<const remove_cvref_t<T>&>()))>> : std::true_type
    {
    };

    template <typename T>
    struct has_mutable_begin_end<T,
                                 void_t<decltype(detail::range_begin(std::declval<T>())), decltype(detail::range_end(std::declval<T>())),
...
// Check for integer_sequence
    ...
    class is_tuple_formattable_<T, C, true>
    {
        ...
        static std::true_type check2(index_sequence<Is...>, integer_sequence<bool, (Is == Is)...>);
        static std::false_type check2(...);
        template <std::size_t... Is>
        static decltype(check2(index_sequence<Is...>{}, integer_sequence<bool, (is_formattable<typename std::tuple_element<Is, T>::type, C>::value)...>{})) check(index_sequence<Is...>);
    ...
    {
        for_each2(tuple_index_sequence<remove_cvref_t<Tuple1>>(), std::forward<Tuple1>(t1), std::forward<Tuple2>(t2), std::forward<F>(f));
    }
...
struct is_range
{
    static constexpr const bool value = detail::is_range_<T>::value && !detail::is_std_string_like<T>::value && !std::is_convertible<T, std::basic_string<Char>>::value && !std::is_convertible<T, detail::std_string_view<Char>>::value;
};
...
{
    template <typename T>
    struct range_format_kind_ : std::integral_constant<range_format,
    ...
    struct range_default_formatter<K, R, Char, enable_if_t<(K == range_format::sequence || K == range_format::map || K == range_format::set)>>
    {
        ...
        FMT_CONSTEXPR void init(range_format_constant<range_format::set>) { underlying_.set_brackets(detail::string_literal<Char, '{'>{}, detail::string_literal<Char, '}'>{}); }
...
template <typename T, typename Char>
struct formatter<T, Char, enable_if_t<conjunction<detail::is_container_adaptor_like<T>, bool_constant<range_format_kind<T, Char>::value == range_format::disabled>>::value>> : formatter<detail::all<typename T::container_type>, Char>
{
