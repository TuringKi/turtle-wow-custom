...
#define FMT_COMPILE(s) FMT_STRING(s)
    ...
    constexpr auto parse_replacement_field_then_tail(S format_str)
    {
        ...
        if constexpr (c == '}')
        {
            return parse_tail<Args, END_POS + 1, NEXT_ID>(field<char_type, typename field_type<T>::type, ARG_INDEX>(), format_str);
        }
        else if constexpr (c != ':')
        {
        ...
        {
            constexpr auto result = parse_specs<typename field_type<T>::type>(str, END_POS + 1, NEXT_ID == manual_indexing_id ? 0 : NEXT_ID);
            ...
            {
                return parse_tail<Args, result.end + 1, result.next_arg_id>(spec_field<char_type, typename field_type<T>::type, ARG_INDEX>{result.fmt}, format_str);
            }
        }
    }
    ...
    constexpr auto compile_format_string(S format_str)
    {
        ...
        if constexpr (str[POS] == '{')
        {
            ...
            else
            {
                ...
                {
                    static_assert(ID == manual_indexing_id || ID == 0, "cannot switch from automatic to manual argument indexing");
                    constexpr auto arg_index = arg_id_result.arg_id.val.index;
                    return parse_replacement_field_then_tail<get_type<arg_index, Args>, Args, arg_id_end_pos, arg_index, manual_indexing_id>(format_str);
                }
                else if constexpr (arg_id_result.arg_id.kind == arg_id_kind::name)
                {
                    ...
                    {
                        constexpr auto next_id = ID != manual_indexing_id ? ID + 1 : manual_indexing_id;
                        return parse_replacement_field_then_tail<decltype(get_type<arg_index, Args>::value), Args, arg_id_end_pos, arg_index, next_id>(format_str);
                    }
                    else if constexpr (c == '}')
                    {
                        return parse_tail<Args, arg_id_end_pos + 1, ID>(runtime_named_field<char_type>{arg_id_result.arg_id.val.name}, format_str);
                    }
                    else if constexpr (c == ':')
                    {
