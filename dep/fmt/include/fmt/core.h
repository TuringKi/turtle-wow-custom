...
#if ((FMT_GCC_VERSION >= 1000 || FMT_CLANG_VERSION >= 1101) && (!defined(__apple_build_version__) || __apple_build_version__ >= 14000029L) && FMT_CPLUSPLUS >= 202002L) || (defined(__cpp_consteval) && (!FMT_MSC_VERSION || _MSC_FULL_VER >= 193030704))
...
#if defined(__cpp_nontype_template_args) && ((FMT_GCC_VERSION >= 903 && FMT_CPLUSPLUS >= 201709L) || __cpp_nontype_template_args >= 201911L) && !defined(__NVCOMPILER) && !defined(__LCC__)
...
#if FMT_CPLUSPLUS >= 202002L && defined(_GLIBCXX_RELEASE) && _GLIBCXX_RELEASE >= 12 && (FMT_CLANG_VERSION >= 1400 && FMT_CLANG_VERSION < 1500)
...
public:
    ...
    FMT_INLINE
    basic_string_view(const Char* s) : data_(s), size_(detail::const_check(std::is_same<Char, char>::value && !detail::is_constant_evaluated(true)) ? std::strlen(reinterpret_cast<const char*>(s)) : std::char_traits<Char>::length(s)) {}

    /** Constructs a string reference from a ``std::basic_string`` object. */
    template <typename Traits, typename Alloc>
    FMT_CONSTEXPR basic_string_view(const std::basic_string<Char, Traits, Alloc>& s) noexcept : data_(s.data()), size_(s.size())
    {
    }
    ...
    FMT_CONSTEXPR_CHAR_TRAITS bool starts_with(basic_string_view<Char> sv) const noexcept { return size_ >= sv.size_ && std::char_traits<Char>::compare(data_, sv.data_, sv.size_) == 0; }
    FMT_CONSTEXPR_CHAR_TRAITS bool starts_with(Char c) const noexcept { return size_ >= 1 && std::char_traits<Char>::eq(*data_, c); }
    FMT_CONSTEXPR_CHAR_TRAITS bool starts_with(const Char* s) const { return starts_with(basic_string_view<Char>(s)); }
    ...
    FMT_CONSTEXPR_CHAR_TRAITS friend auto operator==(basic_string_view lhs, basic_string_view rhs) -> bool { return lhs.compare(rhs) == 0; }
...
public:
    ...
    explicit constexpr basic_format_parse_context(basic_string_view<Char> format_str, int next_arg_id = 0) : format_str_(format_str), next_arg_id_(next_arg_id) {}
...
namespace detail
{
    ...
    public:
        explicit FMT_CONSTEXPR compile_parse_context(basic_string_view<Char> format_str, int num_args, const type* types, int next_arg_id = 0) : base(format_str, next_arg_id), num_args_(num_args), types_(types) {}
    ...
    template <typename Container>
    class iterator_buffer<std::back_insert_iterator<Container>, enable_if_t<is_contiguous<Container>::value, typename Container::value_type>> final : public buffer<typename Container::value_type>
    {
    private:
...
namespace detail
{
    ...
    struct arg_mapper
    {
        ...
        struct formattable : bool_constant<has_const_formatter<U, Context>() || (has_formatter<U, Context>::value && !std::is_const<T>::value)>
        {
        };
    ...
    struct is_output_iterator<It, T, void_t<typename std::iterator_traits<It>::iterator_category, decltype(*std::declval<It>() = std::declval<T>())>> : std::true_type
    {
    };
...
public:
     ...
     */
    constexpr basic_format_context(OutputIt out, format_args ctx_args, detail::locale_ref loc = {}) : out_(out), args_(ctx_args), loc_(loc) {}
...
private:
    ...
    constexpr FMT_INLINE basic_format_args(unsigned long long desc, const detail::value<Context>* values) : desc_(desc), values_(values) {}
...
public:
     ...
     */
    template <typename... Args>
    constexpr FMT_INLINE basic_format_args(const format_arg_store<Context, Args...>& store) : basic_format_args(format_arg_store<Context, Args...>::desc, store.data_.args())
    {
    }
    ...
    /**
     ...
     \endrst
     */
    constexpr FMT_INLINE basic_format_args(const dynamic_format_arg_store<Context>& store) : basic_format_args(store.get_types(), store.data()) {}
    ...
    /**
     ...
     \endrst
     */
    constexpr basic_format_args(const format_arg* args, int count) : basic_format_args(detail::is_unpacked_bit | detail::to_unsigned(count), args) {}
...
struct format_specs
{
    ...
    constexpr format_specs() : width(0), precision(-1), type(presentation_type::none), align(align::none), sign(sign::none), alt(false), localized(false) {}
};
...
namespace detail
{
    ...
    FMT_CONSTEXPR auto parse_nonnegative_int(const Char*& begin, const Char* end, int error_value) noexcept -> int
    {
        ...
        return num_digits == std::numeric_limits<int>::digits10 + 1 && prev * 10ull + unsigned(p[-1] - '0') <= max ? static_cast<int>(value) : error_value;
    }
    ...
    FMT_CONSTEXPR auto parse_format_specs(ParseContext& ctx) -> decltype(ctx.begin())
    {
        ...
        using mapped_type = conditional_t<mapped_type_constant<T, context>::value != type::custom_type, decltype(arg_mapper<context>().map(std::declval<const T&>())), typename strip_named_arg<T>::type>;
#if defined(__cpp_if_constexpr)
    ...
    public:
        explicit FMT_CONSTEXPR format_string_checker(basic_string_view<Char> fmt) : types_{mapped_type_constant<Args, buffer_context<Char>>::value...}, context_(fmt, num_args, types_), parse_funcs_{&parse_format_specs<Args, parse_context_type>...} {}
...
public:
    ...
    {
        static_assert(detail::count<(std::is_base_of<detail::view, remove_reference_t<Args>>::value && std::is_reference<Args>::value)...>() == 0, "passing views as lvalues is disallowed");
