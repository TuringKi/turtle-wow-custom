...
#if defined(__GXX_RTTI) || FMT_HAS_FEATURE(cxx_rtti) || FMT_MSC_VERSION || defined(__INTEL_RTTI__) || defined(__RTTI)
...
namespace detail
{
    ...
    template <>
    inline void write_escaped_path<char>(memory_buffer& quoted, const std::filesystem::path& p)
    {
    ...
    template <>
    inline void write_escaped_path<std::filesystem::path::value_type>(basic_memory_buffer<std::filesystem::path::value_type>& quoted, const std::filesystem::path& p)
    {
...
private:
    ...
    static constexpr basic_string_view<Char> optional = detail::string_literal<Char, 'o', 'p', 't', 'i', 'o', 'n', 'a', 'l', '('>{};
...
struct formatter<Variant, Char, std::enable_if_t<std::conjunction_v<is_variant_like<Variant>, is_variant_formattable<Variant, Char>>>>
{
    template <typename ParseContext>
    FMT_CONSTEXPR auto parse(ParseContext& ctx) -> decltype(ctx.begin())
    {
    ...
    auto format(const Variant& value, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        FMT_CATCH(const std::bad_variant_access&) { detail::write<Char>(out, "valueless by exception"); }
        *out++ = ')';
        return out;
    }
};
...
public:
    ...
    auto format(const std::exception& ex, basic_format_context<OutputIt, Char>& ctx) const -> OutputIt
    {
        ...
        int status = 0;
        std::size_t size = 0;
        std::unique_ptr<char, decltype(&std::free)> demangled_name_ptr(abi::__cxa_demangle(ti.name(), nullptr, &size, &status), &std::free);
...
namespace detail
{
    ...
    {
        static constexpr const bool value = std::is_convertible<T, bool>::value && std::is_nothrow_assignable<T, bool>::value && has_flip<T>::value;
    };
