...
namespace dpp
{
    ...
    guild::guild() : managed(), owner_id(0), afk_channel_id(0), application_id(0), system_channel_id(0), rules_channel_id(0), public_updates_channel_id(0), widget_channel_id(0), member_count(0), flags(0), max_presences(0), max_members(0), shard_id(0), premium_subscription_count(0), afk_timeout(afk_off), max_video_channel_users(0), default_message_notifications(dmn_all), premium_tier(tier_none), verification_level(ver_none), explicit_content_filter(expl_disabled), mfa_level(mfa_none), nsfw_level(nsfw_default), flags_extra(0) {}


    guild_member::guild_member() : guild_id(0), user_id(0), communication_disabled_until(0), joined_at(0), premium_since(0), flags(0) {}

    std::string guild_member::get_mention() const { return "<@" + std::to_string(user_id) + ">"; }
    ...
    bool guild_member::is_communication_disabled() const { return communication_disabled_until > time(nullptr); }
    ...
    std::string guild_member::get_avatar_url(uint16_t size) const
    {
        ...
        if (!this->avatar.to_string().empty())
        {
            ...
            return utility::cdn_host + "/guilds/" + std::to_string(this->guild_id) + "/" + std::to_string(this->user_id) + (has_animated_guild_avatar() ? "/a_" : "/") + this->avatar.to_string() + (has_animated_guild_avatar() ? ".gif" : ".png") + utility::avatar_size(size);
        }
        else
        {
            return std::string();
    ...
    bool guild_member::has_animated_guild_avatar() const { return this->flags & gm_animated_avatar; }
    ...
    dpp::user* guild_member::get_user() const { return dpp::find_user(user_id); }

    bool guild_member::is_deaf() const { return flags & dpp::gm_deaf; }

    bool guild_member::is_muted() const { return flags & dpp::gm_mute; }

    bool guild_member::is_pending() const { return flags & dpp::gm_pending; }

    bool guild::is_large() const { return this->flags & g_large; }

    bool guild::is_unavailable() const { return this->flags & g_unavailable; }

    bool guild::widget_enabled() const { return this->flags & g_widget_enabled; }

    bool guild::has_invite_splash() const { return this->flags & g_invite_splash; }

    bool guild::has_vip_regions() const { return this->flags & g_vip_regions; }

    bool guild::has_vanity_url() const { return this->flags & g_vanity_url; }

    bool guild::has_premium_progress_bar_enabled() const { return this->flags_extra & g_premium_progress_bar_enabled; }

    bool guild::has_invites_disabled() const { return this->flags_extra & g_invites_disabled; }

    bool guild::has_channel_banners() const { return this->flags & g_channel_banners; }

    bool guild::is_verified() const { return this->flags & g_verified; }

    bool guild::is_partnered() const { return this->flags & g_partnered; }

    bool guild::is_community() const { return this->flags & g_community; }

    bool guild::has_commerce() const { return this->flags & g_commerce; }

    bool guild::has_news() const { return this->flags & g_news; }

    bool guild::is_discoverable() const { return this->flags & g_discoverable; }

    bool guild::is_featureable() const { return this->flags & g_featureable; }

    bool guild::has_animated_banner() const { return this->flags_extra & g_animated_banner; }

    bool guild::has_auto_moderation() const { return this->flags_extra & g_auto_moderation; }

    bool guild::has_support_server() const { return this->flags_extra & g_developer_support_server; }

    bool guild::has_animated_icon() const { return this->flags & g_animated_icon; }

    bool guild::has_banner() const { return this->flags & g_banner; }

    bool guild::is_welcome_screen_enabled() const { return this->flags & g_welcome_screen_enabled; }

    bool guild::has_member_verification_gate() const { return this->flags & g_member_verification_gate; }

    bool guild::is_preview_enabled() const { return this->flags & g_preview_enabled; }

    bool guild::has_animated_icon_hash() const { return this->flags & g_has_animated_icon; }

    bool guild::has_animated_banner_hash() const { return this->flags & g_has_animated_banner; }

    bool guild::has_monetization_enabled() const { return this->flags & g_monetization_enabled; }

    bool guild::has_more_stickers() const { return this->flags & g_more_stickers; }

    bool guild::has_private_threads() const { return this->flags & g_private_threads; }

    bool guild::has_role_icons() const { return this->flags & g_role_icons; }

    bool guild::has_seven_day_thread_archive() const { return this->flags & g_seven_day_thread_archive; }

    bool guild::has_three_day_thread_archive() const { return this->flags & g_three_day_thread_archive; }

    bool guild::has_ticketed_events() const { return this->flags & g_ticketed_events; }
    ...
    guild& guild::fill_from_json(nlohmann::json* d) { return fill_from_json(nullptr, d); }
    ...
    std::string guild_widget::build_json(bool with_id) const { return json({{"channel_id", channel_id}, {"enabled", enabled}}).dump(); }
    ...
    std::string guild::get_banner_url(uint16_t size) const
    {
        ...
        {
            return utility::cdn_host + "/banners/" + std::to_string(this->id) + (has_animated_banner_hash() ? "/a_" : "/") + this->banner.to_string() + (has_animated_banner_hash() ? ".gif" : ".png") + utility::avatar_size(size);
        }
        else
        {
            return std::string();
    ...
    std::string guild::get_discovery_splash_url(uint16_t size) const
    {
        ...
        {
            return utility::cdn_host + "/discovery-splashes/" + std::to_string(this->id) + "/" + this->discovery_splash.to_string() + ".png" + utility::avatar_size(size);
        }
        else
        {
            return std::string();
    ...
    std::string guild::get_icon_url(uint16_t size) const
    {
        ...
        {
            return utility::cdn_host + "/icons/" + std::to_string(this->id) + (has_animated_icon_hash() ? "/a_" : "/") + this->icon.to_string() + (has_animated_icon_hash() ? ".gif" : ".png") + utility::avatar_size(size);
        }
        else
        {
            return std::string();
    ...
    std::string guild::get_splash_url(uint16_t size) const
    {
        ...
        {
            return utility::cdn_host + "/splashes/" + std::to_string(this->id) + "/" + this->splash.to_string() + utility::avatar_size(size);
        }
        else
        {
            return std::string();
