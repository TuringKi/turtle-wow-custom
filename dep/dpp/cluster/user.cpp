...
namespace dpp
{
    ...
    void cluster::current_application_get(command_completion_event_t callback) { rest_request<application>(this, API_PATH "/oauth2/applications", "@me", "", m_get, "", callback); }

    void cluster::current_user_get(command_completion_event_t callback) { rest_request<user_identified>(this, API_PATH "/users", "@me", "", m_get, "", callback); }
    ...
    {
        json j({{"channel_id", channel_id}, {"suppress", suppress}});
    ...
    void cluster::current_user_connections_get(command_completion_event_t callback) { rest_request_list<connection>(this, API_PATH "/users", "@me", "connections", m_get, "", callback); }

    void cluster::current_user_get_guilds(command_completion_event_t callback) { rest_request_list<guild>(this, API_PATH "/users", "@me", "guilds", m_get, "", callback); }

    void cluster::current_user_leave_guild(snowflake guild_id, command_completion_event_t callback) { rest_request<confirmation>(this, API_PATH "/users", "@me", "guilds/" + std::to_string(guild_id), m_delete, "", callback); }

    void cluster::user_get(snowflake user_id, command_completion_event_t callback) { rest_request<user_identified>(this, API_PATH "/users", std::to_string(user_id), "", m_get, "", callback); }
