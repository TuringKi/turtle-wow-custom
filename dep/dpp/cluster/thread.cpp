...
namespace dpp
{
    ...
    void cluster::current_user_join_thread(snowflake thread_id, command_completion_event_t callback) { rest_request<confirmation>(this, API_PATH "/channels", std::to_string(thread_id), "/thread-members/@me", m_put, "", callback); }

    void cluster::current_user_leave_thread(snowflake thread_id, command_completion_event_t callback) { rest_request<confirmation>(this, API_PATH "/channels", std::to_string(thread_id), "/thread-members/@me", m_delete, "", callback); }

    void cluster::threads_get_active(snowflake channel_id, command_completion_event_t callback) { rest_request_list<thread>(this, API_PATH "/channels", std::to_string(channel_id), "/threads/active", m_get, "", callback); }
    ...
    void cluster::thread_member_get(const snowflake thread_id, const snowflake user_id, command_completion_event_t callback) { rest_request<thread_member>(this, API_PATH "/channels", std::to_string(thread_id), "/threads-members/" + std::to_string(user_id), m_get, "", callback); }

    void cluster::thread_members_get(snowflake thread_id, command_completion_event_t callback) { rest_request_list<thread_member>(this, API_PATH "/channels", std::to_string(thread_id), "/threads-members", m_get, "", callback); }
    ...
    void cluster::thread_create_in_forum(const std::string& thread_name, snowflake channel_id, const message& msg, auto_archive_duration_t auto_archive_duration, uint16_t rate_limit_per_user, std::vector<snowflake> applied_tags, command_completion_event_t callback)
    {
        ...
        this->post_rest_multipart(
            API_PATH "/channels", std::to_string(channel_id), "threads", m_post, j.dump(),
    ...
    {
        json j({{"name", thread_name}, {"auto_archive_duration", auto_archive_duration}, {"type", thread_type}, {"invitable", invitable}, {"rate_limit_per_user", rate_limit_per_user}});
    ...
    {
        json j({{"name", thread_name}, {"auto_archive_duration", auto_archive_duration}, {"rate_limit_per_user", rate_limit_per_user}});
    ...
    void cluster::thread_member_add(snowflake thread_id, snowflake user_id, command_completion_event_t callback) { rest_request<confirmation>(this, API_PATH "/channels", std::to_string(thread_id), "/thread-members/" + std::to_string(user_id), m_put, "", callback); }

    void cluster::thread_member_remove(snowflake thread_id, snowflake user_id, command_completion_event_t callback) { rest_request<confirmation>(this, API_PATH "/channels", std::to_string(thread_id), "/thread-members/" + std::to_string(user_id), m_delete, "", callback); }
