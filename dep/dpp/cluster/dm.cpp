...
namespace dpp
{
    ...
    void cluster::create_dm_channel(snowflake user_id, command_completion_event_t callback) { rest_request<channel>(this, API_PATH "/users", "@me", "channels", m_post, json({{"recipient_id", std::to_string(user_id)}}).dump(), callback); }

    void cluster::current_user_get_dms(command_completion_event_t callback) { rest_request_list<channel>(this, API_PATH "/users", "@me", "channels", m_get, "", callback); }
    ...
    void cluster::direct_message_create(snowflake user_id, const message& m, command_completion_event_t callback)
    {
        ...
        if (!dm_channel_id)
        {
            this->create_dm_channel(user_id,
                                    [user_id, this, msg, callback](const dpp::confirmation_callback_t& completion)
                                    {
    ...
    void cluster::gdm_remove(snowflake channel_id, snowflake user_id, command_completion_event_t callback) { rest_request<confirmation>(this, API_PATH "/channels", std::to_string(channel_id), "recipients/" + std::to_string(user_id), m_delete, "", callback); }
