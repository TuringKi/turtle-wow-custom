...
namespace dpp
{
    ...
    websocket_client::websocket_client(const std::string& hostname, const std::string& port, const std::string& urlpath, ws_opcode opcode) : ssl_client(hostname, port), key(std::to_string(time(nullptr))), state(HTTP_HEADERS), path(urlpath), data_opcode(opcode) {}
    ...
    ws_state websocket_client::get_state() { return this->state; }
