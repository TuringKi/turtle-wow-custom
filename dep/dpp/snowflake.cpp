...
namespace dpp
{
    ...
    snowflake::operator uint64_t() const { return value; }

    snowflake::operator uint64_t&() { return value; }
    ...
    bool snowflake::operator==(const snowflake& other) const { return other.value == value; }

    bool snowflake::operator==(const uint64_t& other) const { return other == value; }
    ...
    uint8_t snowflake::get_worker_id() const { return (uint8_t)((value & 0x3E0000) >> 17); }

    uint8_t snowflake::get_process_id() const { return (uint8_t)((value & 0x1F000) >> 12); }

    uint16_t snowflake::get_increment() const { return (value & 0xFFF); }
