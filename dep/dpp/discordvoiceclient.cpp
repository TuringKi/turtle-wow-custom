...
namespace dpp
{
    ...
    bool discord_voice_client::voice_payload::operator<(const voice_payload& other) const
    {
        ...
        if ((seq < wrap_around_test_boundary && other.seq >= wrap_around_test_boundary) || (seq >= wrap_around_test_boundary && other.seq < wrap_around_test_boundary))
        {
             ...
             * wider types.
             */
            return static_cast<rtp_seq_t>(seq + wrap_around_test_boundary) > static_cast<rtp_seq_t>(other.seq + wrap_around_test_boundary);
        }
        else
        {
        ...
        while (true)
        {
             ...
             */
            {
                ...
                for (auto& [user_id, parking_lot] : shared_state.parked_voice_payloads)
                {
                    ...
                    flush_data.push_back(flush_data_t{user_id, parking_lot.range.min_seq, std::move(parking_lot.parked_payloads),
                                                      /* Quickly check if we already have a decoder and only take the pending ctls if so. */
                                                      parking_lot.decoder ? std::move(parking_lot.pending_decoder_ctls) : decltype(parking_lot.pending_decoder_ctls){}, parking_lot.decoder});
            ...
            for (auto& d : flush_data)
            {
                ...
                for (rtp_seq_t seq = d.min_seq; !d.parked_payloads.empty(); ++seq)
                {
                    ...
                    if (d.parked_payloads.top().seq != seq)
                    {
                         ...
                         */
                        if (int samples = opus_decode(d.decoder.get(), nullptr, 0, pcm, 5760, 0); samples >= 0)
                        {
                            /*
                             * Since this sample comes from a lost packet,
                             * we can only pretend there is an event, without any raw payload byte.
                             */
                            voice_receive_t vr(nullptr, "", &client, d.user_id, reinterpret_cast<uint8_t*>(pcm), samples * opus_channel_count * sizeof(opus_int16));
                    ...
                    else
                    {
                        ...
                        if (int samples = opus_decode(d.decoder.get(), vr.audio_data.data(), static_cast<opus_int32>(vr.audio_data.length() & 0x7FFFFFFF), pcm, 5760, 0); samples >= 0)
                        {
                            vr.reassign(&client, d.user_id, reinterpret_cast<uint8_t*>(pcm), samples * opus_channel_count * sizeof(opus_int16));
            ...
            if (park_count)
            {
                ...
                voice_receive_t vr(nullptr, "", &client, 0, reinterpret_cast<uint8_t*>(pcm_downsample), max_samples * opus_channel_count * sizeof(opus_int16));
    ...
    discord_voice_client::discord_voice_client(dpp::cluster* _cluster, snowflake _channel_id, snowflake _server_id, const std::string& _token, const std::string& _session_id, const std::string& _host) :
        websocket_client(_host.substr(0, _host.find(":")), _host.substr(_host.find(":") + 1, _host.length()), "/?v=4", OP_TEXT), runner(nullptr), connect_time(0), port(0), ssrc(0), timescale(1000000), paused(false),
#if HAVE_VOICE
        encoder(nullptr), repacketizer(nullptr),
#endif
        fd(INVALID_SOCKET), secret_key(nullptr), sequence(0), timestamp(0), last_timestamp(std::chrono::high_resolution_clock::now()), sending(false), tracks(0), creator(_cluster), terminating(false), heartbeat_interval(0), last_heartbeat(time(nullptr)), token(_token), sessionid(_session_id), server_id(_server_id), channel_id(_channel_id)
    {
#if HAVE_VOICE
    ...
    bool discord_voice_client::is_ready() { return secret_key != nullptr; }
    ...
    int discord_voice_client::udp_recv(char* data, size_t max_length) { return recv(this->fd, data, (int)max_length, 0); }
    ...
    bool discord_voice_client::handle_frame(const std::string& data)
    {
        ...
        if (j.find("op") != j.end())
        {
            ...
            case 12:
                {
                    if (j.find("d") != j.end() && j["d"].find("user_id") != j["d"].end() && !j["d"]["user_id"].is_null() && j["d"].find("ssrc") != j["d"].end() && !j["d"]["ssrc"].is_null() && j["d"]["ssrc"].is_number_integer())
                    {
            ...
            case 2:
                {
                    ...
                    if ((newfd = ::socket(AF_INET, SOCK_DGRAM, 0)) >= 0)
                    {
                        ...
                        this->write(json({{"op", 1}, {"d", {{"protocol", "udp"}, {"data", {{"address", external_ip}, {"port", bound_port}, {"mode", "xsalsa20_poly1305"}}}}}}).dump());
                    }
                }
                break;
            }
    ...
    bool discord_voice_client::is_paused() { return this->paused; }
    ...
    void discord_voice_client::read_ready()
    {
        ...
        if (r > 0 && (!creator->on_voice_receive.empty() || !creator->on_voice_receive_combined.empty()))
        {
            ...
            if (crypto_secretbox_open_easy(encrypted_data, encrypted_data, encrypted_data_len, nonce, secret_key))
            {
...

            {
                ...
                if (!decoder)
                {
                    ...
                    int opus_error = 0;
                    decoder.reset(opus_decoder_create(opus_sample_rate_hz, opus_channel_count, &opus_error), &opus_decoder_destroy);
                    if (opus_error)
                    {
    ...
    dpp::utility::uptime discord_voice_client::get_uptime() { return dpp::utility::uptime(time(nullptr) - connect_time); }

    bool discord_voice_client::is_connected() { return (this->get_state() == CONNECTED); }
    ...
    dpp::socket discord_voice_client::want_read() { return fd; }

    void discord_voice_client::error(uint32_t errorcode)
    {
        const static std::map<uint32_t, std::string> errortext = {{1000, "Socket shutdown"},
    ...
    void discord_voice_client::set_user_gain(snowflake user_id, float factor)
    {
        ...
        voice_courier_shared_state
            ...
            .pending_decoder_ctls.push_back([gain](OpusDecoder& decoder) { opus_decoder_ctl(&decoder, OPUS_SET_GAIN(gain)); });
#endif
    }

    void discord_voice_client::log(dpp::loglevel severity, const std::string& msg) const { creator->log(severity, msg); }
    ...
    discord_voice_client& discord_voice_client::speak()
    {
        if (!this->sending)
        {
            this->queue_message(json({{"op", 5}, {"d", {{"speaking", 1}, {"delay", 0}, {"ssrc", ssrc}}}}).dump(), true);
            sending = true;
        }
        return *this;
    }
    ...
    uint64_t discord_voice_client::get_timescale() { return timescale; }
