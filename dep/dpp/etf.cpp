...
namespace dpp
{
    ...
    json etf_parser::process_atom(const char* atom, uint16_t length)
    {
        ...
        {
            if (length == 4 && *((uint32_t*)atom) == *((uint32_t*)atom_null))
            { // "null"
                return j;
            }
            else if (length == 4 && *((uint32_t*)atom) == *((uint32_t*)atom_true))
            { // "true"
                return true;
            }
            else if (length == 3 && atom[0] == 'n' && atom[1] == 'i' && atom[2] == 'l')
            { // "nil"
                return j;
            }
            else if (length == 5 && *((uint32_t*)atom) == *((uint32_t*)atom_false) && atom[4] == 'e')
            { // "fals","e"
                return false;
            }
        }
    ...
    json etf_parser::decode_tuple(uint32_t length) { return decode_array(length); }

    json etf_parser::decode_nil() { return json::array(); }
    ...
    json etf_parser::decode_tuple_small() { return decode_tuple(read_8_bits()); }

    json etf_parser::decode_tuple_large() { return decode_tuple(read_32_bits()); }
