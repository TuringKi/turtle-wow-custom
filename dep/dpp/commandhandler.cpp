...
namespace dpp
{
    ...
    commandhandler::commandhandler(cluster* o, bool auto_hook_events, snowflake application_id) : slash_commands_enabled(false), owner(o), app_id(application_id)
    {
        ...
        if (auto_hook_events)
        {
            interactions = o->on_slashcommand([this](const dpp::slashcommand_t& event) { this->route(event); });
            messages = o->on_message_create([this](const dpp::message_create_t& event) { this->route(event); });
        }
        else
        {
    ...
    commandhandler& commandhandler::register_commands()
    {
        ...
        {
            owner->guild_bulk_command_create(guild_commands.second, guild_commands.first,
                                             [guild_commands, this](const dpp::confirmation_callback_t& callback)
                                             {
        ...
        owner->global_bulk_command_create(bulk_registration_list_global,
                ...
                case pt_user:
                    {
                        ...
                        {
                            /* Otherwise use interaction resolved fields */
                            if (event.command.resolved.users.find(uid) != event.command.resolved.users.end() && event.command.resolved.members.find(uid) != event.command.resolved.members.end())
                            {
    ...
    void commandhandler::thonk(command_source source, command_completion_event_t callback) { thinking(source, callback); }
