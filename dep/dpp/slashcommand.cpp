...
namespace dpp
{
    ...
    command_option_choice& command_option_choice::fill_from_json(nlohmann::json* j)
    {
        ...
        else if (((*j)["value"].is_string() && snowflake_not_null(j, "value") != 0))
        { // is snowflake (string containing 64 bit integer)
    ...
    command_option::command_option(command_option_type t, const std::string& n, const std::string& d, bool r) : type(t), name(n), description(d), required(r), autocomplete(false) {}
    ...
    void from_json(const nlohmann::json& j, component_interaction& bi)
    {
        ...
        if ((bi.component_type == cot_selectmenu || bi.component_type == cot_user_selectmenu || bi.component_type == cot_role_selectmenu || bi.component_type == cot_mentionable_selectmenu || bi.component_type == cot_channel_selectmenu) && j.find("values") != j.end())
        {
            /* Get values */
    ...
    interaction_response::interaction_response() { msg = new message(); }

    interaction_response::~interaction_response() { delete msg; }
    ...
    interaction_response::interaction_response(interaction_response_type t) : interaction_response() { type = t; }
    ...
    interaction_modal_response::interaction_modal_response(const std::string& _custom_id, const std::string& _title, const std::vector<component> _components) : interaction_response(ir_modal_dialog), current_row(0), custom_id(_custom_id), title(dpp::utility::utf8substr(_title, 0, 45))
    {
        // Default to one empty row
    ...
    command_permission::command_permission(snowflake id, const command_permission_type t, bool permission) : id(id), type(t), permission(permission) {}
    ...
    const dpp::user& interaction::get_resolved_user(snowflake id) const { return get_resolved<user, std::map<snowflake, user>>(id, resolved.users); }

    const dpp::role& interaction::get_resolved_role(snowflake id) const { return get_resolved<role, std::map<snowflake, role>>(id, resolved.roles); }

    const dpp::channel& interaction::get_resolved_channel(snowflake id) const { return get_resolved<channel, std::map<snowflake, channel>>(id, resolved.channels); }

    const dpp::guild_member& interaction::get_resolved_member(snowflake id) const { return get_resolved<guild_member, std::map<snowflake, guild_member>>(id, resolved.members); }

    const dpp::permission& interaction::get_resolved_permission(snowflake id) const { return get_resolved<permission, std::map<snowflake, permission>>(id, resolved.member_permissions); }

    const dpp::message& interaction::get_resolved_message(snowflake id) const { return get_resolved<message, std::map<snowflake, message>>(id, resolved.messages); }

    const dpp::attachment& interaction::get_resolved_attachment(snowflake id) const { return get_resolved<attachment, std::map<snowflake, attachment>>(id, resolved.attachments); }

    const dpp::user& interaction::get_issuing_user() const { return usr; }

    const dpp::message& interaction::get_context_message() const { return msg; }
    ...
    std::string command_interaction::get_mention() const { return dpp::utility::slashcommand_mention(id, name); }

    std::string slashcommand::get_mention() const { return dpp::utility::slashcommand_mention(id, name); }
