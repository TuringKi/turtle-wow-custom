...
namespace dpp
{
    ...
    public:
         ...
         */
        friend inline bool operator<(const snowflake& lhs, const snowflake& rhs) { return lhs.value < rhs.value; }
...
struct std::hash<dpp::snowflake>
{
     ...
     */
    std::size_t operator()(dpp::snowflake const& s) const noexcept { return std::hash<uint64_t>{}(s.value); }
};
