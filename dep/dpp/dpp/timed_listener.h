...
namespace dpp
{
    ...
    public:
         ...
         */
        timed_listener(cluster* cl, uint64_t _duration, attached_event& event, listening_function listener, timer_callback_t on_end = {}) : owner(cl), duration(_duration), ev(event)
        {
            /* Attach event */
            listener_handle = ev(listener);
            /* Create timer */
            th = cl->start_timer(
                [this](dpp::timer timer_handle)
                {
