...
namespace dpp
{
    ...
    protected:
         ...
         */
        void set_warning_callback(std::function<void(const T&)> warning_function) { warning = warning_function; }
    ...
    public:
        ...
        void call(const T& event) const
        {
            ...
            std::for_each(dispatch_container.begin(), dispatch_container.end(),
                          [&](auto& ev)
                          {
...
#ifdef DPP_CORO
            std::for_each(coroutine_container.begin(), coroutine_container.end(),
                          [&](auto& ev)
                          {
                              if (!event.is_cancelled())
                              {
         ...
         */
        event_handle operator()(std::function<void(const T&)> func) { return this->attach(func); }
