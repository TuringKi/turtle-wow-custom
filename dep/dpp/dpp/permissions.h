...
namespace dpp
{
    ...
    public:
         ...
         */
        template <typename... T>
        typename std::enable_if<(std::is_convertible<T, uint64_t>::value && ...), permission&>::type add(T... values)
        {
            value |= (0 | ... | values);
            return *this;
        }
         ...
         */
        template <typename... T>
        typename std::enable_if<(std::is_convertible<T, uint64_t>::value && ...), permission&>::type set(T... values)
        {
            value = (0 | ... | values);
            return *this;
        }
         ...
         */
        template <typename... T>
        typename std::enable_if<(std::is_convertible<T, uint64_t>::value && ...), permission&>::type remove(T... values)
        {
            value &= ~(0 | ... | values);
            return *this;
        }
