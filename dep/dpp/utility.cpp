...
namespace dpp
{
    ...
    namespace utility
    {
        ...
        uint64_t uptime::to_secs() const { return secs + (mins * 60) + (hours * 60 * 60) + (days * 60 * 60 * 24); }

        uint64_t uptime::to_msecs() const { return to_secs() * 1000; }
        ...
        iconhash::iconhash(const std::string& hash) { set(hash); }
        ...
        bool iconhash::operator==(const iconhash& other) const { return other.first == first && other.second == second; }
        ...
        uint32_t rgb(float red, float green, float blue) { return (((uint32_t)(red * 255)) << 16) | (((uint32_t)(green * 255)) << 8) | ((uint32_t)(blue * 255)); }

        /* NOTE: Parameters here are `int` instead of `uint32_t` or `uint8_t` to prevent ambiguity error with rgb(float, float, float) */
        uint32_t rgb(int red, int green, int blue) { return ((uint32_t)red << 16) | ((uint32_t)green << 8) | (uint32_t)blue; }

        void exec(const std::string& cmd, std::vector<std::string> parameters, cmd_result_t callback)
        {
            auto t = std::thread(
                [cmd, parameters, callback]()
                {
        ...
        std::function<void(const dpp::confirmation_callback_t& detail)> log_error()
        {
            return [](const dpp::confirmation_callback_t& detail)
            {
                if (detail.is_error())
                {
                    if (detail.bot)
                    {
                        detail.bot->log(dpp::ll_error, "Error " + std::to_string(detail.get_error().code) + " [" + detail.get_error().message + "] on API request, returned content was: " + detail.http_info.body);
                    }
                }
            };
        }
        ...
        std::string slashcommand_mention(snowflake command_id, const std::string& command_name, const std::string& subcommand) { return "</" + command_name + (!subcommand.empty() ? (" " + subcommand) : "") + ":" + std::to_string(command_id) + ">"; }

        std::string slashcommand_mention(snowflake command_id, const std::string& command_name, const std::string& subcommand_group, const std::string& subcommand) { return "</" + command_name + " " + subcommand_group + " " + subcommand + ":" + std::to_string(command_id) + ">"; }
        ...
        std::string version() { return DPP_VERSION_TEXT; }
