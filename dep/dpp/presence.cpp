...
namespace dpp
{
    ...
    std::string activity::get_large_asset_url(uint16_t size) const
    {
        ...
        if (!this->assets.large_image.empty() && this->application_id && this->assets.large_image.find(':') == std::string::npos)
        { // make sure it's not a prefixed proxy image
    ...
    {
        // https://discord.com/developers/docs/topics/gateway#activity-object-activity-asset-image
        if (!this->assets.small_image.empty() && this->application_id && this->assets.small_image.find(':') == std::string::npos)
        { // make sure it's not a prefixed proxy image
    ...
    activity::activity(const activity_type typ, const std::string& nam, const std::string& stat, const std::string& url_) : name(nam), state(stat), url(url_), type(typ), created_at(0), start(0), end(0), application_id(0), flags(0), is_instance(false) {}
            ...
            for (auto& act : (*j)["activities"])
            {
                ...
                if (act.find("buttons") != act.end())
                {
                    for (auto& b : act["buttons"])
                    {
                        activity_button btn;
                        if (b.is_string())
                        { // its may be just a string (label) because normal bots cannot access the button URLs
                ...
                if (act.find("party") != act.end())
                {
                    ...
                    if (act["party"].find("size") != act["party"].end())
                    { // "size" is an array of two integers
                        try
                        {
    ...
    {
        std::map<presence_status, std::string> status_name_mapping = {{ps_online, "online"}, {ps_offline, "offline"}, {ps_idle, "idle"}, {ps_dnd, "dnd"}};
        json j({

            {"op", 3}, {"d", {{"status", status_name_mapping[status()]}, {"since", json::value_t::null}, {"afk", false}}}});
        if (activities.size())
        {
            for (const auto& i : activities)
            {
                json j2({{"name", i.name}, {"type", i.type}});
                if (!i.url.empty())
                    j2["url"] = i.url;
                if (!i.state.empty())
                    j2["details"] = i.state; // bot activity is details, not state
    ...
    presence_status presence::desktop_status() const { return (presence_status)((flags >> PF_SHIFT_DESKTOP) & PF_STATUS_MASK); }

    presence_status presence::web_status() const { return (presence_status)((flags >> PF_SHIFT_WEB) & PF_STATUS_MASK); }

    presence_status presence::mobile_status() const { return (presence_status)((flags >> PF_SHIFT_MOBILE) & PF_STATUS_MASK); }

    presence_status presence::status() const { return (presence_status)((flags >> PF_SHIFT_MAIN) & PF_STATUS_MASK); }
