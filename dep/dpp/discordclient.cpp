...
namespace dpp
{
    ...
    discord_client::discord_client(dpp::cluster* _cluster, uint32_t _shard_id, uint32_t _max_shards, const std::string& _token, uint32_t _intents, bool comp, websocket_protocol_t ws_proto) : websocket_client(_cluster->default_gateway, "443", comp ? (ws_proto == ws_json ? PATH_COMPRESSED_JSON : PATH_COMPRESSED_ETF) : (ws_proto == ws_json ? PATH_UNCOMPRESSED_JSON : PATH_UNCOMPRESSED_ETF)), terminating(false), runner(nullptr), compressed(comp), decomp_buffer(nullptr), decompressed_total(0), connect_time(0), ping_start(0.0), creator(_cluster), heartbeat_interval(0), last_heartbeat(time(nullptr)), shard_id(_shard_id), max_shards(_max_shards), last_seq(0), token(_token), intents(_intents), sessionid(""), resumes(0), reconnects(0), websocket_ping(0.0), ready(false), last_heartbeat_ack(time(nullptr)), protocol(ws_proto), resume_gateway_url(_cluster->default_gateway)
    {
    ...
    uint64_t discord_client::get_decompressed_bytes_in() { return decompressed_total; }
    ...
    void discord_client::set_resume_hostname() { hostname = resume_gateway_url; }
    ...
    bool discord_client::handle_frame(const std::string& buffer)
    {
        ...
        if (compressed)
        {
            ...
            if ((uint8_t)buffer[buffer.size() - 4] == 0x00 && (uint8_t)buffer[buffer.size() - 3] == 0x00 && (uint8_t)buffer[buffer.size() - 2] == 0xFF && (uint8_t)buffer[buffer.size() - 1] == 0xFF)
            {
                /* Decompress buffer */
                decompressed.clear();
        ...
        if (o != j.end() && !o->is_null())
        {
            ...
            case 10:
                ...
                if (last_seq && !sessionid.empty())
                {
                    ...
                    json obj = {{"op", 6}, {"d", {{"token", this->token}, {"session_id", this->sessionid}, {"seq", this->last_seq}}}};
                ...
                else
                {
                    ...
                    json obj = {{"op", 2}, {"d", {{"token", this->token}, {"properties", {{"os", STRINGIFY(DPP_OS)}, {"browser", "D++"}, {"device", "D++"}}}, {"shard", json::array({shard_id, max_shards})}, {"compress", false}, {"large_threshold", 250}}}};
                    if (this->intents)
                    {
    ...
    dpp::utility::uptime discord_client::get_uptime() { return dpp::utility::uptime(time(NULL) - connect_time); }

    bool discord_client::is_connected() { return (this->get_state() == CONNECTED) && (this->ready); }
    ...
    void discord_client::one_second_timer()
    {
        ...
        if (this->is_connected())
        {
            ...
            if (this->heartbeat_interval && this->last_seq)
            {
                ...
                {
                    queue_message(jsonobj_to_string(json({{"op", 1}, {"d", last_seq}})), true);
    ...
    discord_client& discord_client::connect_voice(snowflake guild_id, snowflake channel_id, bool self_mute, bool self_deaf)
    {
        ...
        if (connecting_voice_channels.find(guild_id) == connecting_voice_channels.end())
        {
            ...
            queue_message(jsonobj_to_string(json({{"op", 4},
                                                  {"d",
                                                   {
    ...
    bool voiceconn::is_ready() { return (!websocket_hostname.empty() && !session_id.empty() && !token.empty()); }

    bool voiceconn::is_active() { return voiceclient != nullptr; }
    ...
    voiceconn::~voiceconn() { this->disconnect(); }
