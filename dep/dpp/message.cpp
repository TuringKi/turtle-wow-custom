...
namespace dpp
{
    ...
    component::component() : type(cot_action_row), label(""), style(cos_primary), custom_id(""), min_values(-1), max_values(-1), min_length(0), max_length(0), disabled(false), required(false)
    {
    ...
    message::message() : managed(0), channel_id(0), guild_id(0), sent(0), edited(0), webhook_id(0), owner(nullptr), type(mt_default), flags(0), pinned(false), tts(false), mention_everyone(false)
    {
    ...
    message::message(class cluster* o) : message() { owner = o; }
    ...
    attachment::attachment(struct message* o) : id(0), size(0), width(0), height(0), ephemeral(false), owner(o) {}
    ...
    std::string message::build_json(bool with_id, bool is_interaction_response) const
    {
        ...
        json j({{"channel_id", channel_id}, {"tts", tts}, {"nonce", nonce}, {"flags", flags}, {"type", type}, {"content", content}});
    ...
    bool message::is_crossposted() const { return flags & m_crossposted; }

    bool message::is_crosspost() const { return flags & m_is_crosspost; }

    bool message::is_dm() const { return guild_id.empty(); }

    bool message::suppress_embeds() const { return flags & m_suppress_embeds; }

    bool message::is_source_message_deleted() const { return flags & m_source_message_deleted; }

    bool message::is_urgent() const { return flags & m_urgent; }

    bool message::has_thread() const { return flags & m_has_thread; }

    bool message::is_ephemeral() const { return flags & m_ephemeral; }

    bool message::is_loading() const { return flags & m_loading; }

    bool message::is_thread_mention_failed() const { return flags & m_thread_mention_failed; }
    ...
    message& message::fill_from_json(json* d, cache_policy_t cp)
    {
        ...
        if (d->find("interaction") != d->end())
        {
            ...
            if (inter.contains("user") && !inter["user"].is_null())
                from_json(inter["user"], interaction.usr);
        }
