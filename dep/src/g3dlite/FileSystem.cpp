...
namespace G3D
{
    ...
    void FileSystem::_setCacheLifetime(float t) { m_cacheLifetime = t; }
    ...
    const Set<std::string>& FileSystem::usedFiles() { return _filesUsed; }
    ...
    void FileSystem::listHelper(const std::string& shortSpec, const std::string& parentPath, Array<std::string>& result, const ListSettings& settings)
    {
        ...
        for (int i = 0; i < dir.nodeArray.size(); ++i)
        {
            ...
            if (FilePath::matches(entry.name, shortSpec, settings.caseSensitive))
            {
                ...
                if ((entry.type == UNKNOWN) && (!(settings.files && settings.directories) || settings.recursive))
                {
                ...
                if ((settings.files && settings.directories) || (settings.files && (entry.type == FILE_TYPE)) || (settings.directories && (entry.type == DIR_TYPE)))
                {
    ...
    std::string FilePath::concat(const std::string& dirname, const std::string& file)
    {
        ...
        if (!dirname.empty() && !isSlash(dirname[dirname.size() - 1]) && (dirname[dirname.size() - 1] != ':'))
        {
            return dirname + '/' + file;
        }
        else
    ...
    bool FilePath::containsWildcards(const std::string& filename) { return (filename.find('*') != std::string::npos) || (filename.find('?') != std::string::npos); }
    ...
    static int fixslash(int c) { return (c == '\\') ? '/' : c; }
    ...
    void FilePath::parse(const std::string& filename, std::string& root, Array<std::string>& path, std::string& base, std::string& ext)
    {
