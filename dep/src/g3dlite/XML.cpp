...
namespace G3D
{
    ...
    XML::XML(TextInput& t) : m_type(VALUE) { deserialize(t); }

    double XML::number() const { return TextInput::parseNumber(m_value); }


    bool XML::boolean() const { return TextInput::parseBoolean(m_value); }
    ...
    begin:
        ...
        if ((n.type() == Token::SYMBOL) && (n.string() == "<"))
        {
            ...
            bool isComment = (n.string() == "!") && (t.peek().type() == Token::SYMBOL) && (t.peek().string() == "--");
