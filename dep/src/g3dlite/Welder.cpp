...
namespace G3D
{
    namespace _internal
    {
        ...
        public:
            ...
            VNTi(const Vector3& v, const Vector3& n, const Vector2& t, int i) : vertex(v), normal(n), texCoord(t), index(i) {}
        };
...
namespace G3D
{
    namespace _internal
    {
        ...
        private:
            ...
            {
                PointHashGrid<VNTi>::SphereIterator it = weldGrid.begin(Sphere(v, vertexWeldRadius));
                ...
                else
                {
                    while (it.isValid())
                    {
                        if (((n - it->normal).squaredLength() <= normalWeldRadius2) && ((t - it->texCoord).squaredLength() <= texCoordWeldRadius2))
                        {
                            // This is the vertex
                            return it->index;
                        }
            ...
            void smoothNormals(const Array<Point3>& vertexArray, const Array<Vector3>& normalArray, Array<Vector3>& smoothNormalArray)
            {
                ...
                else
                {
                    ...
                    for (int v = 0; v < normalArray.size(); ++v)
                    {
                        ...
                        PointHashGrid<VN>::SphereIterator it = grid.begin(Sphere(vertexArray[v], vertexWeldRadius));
        ...
        public:
            ...
            void process(Array<Vector3>& vertexArray, Array<Vector2>& texCoordArray, Array<Vector3>& normalArray, Array<Array<int>*>& indexArrayArray, float normAngle, float texRadius, float normRadius)
            {
                ...
                if (hasTexCoords)
                {
                    debugAssertM(vertexArray.size() == texCoordArray.size(), "Input arrays are not parallel.");
                }
                ...
                unroll(indexArrayArray, vertexArray, texCoordArray, unrolledVertexArray, unrolledTexCoordArray);
            ...
            WeldHelper(float vertRadius) : weldGrid(max(vertRadius, 0.1f), AreaMemoryManager::create()), vertexWeldRadius(vertRadius) {}
        };
    } // namespace _internal
    ...
    void Welder::weld(Array<Vector3>& vertexArray, Array<Vector2>& texCoordArray, Array<Vector3>& normalArray, Array<Array<int>*>& indexArrayArray, const Welder::Settings& settings) { _internal::WeldHelper(settings.vertexWeldRadius).process(vertexArray, texCoordArray, normalArray, indexArrayArray, settings.normalSmoothingAngle, settings.textureWeldRadius, settings.normalWeldRadius); }
