...
namespace G3D
{
    ...
    Box::Box(const Point3& min, const Point3& max) { init(min.min(max), min.max(max)); }


    Box::Box(const Point3& min) { init(min, min); }


    Box::Box(const Point3& min, const Point3& max, const CFrame& c)
    {
    ...
    Box::Box(BinaryInput& b) { deserialize(b); }
    ...
    void Box::deserialize(class BinaryInput& b)
    {
        ...
        _area = 2 * (extent0 * extent1 + extent1 * extent2 + extent2 * extent0);
    }


    void Box::init(const Point3& min, const Point3& max)
    {
        ...
        _area = 2 * (bounds.x * bounds.y + bounds.y * bounds.z + bounds.z * bounds.x);
    }
    ...
    Vector3 Box::corner(int i) const
    {
        ...
        {
        case 0:
            return _center + (0.5f * (-_edgeVector[0] - _edgeVector[1] - _edgeVector[2]));
        case 1:
            return _center + (0.5f * (_edgeVector[0] - _edgeVector[1] - _edgeVector[2]));
        case 2:
            return _center + (0.5f * (-_edgeVector[0] + _edgeVector[1] - _edgeVector[2]));
        ...
        case 6:
            return _center + (0.5f * (-_edgeVector[0] + _edgeVector[1] + _edgeVector[2]));
        default:
            return _center + (0.5f * (_edgeVector[0] + _edgeVector[1] + _edgeVector[2])); // case 7
        }
    }

    float Box::volume() const { return _volume; }


    float Box::area() const { return _area; }
    ...
    void Box::getLocalFrame(CoordinateFrame& frame) const
    {
        ...
        frame.rotation = Matrix3(axis0[0], axis1[0], axis2[0], axis0[1], axis1[1], axis2[1], axis0[2], axis1[2], axis2[2]);
    ...
    void Box::getRandomSurfacePoint(Vector3& P, Vector3& N) const
    {
        ...
        if (r < aXY)
        {
            P = _edgeVector[0] * (float)uniformRandom(-0.5, 0.5) + _edgeVector[1] * (float)uniformRandom(-0.5, 0.5) + _center + _edgeVector[2] * d * 0.5f;
            N = axis(2) * d;
        }
        else if (r < aYZ)
        {
            P = _edgeVector[1] * (float)uniformRandom(-0.5, 0.5) + _edgeVector[2] * (float)uniformRandom(-0.5, 0.5) + _center + _edgeVector[0] * d * 0.5f;
            N = axis(0) * d;
        }
        else
        {
            P = _edgeVector[2] * (float)uniformRandom(-0.5, 0.5) + _edgeVector[0] * (float)uniformRandom(-0.5, 0.5) + _center + _edgeVector[1] * d * 0.5f;
            N = axis(1) * d;
        }
    }
    ...
    Box Box::inf() { return Box(-Vector3::inf(), Vector3::inf()); }
