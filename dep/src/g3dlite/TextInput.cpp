...
namespace G3D
{
    ...
    bool TextInput::parseBoolean(const std::string& _string) { return toLower(_string) == "true"; }
    ...
    double TextInput::parseNumber(const std::string& _string)
    {
        ...
        double n;
        if ((_string.length() > 2) && (_string[0] == '0') && (_string[1] == 'x'))
        {
            // Hex
            uint32 i;
    ...
    int TextInput::peekLineNumber() { return peek().line(); }


    int TextInput::peekCharacterNumber() { return peek().character(); }
    ...
    std::string TextInput::readUntilNewlineAsString() { return readUntilDelimiterAsString('\r', '\n'); }
    ...
    void TextInput::push(const Token& t) { stack.push_front(t); }


    bool TextInput::hasMore() { return (peek()._type != Token::END); }
    ...
    void TextInput::nextToken(Token& t)
    {
        ...
        while (!whitespaceDone)
        {
            ...
            else if (options.otherCommentCharacter && (options.otherCommentCharacter != '\0' && c == options.otherCommentCharacter))
            {
                // set start of line comment and eat markers
                isLineComment = true;
                eatInputChar();
            }
            else if (options.otherCommentCharacter && (options.otherCommentCharacter2 != '\0' && c == options.otherCommentCharacter2))
            {
                // set start of line comment and eat markers
                isLineComment = true;
                eatInputChar();
...
#define SETUP_SYMBOL(c)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               \
    {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
        ...
        case '-': // negative number, -, --, -=, or ->
            ...
            if (options.signedNumbers)
            {
                ...
                else
                {
                    ...
                    if (options.simpleFloatSpecials && (c == 'i') && (peekInputChar(1) == 'n') && (peekInputChar(2) == 'f') && !isLetter(terminal) && (terminal != '_'))
                    {
                        // negative infinity
        ...
        case '\\': // backslash or escaped comment char.
            ...
            if ((options.otherCommentCharacter != '\0' && c == options.otherCommentCharacter) || (options.otherCommentCharacter2 != '\0' && c == options.otherCommentCharacter2))
            {
...
#undef SETUP_SYMBOL
            ...
            else
            {
                ...
                if (c == '.')
                {
                    ...
                    if (options.msvcFloatSpecials && (c == '#'))
                    {
                        ...
                        if (test != 'I')
                        {
                            throw BadMSVCSpecial("Incorrect floating-point special (inf or nan) "
                                                 "format.",
                                                 t.line(), charNumber);
                        }
                        c = eatAndPeekInputChar();
        ...
        while (true)
        {
            ...
            if (options.escapeSequencesInStrings && (c == '\\'))
            {
                ...
                default:
                    ...
                    if (((c == options.otherCommentCharacter) && (options.otherCommentCharacter != '\0')) || ((c == options.otherCommentCharacter2) && (options.otherCommentCharacter2 != '\0')))
                    {
                        t._string += c;
                    }
    ...
    bool TextInput::readBoolean()
    {
        ...
        push(t);
        throw WrongTokenType(options.sourceFileName, t.line(), t.character(), Token::BOOLEAN, t._type);
    }
    ...
    int TextInput::readInteger()
    {
        ...
        if (t._extendedType == Token::INTEGER_TYPE)
        { // common case
            return int(t.number());
        }
        else
        {
            ...
            if (!options.signedNumbers && (t._type == Token::SYMBOL) && ((t._string == "-") || (t._string == "+")))
            {

                Token t2;
                read(t2);

                if ((t2._extendedType == Token::INTEGER_TYPE) && (t2._character == t._character + 1))
                {
            ...
            // Logically, the number started there.
            push(t);
            throw WrongTokenType(options.sourceFileName, t.line(), t.character(), Token::NUMBER, t._type);
        }
    }
    ...
    double TextInput::readNumber()
    {
        ...
        if (!options.signedNumbers && (t._type == Token::SYMBOL) && ((t._string == "-") || (t._string == "+")))
        {
            ...
            if ((t2._type == Token::NUMBER) && (t2._character == t._character + 1))
            {
    ...
    Token TextInput::readStringToken()
    {
        ...
        push(t);
        throw WrongTokenType(options.sourceFileName, t.line(), t.character(), Token::STRING, t._type);
    }

    std::string TextInput::readString() { return readStringToken()._string; }
    ...
    void TextInput::readString(const std::string& s)
    {
        ...
        push(t);
        throw WrongString(options.sourceFileName, t.line(), t.character(), s, t._string);
    }
    ...
    Token TextInput::readCommentToken()
    {
        ...
        if (t._type == Token::COMMENT)
        { // fast path
            return t;
        }

        push(t);
        throw WrongTokenType(options.sourceFileName, t.line(), t.character(), Token::COMMENT, t._type);
    }


    std::string TextInput::readComment() { return readCommentToken()._string; }
    ...
    Token TextInput::readNewlineToken()
    {
        ...
        if (t._type == Token::NEWLINE)
        { // fast path
            return t;
        }

        push(t);
        throw WrongTokenType(options.sourceFileName, t.line(), t.character(), Token::NEWLINE, t._type);
    }

    std::string TextInput::readNewline() { return readNewlineToken()._string; }
    ...
    void TextInput::readSymbolToken(Token& t)
    {
        ...
        push(t);
        throw WrongTokenType(options.sourceFileName, t.line(), t.character(), Token::SYMBOL, t._type);
    }


    std::string TextInput::readSymbol() { return readSymbolToken()._string; }
    ...
    void TextInput::readSymbol(const std::string& symbol)
    {
        ...
        push(t);
        throw WrongSymbol(options.sourceFileName, t.line(), t.character(), symbol, t._string);
    }
    ...
    const std::string& TextInput::filename() const { return options.sourceFileName; }

    ///////////////////////////////////////////////////////////////////////////////////

    TextInput::TokenException::TokenException(const std::string& src, int ln, int ch) : ParseError(src, ln, ch, format("%s(%d) : ", src.c_str(), ln)), sourceFile(src) {}
    ...
    TextInput::WrongTokenType::WrongTokenType(const std::string& src, int ln, int ch, Token::Type e, Token::Type a) : TokenException(src, ln, ch), expected(e), actual(a) { message += format("Expected token of type %s, found type %s.", tokenTypeToString(e), tokenTypeToString(a)); }


    TextInput::BadMSVCSpecial::BadMSVCSpecial(const std::string& src, int ln, int ch) : TokenException(src, ln, ch) {}


    TextInput::WrongSymbol::WrongSymbol(const std::string& src, int ln, int ch, const std::string& e, const std::string& a) : TokenException(src, ln, ch), expected(e), actual(a) { message += format("Expected symbol '%s', found symbol '%s'.", e.c_str(), a.c_str()); }


    TextInput::WrongString::WrongString(const std::string& src, int ln, int ch, const std::string& e, const std::string& a) : TokenException(src, ln, ch), expected(e), actual(a) { message += format("Expected string '%s', found string '%s'.", e.c_str(), a.c_str()); }


    void deserialize(bool& b, TextInput& ti) { b = ti.readSymbol() == "true"; }


    void deserialize(int& b, TextInput& ti) { b = iRound(ti.readNumber()); }


    void deserialize(uint8& b, TextInput& ti) { b = (uint8)iRound(ti.readNumber()); }


    void deserialize(double& b, TextInput& ti) { b = ti.readNumber(); }


    void deserialize(float& b, TextInput& ti) { b = (float)ti.readNumber(); }
