...
namespace G3D
{
    ...
    std::ostream& operator<<(std::ostream& os, const NetAddress& a) { return os << a.toString(); }
    ...
    const char* errnoToString()
    {
        ...
        case ENOTTY:
            return "file descriptor is not associated with a character special device OR "
    ...
    std::string NetworkDevice::formatIP(uint32 addr) { return format("%3d.%3d.%3d.%3d", (addr >> 24) & 0xFF, (addr >> 16) & 0xFF, (addr >> 8) & 0xFF, addr & 0xFF); }


    std::string NetworkDevice::formatMAC(const uint8 MAC[6]) { return format("%02x:%02x:%02x:%02x:%02x:%02x", MAC[0], MAC[1], MAC[2], MAC[3], MAC[4], MAC[5]); }
...
//        WSACleanup();
    ...
    {
        Log::common()->printf("Binding socket %d on port %d ", sock, htons(addr.addr.sin_port));
    ...
    ReliableConduitRef ReliableConduit::create(const NetAddress& address) { return ReliableConduitRef(new ReliableConduit(address)); }


    ReliableConduit::ReliableConduit(const NetAddress& _addr) : state(NO_MESSAGE), receiveBuffer(NULL), receiveBufferTotalSize(0), receiveBufferUsedSize(0)
    {
        ...
        if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (const char*)&T, sizeof(T)) == SOCKET_ERROR)
        {
    ...
    ReliableConduit::ReliableConduit(const SOCKET& _sock, const NetAddress& _addr) : state(NO_MESSAGE), receiveBuffer(NULL), receiveBufferTotalSize(0), receiveBufferUsedSize(0)
    {
        sock = _sock;
        addr = _addr;
    ...
    void ReliableConduit::receiveHeader()
    {
        ...
        if ((ret == SOCKET_ERROR) || (ret != sizeof(messageSize)))
        {
            Log::common()->printf("Call to recv failed.  ret = %d,"
                                  " sizeof(len) = %d\n",
                                  (int)ret, (int)sizeof(messageSize));
            Log::common()->println(socketErrorCode());
            nd->closesocket(sock);
            messageType = 0;
            return;
    ...
    ///////////////////////////////////////////////////////////////////////////////
    LightweightConduitRef LightweightConduit::create(uint16 receivePort, bool enableReceive, bool enableBroadcast) { return LightweightConduitRef(new LightweightConduit(receivePort, enableReceive, enableBroadcast)); }


    LightweightConduit::LightweightConduit(uint16 port, bool enableReceive, bool enableBroadcast)
    {
        NetworkDevice* nd = NetworkDevice::instance();
    ...
    {
        NetworkDevice* nd = NetworkDevice::instance();
        if (sendto(sock, (const char*)b.getCArray(), (int)b.size(), 0, (struct sockaddr*)&(a.addr), sizeof(a.addr)) == SOCKET_ERROR)
        {
    ...
    NetListenerRef NetListener::create(const uint16 port) { return NetListenerRef(new NetListener(port)); }
...
#ifndef G3D_WINDOWS
        return ReliableConduitRef(new ReliableConduit(sClient, NetAddress(htonl(remote_addr.sin_addr.s_addr), ntohs(remote_addr.sin_port))));
#else
        return ReliableConduitRef(ReliableConduitRef(new ReliableConduit(sClient, NetAddress(ntohl(remote_addr.sin_addr.S_un.S_addr), ntohs(remote_addr.sin_port)))));
#endif
    }
