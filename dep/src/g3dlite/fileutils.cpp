...
namespace G3D
{
    ...
    std::string pathConcat(const std::string& dirname, const std::string& file)
    {
        ...
        if ((dirname.size() != 0) && (dirname[dirname.size() - 1] != '/') && (dirname[dirname.size() - 1] != '\\') && (dirname[dirname.size() - 1] != ':'))
        {
            return dirname + '/' + file;
        }
        else
        ...
        else
        {
            ...
            struct zip* z = zip_open(zipfile.c_str(), ZIP_CHECKCONS, NULL);
            {
                ...
                else
                {
                    ...
                    debugAssertM((int64)bytesRead == (int64)info.size, internalFile + " was corrupt because it unzipped to the wrong size.");
                    (void)bytesRead;
                    zip_fclose(zf);
                }
                // Copy the string
    ...
    ///////////////////////////////////////////////////////////////////////////////
    void writeWholeFile(const std::string& filename, const std::string& str, bool flush)
    {
    ...
    static void getFileOrDirListNormal(const std::string& filespec, Array<std::string>& files, bool wantFiles, bool includePath)
    {
        ...
        if (((j != std::string::npos) && (j > i)) || (i == std::string::npos))
        {
            i = j;
        }

        if (((k != std::string::npos) && (k > i)) || (i == std::string::npos))
        {
            i = k;
        }
...
#ifdef G3D_WINDOWS
        {
            ...
            while (result != -1)
            {
                if ((((fileinfo.attrib & _A_SUBDIR) == 0) == test) && strcmp(fileinfo.name, ".") && strcmp(fileinfo.name, ".."))
                {
                ...
                while (entry != NULL)
                {
                    ...
                    if ((strcmp(entry->d_name, ".") != 0) && (strcmp(entry->d_name, "..") != 0))
                    {
                        ...
                        if (exists &&
                            ...
                            (fnmatch(filespec.c_str(), filename.c_str(), FNM_PATHNAME) == 0))
                        {
     ...
     */
    static void _zip_addEntry(const std::string& path, const std::string& prefix, const std::string& file, Set<std::string>& files, bool wantFiles, bool includePath)
    {
    ...
    void getFiles(const std::string& filespec, Array<std::string>& files, bool includePath) { determineFileOrDirList(filespec, files, true, includePath); }


    void getDirs(const std::string& filespec, Array<std::string>& files, bool includePath) { determineFileOrDirList(filespec, files, false, includePath); }
    ...
    bool filenameContainsWildcards(const std::string& filename) { return (filename.find('*') != std::string::npos) || (filename.find('?') != std::string::npos); }
