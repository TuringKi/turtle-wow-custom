...
namespace G3D
{
    ...
    Vector2::Vector2(BinaryInput& b) { deserialize(b); }
    ...
    Vector2 Vector2::operator/(float k) const { return *this * (1.0f / k); }
    ...
    std::string Vector2::toString() const { return G3D::format("(%g, %g)", x, y); }
    ...
    void serialize(const Vector2& v, class BinaryOutput& b) { v.serialize(b); }

    void deserialize(Vector2& v, class BinaryInput& b) { v.deserialize(b); }
