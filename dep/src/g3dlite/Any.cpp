...
namespace G3D
{
    ...
    static bool isContainerType(Any::Type& t) { return (t == Any::ARRAY) || (t == Any::TABLE) || (t == Any::EMPTY_CONTAINER); }
    ...
    bool Any::nameBeginsWith(const std::string& s) const { return nameBeginsWith(s.c_str()); }
    ...
    Any::Any(const std::string& s) : m_type(STRING), m_data(Data::create(STRING)) { *(m_data->value.s) = s; }
    ...
    void Any::deserializeName(TextInput& ti, Token& token, std::string& name)
    {
        ...
        while (!isOpen(s[0]))
        {
            ...
            {
                throw ParseError(ti.filename(), token.line(), token.character(), "Expected symbol while parsing Any");
            }
            s = token.string();
        }
    }
    ...
    void Any::deserialize(TextInput& ti, Token& token)
    {
        ...
        if (token.type() == Token::END)
        {
            ...
            throw ParseError(ti.filename(), token.line(), token.character(), "File ended without a properly formed Any");
        }
        ...
        case Token::SYMBOL:
            ...
            if (token.string() == "#")
            {
                ...
                token = ti.read();
                if (!((token.type() == Token::SYMBOL) && (token.string() == "include")))
                {
                    throw ParseError(ti.filename(), token.line(), token.character(), "Expected 'include' pragma after '#'");
                }
                ...
                m_data->source.filename += format(" [included from %s:%d(%d)]", ti.filename().c_str(), token.line(), token.character());
            ...
            else
            {
                ...
                if (token.type() != Token::SYMBOL)
                {
                    throw ParseError(ti.filename(), token.line(), token.character(), "Malformed Any TABLE or ARRAY; must start with [, (, or {");
                }
        ...
        default:
            throw ParseError(ti.filename(), token.line(), token.character(), "Unexpected token");
    ...
    void Any::readUntilSeparatorOrClose(TextInput& ti, Token& token)
    {
        ...
        while (!(atClose || atSeparator))
        {
            ...
            default:
                throw ParseError(ti.filename(), token.line(), token.character(), "Expected a comma or close paren");
            }
    ...
    */
    ...
    void Any::verifyName(const std::string& n, const std::string& m, const std::string& p) const
    {
        beforeRead();
        const std::string& x = name();
        verify(x == n || x == m || x == p, "Name must be " + n + ", " + m + ", or " + p);
    }
    ...
    void Any::verifyName(const std::string& n, const std::string& m, const std::string& p, const std::string& q) const
    {
        beforeRead();
        const std::string& x = name();
        verify(x == n || x == m || x == p || x == q, "Name must be " + n + ", " + m + ", " + p + ", or " + q);
    }
    ...
    void Any::verifyNameBeginsWith(const std::string& n, const std::string& m) const
    {
        beforeRead();
        const std::string& x = name();
        verify(beginsWith(x, n) || beginsWith(x, m), "Name must be " + n + " or " + m);
    }
    ...
    void Any::verifyNameBeginsWith(const std::string& n, const std::string& m, const std::string& p) const
    {
        beforeRead();
        const std::string& x = name();
        verify(beginsWith(x, n) || beginsWith(x, m) || beginsWith(x, p), "Name must be " + n + ", " + m + ", or " + p);
    }
    ...
    void Any::verifyNameBeginsWith(const std::string& n, const std::string& m, const std::string& p, const std::string& q) const
    {
        beforeRead();
        const std::string& x = name();
        verify(beginsWith(x, n) || beginsWith(x, m) || beginsWith(x, p) || beginsWith(x, q), "Name must be " + n + ", " + m + ", " + p + ", or " + q);
    }
    ...
    void Any::verifyNameBeginsWith(const std::string& n, const std::string& m, const std::string& p, const std::string& q, const std::string& r) const
    {
        beforeRead();
        const std::string& x = name();
        verify(beginsWith(x, n) || beginsWith(x, m) || beginsWith(x, p) || beginsWith(x, q) || beginsWith(x, r), "Name must be " + n + ", " + m + ", " + p + ", or " + q + ", or " + r);
    }
    ...
    void Any::verifyNameBeginsWith(const std::string& n, const std::string& m, const std::string& p, const std::string& q, const std::string& r, const std::string& s) const
    {
        beforeRead();
        const std::string& x = name();
        verify(beginsWith(x, n) || beginsWith(x, m) || beginsWith(x, p) || beginsWith(x, q) || beginsWith(x, r) || beginsWith(x, s), "Name must be " + n + ", " + m + ", " + p + ", or " + q + ", or " + r + ", or " + s);
    }
    ...
    void Any::verifyNameBeginsWith(const std::string& n, const std::string& m, const std::string& p, const std::string& q, const std::string& r, const std::string& s, const std::string& t) const
    {
        beforeRead();
        const std::string& x = name();
        verify(beginsWith(x, n) || beginsWith(x, m) || beginsWith(x, p) || beginsWith(x, q) || beginsWith(x, r) || beginsWith(x, s) || beginsWith(x, t), "Name must be " + n + ", " + m + ", " + p + ", or " + q + ", or " + r + ", or " + s + ", or " + t);
    }
    ...
    {
        beforeRead();
        if ((type() != t0 && !((type() == EMPTY_CONTAINER) && isContainerType(t0))) && (type() != t1 && !((type() == EMPTY_CONTAINER) && isContainerType(t1))))
        {
