...
namespace G3D
{
    ...
    Log::Log(const std::string& filename, int stripFromStackBottom) : stripFromStackBottom(stripFromStackBottom)
    {
...
// Write time is greater than 1ms.  This may be a network drive.... try another file.
    ...
    void __cdecl Log::lazyvprintf(const char* fmt, va_list argPtr) { vfprintf(logFile, fmt, argPtr); }
