...
namespace G3D
{
    ...
    Sphere::Sphere(class BinaryInput& b) { deserialize(b); }
    ...
    std::string Sphere::toString() const { return format("Sphere(<%g, %g, %g>, %g)", center.x, center.y, center.z, radius); }
    ...
    bool Sphere::intersects(const Sphere& other) const { return (other.center - center).length() <= (radius + other.radius); }
    ...
    bool Sphere::culledBy(const Array<Plane>& plane, int& cullingPlaneIndex, const uint32 inMask, uint32& outMask) const { return culledBy(plane.getCArray(), plane.size(), cullingPlaneIndex, inMask, outMask); }


    bool Sphere::culledBy(const Array<Plane>& plane, int& cullingPlaneIndex, const uint32 inMask) const { return culledBy(plane.getCArray(), plane.size(), cullingPlaneIndex, inMask); }
    ...
    Vector3 Sphere::randomSurfacePoint() const { return Vector3::random() * radius + center; }
