...
namespace G3D
{
    ...
    bool Vector3::isNaN() const { return G3D::isNaN(x) || G3D::isNaN(y) || G3D::isNaN(z); }
    ...
    Any Vector3::toAny() const { return toAny("Vector3"); }
    ...
    const Vector3& Vector3::minFinite()
    {
        static const Vector3 v(-FLT_MAX, -FLT_MAX, -FLT_MAX);
        return v;
    }
    const Vector3& Vector3::maxFinite()
    {
        static const Vector3 v(FLT_MAX, FLT_MAX, FLT_MAX);
        return v;
    }
    ...
    size_t Vector3::hashCode() const
    {
        ...
        return HashTrait<uint32>::hashCode(u[0]) ^ HashTrait<uint32>::hashCode(~u[1]) ^ HashTrait<uint32>::hashCode((u[2] << 16) | ~(u[2] >> 16));
    }
    ...
    Vector3::Vector3(TextInput& t) { deserialize(t); }

    Vector3::Vector3(BinaryInput& b) { deserialize(b); }
    ...
    {
        debugAssertM(G3D::fuzzyEq(normal.length(), 1.0f), "cosSphereRandom requires its argument to have unit length");
    ...
    Vector3 Vector3::reflectionDirection(const Vector3& normal) const { return -reflectAbout(normal).direction(); }
    ...
    std::string Vector3::toString() const { return G3D::format("(%g, %g, %g)", x, y, z); }


    //----------------------------------------------------------------------------

    Matrix3 Vector3::cross() const { return Matrix3(0, -z, y, z, 0, -x, -y, x, 0); }


    void serialize(const Vector3::Axis& a, class BinaryOutput& bo) { bo.writeUInt8((uint8)a); }

    void deserialize(Vector3::Axis& a, class BinaryInput& bi) { a = (Vector3::Axis)bi.readUInt8(); }
    ...
    void serialize(const Vector3& v, class BinaryOutput& b) { v.serialize(b); }

    void deserialize(Vector3& v, class BinaryInput& b) { v.deserialize(b); }
