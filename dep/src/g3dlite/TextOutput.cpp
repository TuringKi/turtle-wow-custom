...
namespace G3D
{
    ...
    TextOutput::TextOutput(const TextOutput::Settings& opt) : startingNewLine(true), currentColumn(0), inDQuote(false), filename(""), indentLevel(0), m_currentLine(0) { setOptions(opt); }


    TextOutput::TextOutput(const std::string& fil, const TextOutput::Settings& opt) : startingNewLine(true), currentColumn(0), inDQuote(false), filename(fil), indentLevel(0), m_currentLine(0) { setOptions(opt); }
    ...
    void TextOutput::setIndentLevel(int i)
    {
        ...
        indentSpaces = iClamp(option.spacesPerIndent * indentLevel, 0, option.numColumns - 1);
    }
    ...
    void TextOutput::writeBoolean(bool b) { this->printf("%s ", b ? option.trueSymbol.c_str() : option.falseSymbol.c_str()); }
    ...
    void TextOutput::writeSymbols(const std::string& a, const std::string& b, const std::string& c, const std::string& d, const std::string& e, const std::string& f)
    {
    ...
    void TextOutput::wordWrapIndentAppend(const std::string& str)
    {
        ...
        if ((option.wordWrap == Settings::WRAP_NONE) || (currentColumn + (int)str.size() <= option.numColumns))
        {
        ...
        for (uint32 i = 0; i < str.size(); ++i)
        {
            ...
            if (currentColumn >= cols)
            {
                ...
                while ((k < maxLookBackward) && (lastSpace > 0) && (!((data[(int)lastSpace] == ' ') && unquotedSpace)))
                {
                    --lastSpace;
                    ++k;
                ...
                else
                {
                    ...
                    while ((k < maxLookBackward) && (firstSpace > 0) && (data[(int)firstSpace] == ' '))
                    {
                        --firstSpace;
                        ++k;
                    }
    ...
    void serialize(const bool& b, TextOutput& to) { to.writeSymbol(b ? "true" : "false"); }
