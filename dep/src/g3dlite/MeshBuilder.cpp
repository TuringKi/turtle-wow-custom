...
namespace G3D
{
    ...
    void MeshBuilder::setName(const std::string& n) { name = n; }
    ...
    void MeshBuilder::commit(std::string& n, Array<int>& indexArray, Array<Vector3>& outvertexArray)
    {
        ...
        if (close == MeshBuilder::AUTO_WELD)
        {
            ...
            MeshAlg::computeAreaStatistics(triList, index, minEdgeLen, meanEdgeLen, medianEdgeLen, maxEdgeLen, minFaceArea, meanFaceArea, medianFaceArea, maxFaceArea);
    ...
    void MeshBuilder::addTriangle(const Triangle& t) { addTriangle(t.vertex(0), t.vertex(1), t.vertex(2)); }
