...
namespace G3D
{
    ...
    namespace _internal
    {
        ...
        {
        public:
            BasicThread(const std::string& name, void (*proc)(void*), void* param) : GThread(name), m_wrapperProc(proc), m_param(param) {}

        protected:
            virtual void threadMain() { m_wrapperProc(m_param); }
    ...
    GThread::GThread(const std::string& name) : m_status(STATUS_CREATED), m_name(name)
    {
...
#pragma warning(pop)
    ...
    GThreadRef GThread::create(const std::string& name, void (*proc)(void*), void* param) { return shared_ptr<GThread>(new _internal::BasicThread(name, proc, param)); }


    bool GThread::started() const { return m_status != STATUS_CREATED; }


    int GThread::numCores() { return System::numCores(); }
...
#include <seh.h>
...
#ifdef __MINGW32__
        __seh_try { RaiseException(MS_VC_EXCEPTION, 0, sizeof(info) / sizeof(ULONG_PTR), (ULONG_PTR*)&info); }
    ...
    bool GThread::running() const { return (m_status == STATUS_RUNNING); }


    bool GThread::completed() const { return (m_status == STATUS_COMPLETED); }
