...
namespace G3D
{
    ...
    bool Matrix3::isOrthonormal() const
    {
        ...
        return (G3D::fuzzyEq(X.dot(Y), 0.0f) && G3D::fuzzyEq(Y.dot(Z), 0.0f) && G3D::fuzzyEq(X.dot(Z), 0.0f) && G3D::fuzzyEq(X.squaredMagnitude(), 1.0f) && G3D::fuzzyEq(Y.squaredMagnitude(), 1.0f) && G3D::fuzzyEq(Z.squaredMagnitude(), 1.0f));
    }
    ...
    Matrix3::Matrix3(const float aafEntry[3][3]) { memcpy(elt, aafEntry, 9 * sizeof(float)); }

    //----------------------------------------------------------------------------
    Matrix3::Matrix3(const Matrix3& rkMatrix) { memcpy(elt, rkMatrix.elt, 9 * sizeof(float)); }

    //----------------------------------------------------------------------------
    Matrix3::Matrix3(float fEntry00, float fEntry01, float fEntry02, float fEntry10, float fEntry11, float fEntry12, float fEntry20, float fEntry21, float fEntry22) { set(fEntry00, fEntry01, fEntry02, fEntry10, fEntry11, fEntry12, fEntry20, fEntry21, fEntry22); }

    void Matrix3::set(float fEntry00, float fEntry01, float fEntry02, float fEntry10, float fEntry11, float fEntry12, float fEntry20, float fEntry21, float fEntry22)
    {
    ...
    //----------------------------------------------------------------------------
    bool Matrix3::operator!=(const Matrix3& rkMatrix) const { return !operator==(rkMatrix); }
    ...
    Matrix3 Matrix3::operator*(const Matrix3& rkMatrix) const
    {
        ...
        for (int iRow = 0; iRow < 3; ++iRow)
        {
            for (int iCol = 0; iCol < 3; ++iCol)
            {
                kProd.elt[iRow][iCol] = elt[iRow][0] * rkMatrix.elt[0][iCol] + elt[iRow][1] * rkMatrix.elt[1][iCol] + elt[iRow][2] * rkMatrix.elt[2][iCol];
            }
        }
    ...
    Matrix3& Matrix3::operator*=(const Matrix3& rkMatrix)
    {
        ...
        for (int iRow = 0; iRow < 3; ++iRow)
        {
            for (int iCol = 0; iCol < 3; ++iCol)
            {
                mulMat.elt[iRow][iCol] = elt[iRow][0] * rkMatrix.elt[0][iCol] + elt[iRow][1] * rkMatrix.elt[1][iCol] + elt[iRow][2] * rkMatrix.elt[2][iCol];
            }
        }
    ...
    Matrix3& Matrix3::operator/=(float fScalar) { return *this *= (1.0f / fScalar); }
    ...
    Matrix3 operator*(float fScalar, const Matrix3& rkMatrix) { return (double)fScalar * rkMatrix; }


    Matrix3 operator*(int fScalar, const Matrix3& rkMatrix) { return (double)fScalar * rkMatrix; }
    //----------------------------------------------------------------------------
    ...
    void Matrix3::orthonormalize()
    {
        ...
        // compute q0
        float fInvLength = 1.0f / sqrt(elt[0][0] * elt[0][0] + elt[1][0] * elt[1][0] + elt[2][0] * elt[2][0]);
        ...
        fInvLength = 1.0f / sqrt(elt[0][1] * elt[0][1] + elt[1][1] * elt[1][1] + elt[2][1] * elt[2][1]);
        ...
        fInvLength = 1.0f / sqrt(elt[0][2] * elt[0][2] + elt[1][2] * elt[1][2] + elt[2][2] * elt[2][2]);
            ...
            else
            {
                ...
                if (elt[0][0] >= elt[1][1])
                {
                    ...
                    if (elt[0][0] >= elt[2][2])
                    {
                        ...
                        rkAxis.x = 0.5f * sqrt(elt[0][0] - elt[1][1] - elt[2][2] + 1.0f);
                    ...
                    else
                    {
                        ...
                        rkAxis.z = 0.5f * sqrt(elt[2][2] - elt[0][0] - elt[1][1] + 1.0f);
                ...
                else
                {
                    ...
                    if (elt[1][1] >= elt[2][2])
                    {
                        ...
                        rkAxis.y = 0.5f * sqrt(elt[1][1] - elt[0][0] - elt[2][2] + 1.0f);
    ...
    //----------------------------------------------------------------------------
    Matrix3 Matrix3::fromAxisAngle(const Vector3& _axis, float fRadians) { return fromUnitAxisAngle(_axis.direction(), fRadians); }
    ...
    bool Matrix3::qLAlgorithm(float afDiag[3], float afSubDiag[3])
    {
        ...
        for (int i0 = 0; i0 < 3; ++i0)
        {
            ...
            for (iIter = 0; iIter < iMaxIter; ++iIter)
            {
                ...
                for (i1 = i0; i1 <= 1; ++i1)
                {
                    float fSum = float(G3D::abs(afDiag[i1]) + G3D::abs(afDiag[i1 + 1]));
    ...
    //-----------------------------------------------------------------------------
    std::string Matrix3::toString() const { return G3D::format("[%g, %g, %g; %g, %g, %g; %g, %g, %g]", elt[0][0], elt[0][1], elt[0][2], elt[1][0], elt[1][1], elt[1][2], elt[2][0], elt[2][1], elt[2][2]); }
