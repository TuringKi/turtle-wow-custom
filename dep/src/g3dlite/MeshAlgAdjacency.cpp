...
namespace G3D
{
    ...
    public:
        ...
        public:
            ...
            FaceIndexArray& faceIndex() { return (*m_edgeArray)[m_p].faceIndexArray; }
        ...
        private:
            Iterator(ET& a) : m_i0(-1), m_p(-1), m_array(a), m_edgeArray(NULL), m_end(false) { ++(*this); }
        };
    ...
    void MeshAlg::computeAdjacency(const Array<Vector3>& vertexGeometry, const Array<int>& indexArray, Array<Face>& faceArray, Array<Edge>& edgeArray, Array<Array<int>>& adjacentFaceArray)
    {
    ...
    void MeshAlg::computeAdjacency(const Array<Vector3>& vertexGeometry, const Array<int>& indexArray, Array<Face>& faceArray, Array<Edge>& edgeArray, Array<Vertex>& vertexArray)
    {
        ...
        for (int f = 0; f < faceArray.size(); ++f)
        {
            ...
            const int e0End = (e0 < 0) ? edgeArray[~e0].vertexIndex[0] : edgeArray[e0].vertexIndex[1];

            const int e1Begin = (e1 < 0) ? edgeArray[~e1].vertexIndex[1] : edgeArray[e1].vertexIndex[0];
    ...
    void MeshAlg::weldBoundaryEdges(Array<Face>& faceArray, Array<Edge>& edgeArray, Array<Vertex>& vertexArray)
    {
    ...
    void MeshAlg::weldAdjacency(const Array<Vector3>& originalGeometry, Array<Face>& faceArray, Array<Edge>& edgeArray, Array<Vertex>& vertexArray, float radius)
    {
    ...
    void MeshAlg::debugCheckConsistency(const Array<Face>& faceArray, const Array<Edge>& edgeArray, const Array<Vertex>& vertexArray)
    {
        ...
        for (int e = 0; e < edgeArray.size(); ++e)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                debugAssert((edge.faceIndex[i] == MeshAlg::Face::NONE) || faceArray[edge.faceIndex[i]].containsEdge(e));
