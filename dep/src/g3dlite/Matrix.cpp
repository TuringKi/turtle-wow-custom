...
static inline G3D::Matrix::T negate(G3D::Matrix::T x) { return -x; }
...
#undef INPLACE
    ...
    {
        debugAssertM(vec.cols() == cols(), "A row must be set to a vector of the same size.");
        debugAssertM(vec.rows() == 1, "A row must be set to a row vector.");
    ...
    {
        debugAssertM(vec.rows() == rows(), "A column must be set to a vector of the same size.");
        debugAssertM(vec.cols() == 1, "A column must be set to a column vector.");
    ...
    Matrix::T Matrix::get(int r, int c) const { return impl->get(r, c); }
...
#define TRAMPOLINE_EXPLICIT_1(method)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
    ...
    Matrix3 Matrix::toMatrix3() const
    {
        ...
        return Matrix3(impl->get(0, 0), impl->get(0, 1), impl->get(0, 2), impl->get(1, 0), impl->get(1, 1), impl->get(1, 2), impl->get(2, 0), impl->get(2, 1), impl->get(2, 2));
    }
    ...
    Matrix4 Matrix::toMatrix4() const
    {
        ...
        return Matrix4(impl->get(0, 0), impl->get(0, 1), impl->get(0, 2), impl->get(0, 3), impl->get(1, 0), impl->get(1, 1), impl->get(1, 2), impl->get(1, 3), impl->get(2, 0), impl->get(2, 1), impl->get(2, 2), impl->get(2, 3), impl->get(3, 0), impl->get(3, 1), impl->get(3, 2), impl->get(3, 3));
    }
    ...
    {
        debugAssert(((impl->R == 4) && (impl->C == 1)) || ((impl->R == 1) && (impl->C == 4)));
    ...
    bool Matrix::anyNonZero() const { return impl->anyNonZero(); }


    bool Matrix::allNonZero() const { return impl->allNonZero(); }
...
#undef COMPARE_SCALAR
    ...
    double Matrix::norm() const { return sqrt(normSquared()); }
    ...
    void Matrix::Impl::setZero() { System::memset(data, 0, R * C * sizeof(T)); }
    ...
    void Matrix::Impl::mul(const Impl& B, Impl& out) const
    {
        ...
        debugAssertM((this != &out) && (&B != &out), "Output argument to mul cannot be the same as an input argument.");
...
#undef IMPLEMENT_ARRAY_2
        ...
        case 3:
            {
                ...
                return Matrix::T(elt[0][0] * cofactor00 + elt[0][1] * cofactor10 + elt[0][2] * cofactor20);
            }
    ...
    */
    ...
    Matrix Matrix::svdPseudoInverse(float tolerance) const
    {
        ...
        else
        {
            ...
            */
            ...
            alwaysAssertM(A.cols() == r, "Internal dimension mismatch during pseudoInverse()");
            alwaysAssertM(V.cols() >= r, "Internal dimension mismatch during pseudoInverse()");
    ...
    Matrix Matrix::col2PseudoInverse(const Matrix& B) const
    {
        ...
        const float B2[2][2] = {{B.get(0, 0), B.get(0, 1)}, {B.get(1, 0), B.get(1, 1)}};
    ...
    {
        debugAssertM(R == C,
                     format("Cannot perform Gauss-Jordan inverse on a non-square matrix."
                            " (Argument was %dx%d)",
                            R, C));
