...
namespace G3D
{
    ...
    void System::init()
    {
        ...
        bool success = RegistryUtil::readInt32("HKEY_LOCAL_MACHINE\\HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0", "~MHz", m_cpuSpeed);
        ...
        {
            char c[1024];
            sprintf(c, "%d - %s cores", systemInfo.dwNumberOfProcessors, arch);
            m_cpuArch = c;
        }
        ...
        {
            char c[1000];
            sprintf(c, "Windows %d.%d build %d Platform %d %s", osVersionInfo.dwMajorVersion, osVersionInfo.dwMinorVersion, osVersionInfo.dwBuildNumber, osVersionInfo.dwPlatformId, osVersionInfo.szCSDVersion);
            m_operatingSystem = c;
        }
        else
        {
    ...
    void getG3DVersion(std::string& s)
    {
        ...
        if ((G3D_VER % 100) != 0)
        {
            sprintf(cstr, "G3D Innovation Engine %d.%02d beta %d, %s%s", G3D_VER / 10000, (G3D_VER / 100) % 100, G3D_VER % 100, build, debug);
        }
        else
        {
            sprintf(cstr, "G3D Innovation Engine %d.%02d, %s%s", G3D_VER / 10000, (G3D_VER / 100) % 100, build, debug);
        }
...
#define MARK_LOG()
    ...
    std::string System::findDataFile(const std::string& _full, bool errorIfNotFound, bool caseSensitive)
    {
        MARK_LOG();
        ...
        if (directoryArray.size() == 0)
        {
            ...
            static const std::string subdirs[] = {"font", "gui", "shader", "model", "cubemap", "icon", "material", "image", "md2", "md3", "ifs", "3ds", "sky", "music", "sound", "scene", ""};
...
/** Michael Herf's fastest memset. n32 must be filled with the same
    ...
    RealTime System::time()
    {
        ...
        return ((RealTime)(now.QuadPart - instance().m_start.QuadPart) / instance().m_counterFrequency.QuadPart) + instance().m_realWorldGetTickTime0;
#else
...
#define USERSIZE_FROM_USERPTR(u) (*(size_t*)USERPTR_TO_REALPTR(ptr))
    ...
    public:
        ...
        enum
        {
            maxTinyBuffers = 250000,
            maxSmallBuffers = 40000,
            maxMedBuffers = 5000
        };
    ...
    private:
        ...
        bool inTinyHeap(UserPtr ptr) { return (ptr >= tinyHeap) && (ptr < (uint8*)tinyHeap + maxTinyBuffers * tinyBufferSize); }
...
#define _CrtCheckMemory() ((int)1)
            ...
            if (ptr == NULL)
            {
                if ((System::outOfMemoryCallback() != NULL) && (System::outOfMemoryCallback()(USERSIZE_TO_REALSIZE(bytes), true) == true))
                {
        ...
        std::string performance() const
        {
            if (totalMallocs > 0)
            {
                ...
                return format("malloc performance: %5.1f%% <= %db, %5.1f%% <= %db, "
                              "%5.1f%% <= %db, %5.1f%% > %db",
                              100.0 * mallocsFromTinyPool / total, BufferPool::tinyBufferSize, 100.0 * mallocsFromSmallPool / total, BufferPool::smallBufferSize, 100.0 * mallocsFromMedPool / total, BufferPool::medBufferSize, 100.0 * (1.0 - (double)pooled / total), BufferPool::medBufferSize);
            }
            else
            {
        ...
        std::string status() const { return format("preallocated shared buffers: %5d/%d x %db", maxTinyBuffers - tinyPoolSize, maxTinyBuffers, tinyBufferSize); }
    };
...
//  debugAssert(_CrtIsValidPointer((void*)truePtr, totalBytes, TRUE) );
    ...
    const char* System::getEnv(const std::string& name) { return getenv(name.c_str()); }
    ...
    void System::describeSystem(std::string& s)
    {
    ...
    void System::describeSystem(TextOutput& t)
    {
