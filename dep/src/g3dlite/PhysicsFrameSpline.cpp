...
namespace G3D
{
    ...
    {
        if (beginsWith(any.name(), "PFrameSpline") || beginsWith(any.name(), "PhysicsFrameSpline") || beginsWith(any.name(), "CFrameSpline") || beginsWith(any.name(), "CoordinateFrameSpline") || beginsWith(any.name(), "UprightSpline") || beginsWith(any.name(), "UprightFrameSpline"))
        {
    ...
    {
        if ((extrapolationMode == other.extrapolationMode) && (time.size() == other.size()) && (finalInterval == other.finalInterval) && (control.size() == other.control.size()))
        {
            // Check actual values
    ...
    void PhysicsFrameSpline::correct(PhysicsFrame& frame) const { frame.rotation.unitize(); }
