...
namespace G3D
{
    ...
    NetAddress::NetAddress() { System::memset(&addr, 0, sizeof(addr)); }
    ...
    NetAddress::NetAddress(const std::string& hostname, uint16 port) { init(hostname, port); }


    void NetAddress::init(const std::string& hostname, uint16 port)
    {
    ...
    NetAddress::NetAddress(uint32 hostip, uint16 port) { init(hostip, port); }
    ...
    NetAddress::NetAddress(const SOCKADDR_IN& a) { addr = a; }
    ...
    bool NetAddress::ok() const { return addr.sin_family != 0; }


    std::string NetAddress::ipString() const { return format("%s", inet_ntoa(*(in_addr*)&(addr.sin_addr))); }
