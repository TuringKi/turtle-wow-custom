...
用户补充 5875 客户端崩溃栈后，继续发现 12 条活动船／飞艇的 SQL 路径编号未随 DBC 更新。本项此前未覆盖，是明确的运输数据兼容遗漏。修复范围、客户端栈证据与复测要求见 [client-crash-transport-audit-20260906.md](client-crash-transport-audit-20260906.md)，不能把此前隔离启动成功视作已排除客户端问题。
