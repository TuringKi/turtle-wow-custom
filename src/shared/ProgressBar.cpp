...
BarGoLink::BarGoLink(int row_count) { init(row_count); }

BarGoLink::BarGoLink(uint32 row_count)
{
    assert(row_count < (uint32)ACE_INT32_MAX);
    init((int)row_count);
}

BarGoLink::BarGoLink(uint64 row_count)
{
    assert(row_count < (uint64)ACE_INT32_MAX);
    init((int)row_count);
}
...
void BarGoLink::SetOutputState(bool on) { m_showOutput = on; }
