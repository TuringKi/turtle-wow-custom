...
namespace DBUpdater
{
    ...
    std::unordered_map<std::string, FileMigration> AutoUpdater::LoadFileMigrations(const directory_entry& targetPath) const
    {
        ...
        sLog.outInfo("[DB Auto-Updater] Found %u possible migration%s for %s.", possibleUpdates.size(), possibleUpdates.size() != 1 ? "s" : "", targetPath.path().filename().string().c_str());
    ...
    bool AutoUpdater::ProcessTargetUpdates(const fs::directory_entry& targetPath, DatabaseType* targetDatabase, bool region) const
    {
        ...
        for (auto& fileMigrationPair : fileMigrations)
        {
            ...
            if (dbMigrations.find(hash) != dbMigrations.end()) // already applied, skip
            {
                if (migration.Name != dbMigrations[hash].Name)
                    sLog.outInfo("[DB Auto-Updater] Migration with hash %s was migrated with name %s but now has name %s.", migration.Hash.c_str(), dbMigrations[hash].Name.c_str(), migration.Name.c_str());
        ...
        std::sort(updates.begin(), updates.end(),
                  [](const FileMigration& e1, const FileMigration& e2)
                  {
