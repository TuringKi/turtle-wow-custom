...
enum
{
    ...
    // Others :
    // - Il y a un gangregarde elite a ses pieds, assez relou si on foire le pull. C'est parce que Lord Kazzak est a cot� du PnJ de la quete demoniste. Il devrait patrouiller autour du cratere au sud ouest.
    // - Il passe en evade lorsqu'on s'eloigne trop. En fait il ne devrait jamais evade. Du coup impossible de verif' si ses shadowbolt ont bien 100yards de port�e.
    // - Il repop instantanement apres avoir �t� loot�. (Perso j'suis pas contre, il a des jambieres sympa :lol: )
...
struct boss_lordkazzakAI : public ScriptedAI
{
    ...
    void JustRespawned() { DoScriptText(SAY_INTRO, m_creature); }
    ...
    void JustDied(Unit* /*pKiller*/) { DoScriptText(SAY_DEATH, m_creature); }
...
CreatureAI* GetAI_boss_lordkazzak(Creature* _Creature) { return new boss_lordkazzakAI(_Creature); }
...
CreatureAI* GetAI_boss_spirit_lordkazzak(Creature* _Creature) { return new boss_spirit_lordkazzakAI(_Creature); }
