...
{
    explicit boss_gluthAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
    ...
    void DoSearchZombieChow()
    {
        ...
        for (const auto& chowableZombie : chowableZombies)
        {
            ...
            if (chowableZombie->GetDistance2d(m_creature) < 10.0f) // Distance based on dbc for spellid 289236
            {
