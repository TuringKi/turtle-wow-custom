...
instance_lower_karazhan_halls::instance_lower_karazhan_halls(Map* p_Map) : ScriptedInstance(p_Map) { instance_lower_karazhan_halls::Initialize(); }

void instance_lower_karazhan_halls::Initialize() { m_uiMoroesStage = 0; }
...
InstanceData* GetInstanceData_instance_lower_karazhan_halls(Map* p_Map) { return new instance_lower_karazhan_halls(p_Map); }

struct shadowbane_gluttonAI : public ScriptedAI
{
    shadowbane_gluttonAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_shadowbane_glutton(Creature* pCreature) { return new shadowbane_gluttonAI(pCreature); }

struct phantom_cookAI : public ScriptedAI
{
    phantom_cookAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }

    uint32 m_FireBlastTimer;

    void Reset() override { m_FireBlastTimer = urand(15 * IN_MILLISECONDS, 30 * IN_MILLISECONDS); }
...
CreatureAI* GetAI_phantom_cook(Creature* pCreature) { return new phantom_cookAI(pCreature); }

struct shadowbane_alphaAI : public ScriptedAI
{
    shadowbane_alphaAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_shadowbane_alpha(Creature* pCreature) { return new shadowbane_alphaAI(pCreature); }

struct shadowbane_darkcasterAI : public ScriptedAI
{
    shadowbane_darkcasterAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }

    uint32 m_DarkboltTimer;

    void Reset() override { m_DarkboltTimer = 8 * IN_MILLISECONDS; }
...
CreatureAI* GetAI_shadowbane_darkcaster(Creature* pCreature) { return new shadowbane_darkcasterAI(pCreature); }

struct shadowbane_ragefangAI : public ScriptedAI
{
    shadowbane_ragefangAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_shadowbane_ragefang(Creature* pCreature) { return new shadowbane_ragefangAI(pCreature); }

struct haunted_blacksmithAI : public ScriptedAI
{
    haunted_blacksmithAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }

    uint32 m_SpectralArmorTimer;

    void Reset() override { m_SpectralArmorTimer = urand(6 * IN_MILLISECONDS, 9 * IN_MILLISECONDS); }
...
CreatureAI* GetAI_haunted_blacksmith(Creature* pCreature) { return new haunted_blacksmithAI(pCreature); }

struct phantom_guardsmanAI : public ScriptedAI
{
    phantom_guardsmanAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_phantom_guardsman(Creature* pCreature) { return new phantom_guardsmanAI(pCreature); }

struct haunted_stable_tenderAI : public ScriptedAI
{
    haunted_stable_tenderAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }

    uint32 m_CallSpectralSteedTimer;

    void Reset() override { m_CallSpectralSteedTimer = urand(14 * IN_MILLISECONDS, 18 * IN_MILLISECONDS); }
...
CreatureAI* GetAI_haunted_stable_tender(Creature* pCreature) { return new haunted_stable_tenderAI(pCreature); }

struct shattercage_spearmanAI : public ScriptedAI
{
    shattercage_spearmanAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }

    uint32 m_ShattercageImpaleTimer;

    void Reset() override { m_ShattercageImpaleTimer = urand(10 * IN_MILLISECONDS, 12 * IN_MILLISECONDS); }
...
CreatureAI* GetAI_shattercage_spearman(Creature* pCreature) { return new shattercage_spearmanAI(pCreature); }

struct shattercage_magiskullAI : public ScriptedAI
{
    shattercage_magiskullAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }

    uint32 m_ArcaneExplosionTimer;

    void Reset() override { m_ArcaneExplosionTimer = urand(8 * IN_MILLISECONDS, 12 * IN_MILLISECONDS); }
...
CreatureAI* GetAI_shattercage_magiskull(Creature* pCreature) { return new shattercage_magiskullAI(pCreature); }

struct skitterweb_crawlerAI : public ScriptedAI
{
    skitterweb_crawlerAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }

    uint32 m_LeechingBiteTimer;

    void Reset() override { m_LeechingBiteTimer = urand(2 * IN_MILLISECONDS, 5 * IN_MILLISECONDS); }
...
CreatureAI* GetAI_skitterweb_crawler(Creature* pCreature) { return new skitterweb_crawlerAI(pCreature); }

struct skitterweb_darkfangAI : public ScriptedAI
{
    skitterweb_darkfangAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_skitterweb_darkfang(Creature* pCreature) { return new skitterweb_darkfangAI(pCreature); }

struct skitterweb_venomfangAI : public ScriptedAI
{
    skitterweb_venomfangAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_skitterweb_venomfang(Creature* pCreature) { return new skitterweb_venomfangAI(pCreature); }

struct skitterweb_leaperAI : public ScriptedAI
{
    skitterweb_leaperAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }

    uint32 m_SkitterLeapTimer;

    void Reset() override { m_SkitterLeapTimer = urand(15 * IN_MILLISECONDS, 22 * IN_MILLISECONDS); }
...
CreatureAI* GetAI_skitterweb_leaper(Creature* pCreature) { return new skitterweb_leaperAI(pCreature); }

struct phantom_servantAI : public ScriptedAI
{
    phantom_servantAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_phantom_servant(Creature* pCreature) { return new phantom_servantAI(pCreature); }

struct shadowbane_ambusherAI : public ScriptedAI
{
    shadowbane_ambusherAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_shadowbane_ambusher(Creature* pCreature) { return new shadowbane_ambusherAI(pCreature); }

struct grellkin_shadow_weaverAI : public ScriptedAI
{
    grellkin_shadow_weaverAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_grellkin_shadow_weaver(Creature* pCreature) { return new grellkin_shadow_weaverAI(pCreature); }

struct grellkin_primalistAI : public ScriptedAI
{
    grellkin_primalistAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_grellkin_primalist(Creature* pCreature) { return new grellkin_primalistAI(pCreature); }

struct grellkin_channelerAI : public ScriptedAI
{
    grellkin_channelerAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
CreatureAI* GetAI_grellkin_channeler(Creature* pCreature) { return new grellkin_channelerAI(pCreature); }
...
CreatureAI* GetAI_dark_rider_champion(Creature* pCreature) { return new dark_rider_championAI(pCreature); }
...
struct dark_rider_apprenticeAI : public ScriptedAI
{
    ...
    void Reset() override { m_SoulExchangeTimer = urand(7 * IN_MILLISECONDS, 14 * IN_MILLISECONDS); }
...
CreatureAI* GetAI_dark_rider_apprentice(Creature* pCreature) { return new dark_rider_apprenticeAI(pCreature); }
