...
struct boss_moroesAI : public ScriptedAI
{
    ...
    virtual void GetAIInformation(ChatHandler& Handler) override
    {
        ...
        Handler.PSendSysMessage("Moroes data: InterludeTimer: %u, GlitteringDustTimer: %u, SmokeBombTimer: %u, ShuffleKickTimer: %u"
                                ...
                                m_InterludeTimer, m_GlitteringDustTimer, m_SmokeBombTimer, m_ShuffleKickTimer, m_ShadowBlastTimer, m_MoroesCurseTimer, m_ReflectionTimer, m_TeleportTimer, m_AfterTeleportTimer);
...
bool OnGossipHello_boss_moroes(Player* pPlayer, Creature* pCreature)
{
    ...
    {
        if (pInstance->GetData(DATA_BROOD_QUEEN_ARAXXNA) == DONE && pInstance->GetData(DATA_BLACKWALD_II) == DONE && pInstance->GetData(DATA_CLAWLORD_HOWLFANG) == DONE && pInstance->GetData(DATA_GRIZIKIL) == DONE)
        {
...
CreatureAI* GetAI_boss_moroes(Creature* pCreature) { return new boss_moroesAI(pCreature); }
