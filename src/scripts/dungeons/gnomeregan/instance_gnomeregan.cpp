...
instance_gnomeregan::instance_gnomeregan(Map* pMap) : ScriptedInstance(pMap), m_uiBlastmasterShortfuseGUID(0), m_uiCaveInNorthGUID(0), m_uiCaveInSouthGUID(0), m_uiDoorFinalChamberGUID(0) { Initialize(); }
...
static bool sortFromEastToWest(GameObject* pFirst, GameObject* pSecond) { return pFirst && pSecond && pFirst->GetPositionY() < pSecond->GetPositionY(); }
...
sBombFace* instance_gnomeregan::GetBombFaces() { return m_asBombFaces; }
...
InstanceData* GetInstanceData_instance_gnomeregan(Map* pMap) { return new instance_gnomeregan(pMap); }
