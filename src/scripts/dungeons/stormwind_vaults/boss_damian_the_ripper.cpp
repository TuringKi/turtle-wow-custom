...
{
    explicit boss_damian_the_ripperAI(Creature* c) : ScriptedAI(c) { boss_damian_the_ripperAI::Reset(); }

    void Reset() override { _vanishedAt = 0; }
...
private:
     ...
     */
    [[nodiscard]] bool IsVanished() const { return _vanishedAt != 0; }
     ...
     */
    [[nodiscard]] bool EventCastDarkMendingPredicate()
    {
     ...
     * @return True if the event handler should fire, false if we should requeue the event.
     */
    [[nodiscard]] bool EventCastGougePredicate()
    {
        if (me->IsNonMeleeSpellCasted())
        {
            return false;
     ...
     */
    [[nodiscard]] bool EventCastSinisterStrikePredicate()
    {
        if (me->IsNonMeleeSpellCasted())
        {
            return false;
     ...
     * @return True if the event handler should fire, false if we should requeue the event.
     */
    [[nodiscard]] bool EventCastTheRippersMadnessPredicate()
    {
        if (me->IsNonMeleeSpellCasted())
        {
            return false;
     ...
     * @return True if the event handler should fire, false if we should requeue the event.
     */
    [[nodiscard]] bool EventCastVanishPredicate()
    {
        if (me->IsNonMeleeSpellCasted())
        {
            return false;
...
CreatureAI* GetAI_boss_damian_the_ripper(Creature* pCreature) { return new boss_damian_the_ripperAI(pCreature); }
