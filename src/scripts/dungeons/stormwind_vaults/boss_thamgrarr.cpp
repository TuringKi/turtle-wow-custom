...
{
    explicit boss_thamgrarrAI(Creature* creature) : ScriptedAI(creature) { boss_thamgrarrAI::Reset(); }
...
private:
     ...
     */
    [[nodiscard]] eSpellEvents PopSpellEvent()
    {
     ...
     * \return The event to execute, or EventNone if no action should be taken.
     */
    [[nodiscard]] eTextEvents PopTextEvent()
    {
        // If we're popping events too quickly, return EventNone.
     ...
     */
    [[nodiscard]] bool SpellEventRainOfFirePredicate() const { return !me->IsNonMeleeSpellCasted(); }
     ...
     * \return True if the event handler should fire, false if we should requeue the event.
     */
    [[nodiscard]] bool SpellEventFireballPredicate() const { return !me->IsNonMeleeSpellCasted(); }
     ...
     * \return True if the event handler should fire, false if we should requeue the event.
     */
    [[nodiscard]] bool SpellEventSlamPredicate() const { return !me->IsNonMeleeSpellCasted(); }
     ...
     * \return True if the event handler should fire, false if we should requeue the event.
     */
    [[nodiscard]] bool SpellEventIceBlockPredicate() const
    {
...
CreatureAI* GetAI_boss_thamgrarr(Creature* creature) { return new boss_thamgrarrAI(creature); }
