...
CreatureAI* GetAI_mob_phalanx(Creature* pCreature) { return new mob_phalanxAI(pCreature); }
...
CreatureAI* GetAI_npc_mistress_nagmara(Creature* pCreature) { return new npc_mistress_nagmaraAI(pCreature); }
...
CreatureAI* GetAI_npc_rocknot(Creature* pCreature) { return new npc_rocknotAI(pCreature); }
...
bool GOHello_go_thunderbrew_laguer_keg(Player* pPlayer, GameObject* pGo)
{
    ...
    if (pInstance->GetData(TYPE_THUNDERBREW) == DONE)
    {
        ...
        Creature* pHurley = pPlayer->SummonCreature(NPC_HURLEY, 856.087f, -149.747f, -49.672f, 0.059f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000);
        if (!pHurley)
            return true;
...
struct npc_hurley_blackbreathAI : public ScriptedAI
{
    ...
    void Aggro(Unit* pWho) override { DoScriptText(SAY_HURLEY_AGGRO, m_creature); }
...
CreatureAI* GetAI_npc_hurley_blackbreath(Creature* pCreature) { return new npc_hurley_blackbreathAI(pCreature); }
...
bool GOHello_go_relic_coffer_door(Player* pPlayer, GameObject* pGo)
{
    ...
    {
        if (Creature* pCreature = pPlayer->SummonCreature(RUINEPOIGNE_ENTRY, 819.45f, -348.96f, -50.49f, 0.35f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000))
        {
    ...
    void UpdateAI(uint32 const diff) override
    {
        ...
        if (m_creature->GetHealthPercent() < 51.0f)
        {
            if (BoireLaPotionDeSoins_Timer < diff)
            {
                ...
                BoireLaPotionDeSoins_Timer -= diff;
        }
        ...
        if (FracasserArmure_Timer < diff)
        {
            ...
            FracasserArmure_Timer -= diff;

        DoMeleeAttackIfReady();
    }
};

CreatureAI* GetAI_npc_watchman_doomgrip(Creature* pCreature) { return new npc_watchman_doomgripAI(pCreature); }
...
CreatureAI* GetAI_npc_golem_lord_argelmach(Creature* pCreature) { return new npc_golem_lord_argelmachAI(pCreature); }
...
CreatureAI* GetAI_boss_plugger_spazzring(Creature* pCreature) { return new boss_plugger_spazzringAI(pCreature); }
...
CreatureAI* GetAI_npc_dughal_stormwing(Creature* pCreature) { return new npc_dughal_stormwingAI(pCreature); }
...
struct npc_marshal_reginald_windsorAI : npc_escortAI
{
    ...
    void EnterCombat(Unit* pWho) override
    {
        ...
        case NPC_OGRABISI:
        case NPC_JAZ:
            DoScriptText(SAY_REGINALD_WINDSOR_5_3, m_creature);
            break;
        case NPC_CREST:
            DoScriptText(SAY_REGINALD_WINDSOR_13_2, m_creature);
            break;
        case NPC_SHILL:
            DoScriptText(SAY_REGINALD_WINDSOR_7_4, m_creature);
            break;
        }
...
CreatureAI* GetAI_npc_marshal_reginald_windsor(Creature* pCreature) { return new npc_marshal_reginald_windsorAI(pCreature); }
...
struct npc_marshal_windsorAI : npc_escortAI
{
    ...
    void Aggro(Unit* /*pWho*/) override
    {
        ...
        {
        case 0:
            DoScriptText(SAY_WINDSOR_AGGRO1, m_creature);
            break;
        case 1:
            DoScriptText(SAY_WINDSOR_AGGRO2, m_creature);
            break;
        case 2:
            DoScriptText(SAY_WINDSOR_AGGRO3, m_creature, pPlayer);
            break;
        }
    }
...
CreatureAI* GetAI_npc_marshal_windsor(Creature* pCreature) { return new npc_marshal_windsorAI(pCreature); }
...
struct npc_tobias_seecherAI : npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 0:
            ...
            {
            case 0:
                DoScriptText(SAY_TOBIAS_FREE_1, m_creature);
                break;
            case 1:
                DoScriptText(SAY_TOBIAS_FREE_2, m_creature);
                break;
            }
        case 2:
...
CreatureAI* GetAI_npc_tobias_seecher(Creature* pCreature) { return new npc_tobias_seecherAI(pCreature); }
...
GameObjectAI* GetAI_go_cell_door(GameObject* pGo) { return new go_cell_doorAI(pGo); }
