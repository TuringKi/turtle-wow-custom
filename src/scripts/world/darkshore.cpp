...
static float waveFirstCoords[3][4] = {{NPC_BLACKWOOD_TOTEMIC, 4480.79f, 248.848f, 60.5918f}, {NPC_BLACKWOOD_TOTEMIC, 4470.68f, 260.508f, 59.0824f}, {NPC_BLACKWOOD_URSA, 4461.42f, 271.358f, 61.7843f}};

static float waveLastCoords[3][4] = {{NPC_DARK_STRAND_EXCAVATOR, 3582.69f, 186.224f, 5.41665f}, {NPC_DARK_STRAND_EXCAVATOR, 3573.69f, 221.664f, 4.68143f}, {NPC_DARK_STRAND_ADEPT, 3572.49f, 201.906f, 6.13973f}};
...
struct npc_kerlonianAI : public FollowerAI
{
    ...
    bool GetWaveFirst() { return m_waveFirst; }

    bool GetWaveLast() { return m_waveLast; }

    bool GetReached() { return m_reached; }

    void SetWaveFirst(bool value) { m_waveFirst = value; }

    void SetWaveLast(bool value) { m_waveLast = value; }

    void SetReached(bool value) { m_reached = value; }

    Player* GetLeader() { return GetLeaderForFollower(); }
...
CreatureAI* GetAI_npc_kerlonian(Creature* pCreature) { return new npc_kerlonianAI(pCreature); }
...
CreatureAI* GetAI_npc_triggerQuest5321(Creature* pCreature) { return new npc_triggerQuest5321AI(pCreature); }
...
CreatureAI* GetAI_npc_prospector_remtravel(Creature* pCreature) { return new npc_prospector_remtravelAI(pCreature); }
...
CreatureAI* GetAI_npc_threshwackonator(Creature* pCreature) { return new npc_threshwackonatorAI(pCreature); }
...
CreatureAI* GetAI_npc_therylune(Creature* pCreature) { return new npc_theryluneAI(pCreature); }
...
static const SummonLocation aVolcorSpawnLocs[] = {
    {4630.2f, 22.6f, 70.1f, 2.4f}, {4603.8f, 53.5f, 70.4f, 5.4f}, {4627.5f, 100.4f, 62.7f, 5.8f}, {4692.8f, 75.8f, 56.7f, 3.1f}, {4747.8f, 152.8f, 54.6f, 2.4f}, {4711.7f, 109.1f, 53.5f, 2.4f},
};
// Escape Through Stealth
static SummonLocation aVolcorLocations[] = {{4604.54f, -5.17f, 69.51f, 0.0f}, {4604.26f, -2.02f, 69.42f, 0.0f}, {4607.75f, 3.79f, 70.13f, 0.0f}, {4607.75f, 3.79f, 70.13f, 0.0f}, {4619.77f, 27.47f, 70.40f, 0.0f}, {4640.33f, 33.74f, 68.22f, 0.0f}};
...
CreatureAI* GetAI_npc_volcor(Creature* pCreature) { return new npc_volcorAI(pCreature); }
...
struct npc_rabid_thistle_bearAI : public FollowerAI
{
    ...
    void Reset() override { Rage_Timer = 5000; }
...
CreatureAI* GetAI_npc_rabid_thistle_bear(Creature* pCreature) { return new npc_rabid_thistle_bearAI(pCreature); }
...
{
    npc_tharnariun_treetenderAI(Creature* c) : ScriptedAI(c) { Reset(); }
...
CreatureAI* GetAI_npc_tharnariun_treetender(Creature* _Creature) { return new npc_tharnariun_treetenderAI(_Creature); }
...
static const float m_fSummonPoints[3][3] = {{4984.772f, 596.975f, -1.172f}, {4989.618f, 599.530f, -1.291f}, {4979.620f, 593.845f, -0.881f}};
    ...
    void DoSummon() const
    {
        ...
        case 1:
            for (const auto& position : m_fSummonPoints)
                m_creature->SummonCreature(NPC_MURLOC_COATRUNNER, position[0], position[1], position[2], m_creature->GetOrientation(), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 10 * MINUTE * IN_MILLISECONDS);
            break;
        case 2:
            for (uint8 i = 0; i < 2; ++i)
                m_creature->SummonCreature(NPC_MURLOC_WARRIOR, m_fSummonPoints[i][0], m_fSummonPoints[i][1], m_fSummonPoints[i][2], m_creature->GetOrientation(), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 10 * MINUTE * IN_MILLISECONDS);
            break;
        case 3:
            m_creature->SummonCreature(NPC_MURLOC_HUNTER, m_fSummonPoints[1][0], m_fSummonPoints[1][1], m_fSummonPoints[1][2], m_creature->GetOrientation(), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 10 * MINUTE * IN_MILLISECONDS);
            break;
        }
    }
...
CreatureAI* GetAI_npc_murkdeep(Creature* pCreature) { return new npc_murkdeepAI(pCreature); }
...
bool at_murloc_camp(Player* pPlayer, AreaTriggerEntry const* pAt)
{
    if (pAt->id == AREATRIGGER_MURKDEEP)
    {
        ...
        if (pPlayer && pPlayer->GetQuestStatus(QUEST_WANTED_MURKDEEP) == QUEST_STATUS_INCOMPLETE)
        {
            ...
            pCreature = pPlayer->SummonCreature(NPC_MURKDEEP, m_fSummonPoints[0][0], m_fSummonPoints[0][1], m_fSummonPoints[0][2], pPlayer->GetAngle(m_fSummonPoints[0][0], m_fSummonPoints[0][1]), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 30 * MINUTE * IN_MILLISECONDS, true);
