...
ObjectGuid AhBot::GetAHBplayerGUID() { return ObjectGuid(sAhBotConfig.guid); }
...
void AhBot::ForceUpdate()
{
    ...
    sLog.outString("AhBot auction check finished. %d auctions answered, %d new auctions added. Next check in %d seconds", answered, added, sAhBotConfig.updateInterval);
    updating = false;
}
...
vector<AuctionEntry*> AhBot::LoadAuctions(const AuctionHouseObject::AuctionEntryMap& auctionEntryMap, Category*& category, int& auction)
{
    ...
    for (AuctionHouseObject::AuctionEntryMap::const_iterator itr = auctionEntryMap.begin(); itr != auctionEntryMap.end(); ++itr)
    {
        ...
        {
            sLog.outDetail("%s (x%d) in auction %d: price cannot be determined", item->GetProto()->Name1.c_str(), item->GetCount(), auctionIds[auction]);
            continue;
        }
...
void AhBot::FindMinPrice(const AuctionHouseObject::AuctionEntryMap& auctionEntryMap, AuctionEntry*& entry, Item*& item, uint32* minBid, uint32* minBuyout)
{
    *minBid = 0;
    *minBuyout = 0;
    for (AuctionHouseObject::AuctionEntryMap::const_iterator itr = auctionEntryMap.begin(); itr != auctionEntryMap.end(); ++itr)
...
int AhBot::Answer(int auction, Category* category, ItemBag* inAuctionItems)
{
    ...
    for (vector<AuctionEntry*>::iterator itr = entries.begin(); itr != entries.end(); ++itr)
    {
        ...
        {
            sLog.outDetail("%s (x%d) in auction %d: unavailable item", item->GetProto()->Name1.c_str(), item->GetCount(), auctionIds[auction]);
            continue;
        }
        ...
        {
            sLog.outDetail("%s (x%d) in auction %d: answer count %d > %d (max)", item->GetProto()->Name1.c_str(), item->GetCount(), auctionIds[auction], answerCount, maxAnswerCount);
            continue;
        }

        if (proto->RequiredLevel > sAhBotConfig.maxRequiredLevel || proto->ItemLevel > sAhBotConfig.maxItemLevel)
        {
            sLog.outDetail("%s (x%d) in auction %d: above max required or item level", item->GetProto()->Name1.c_str(), item->GetCount(), auctionIds[auction]);
            continue;
        }
        ...
        if (!price)
        {
            sLog.outDetail("%s (x%d) in auction %d: cannot determine price", item->GetProto()->Name1.c_str(), item->GetCount(), auctionIds[auction]);
            continue;
        }
        ...
        {
            sLog.outDetail("%s (x%d) in auction %d: price %d > %d (buyout price)", proto->Name1.c_str(), item->GetCount(), auctionIds[auction], curPrice, buyoutPrice);
            continue;
        }

        if (availableMoney < curPrice)
        {
            sLog.outDetail("%s (x%d) in auction %d: price %d > %d (available money)", proto->Name1.c_str(), item->GetCount(), auctionIds[auction], curPrice, availableMoney);
            continue;
        }
        ...
        {
            sLog.outDetail("%s (x%d) in auction %d: %d (bid) > %d (minBid)", proto->Name1.c_str(), item->GetCount(), auctionIds[auction], entry->bid, minBid);
            continue;
        }

        if (minBid && entry->startbid && minBid < entry->startbid)
        {
            sLog.outDetail("%s (x%d) in auction %d: %d (startbid) > %d (minBid)", proto->Name1.c_str(), item->GetCount(), auctionIds[auction], entry->startbid, minBid);
            continue;
        }
        ...
        if (time(0) < buytime)
        {
            sLog.outDetail("%s (x%d) in auction %d: will buy/bid in %d seconds", proto->Name1.c_str(), item->GetCount(), auctionIds[auction], buytime - time(0));
            continue;
        }
        ...
        if ((entry->buyout && (entry->bid >= entry->buyout || 100 * (entry->buyout - entry->bid) / price < 25)) && !(minBuyout && entry->buyout && minBuyout < entry->buyout))
        {
            sLog.outDetail("AhBot %d won %s (x%d) in auction %d for %d", bidder, item->GetProto()->Name1.c_str(), item->GetCount(), auctionIds[auction], entry->buyout);
        ...
        {
            sLog.outDetail("AhBot %d placed bid %d for %s (x%d) in auction %d", bidder, entry->bid, item->GetProto()->Name1.c_str(), item->GetCount(), auctionIds[auction]);

            CharacterDatabase.PExecute("UPDATE `auction` SET `buyguid` = '%u',`lastbid` = '%u' WHERE `id` = '%u'", entry->bidder, entry->bid, entry->Id);
            AddToHistory(entry, AHBOT_WON_BID);
        }

        CharacterDatabase.PExecute("DELETE FROM `ahbot_history` WHERE `item` = '%u' AND `won` = 4 AND `auction_house` = '%u' ", proto->ItemId, factions[auctionIds[auction]]);
...
{
    QueryResult* results = CharacterDatabase.PQuery("SELECT MAX(`buytime`) FROM `ahbot_history` WHERE `item` = '%u' AND `won` = '%u' AND `auction_house` = '%u' AND `category` = '%s'", id, type, factions[auctionHouse], category.c_str());
...
{
    CharacterDatabase.PExecute("DELETE FROM `ahbot_history` WHERE `item` = '%u' AND `won` = '%u' AND `auction_house` = '%u' AND `category` = '%s'", id, type, factions[auctionHouse], category.c_str());
    ...
    CharacterDatabase.PExecute("INSERT INTO `ahbot_history` (`buytime`, `item`, `bid`, `buyout`, `category`, `won`, `auction_house`) "
                               ...
                               value, id, 0, 0, category.c_str(), type, factions[auctionHouse]);
}
...
int AhBot::AddAuctions(int auction, Category* category, ItemBag* inAuctionItems)
{
    ...
    for (int32 i = 0; i <= maxAllowedAuctionCount && available.size() > 0 && inAuctionItems->GetCount(category) < maxAllowedAuctionCount; ++i)
    {
        ...
        if (time(0) < sellTime)
        {
            sLog.outDetail("%s in auction %d: will add in %d seconds", proto->Name1.c_str(), auctionIds[auction], sellTime - time(0));
            continue;
        }
        else if (time(0) - sellTime > sAhBotConfig.maxSellInterval)
        {
            sLog.outDetail("%s in auction %d: too old (%d secs)", proto->Name1.c_str(), auctionIds[auction], time(0) - sellTime);
            continue;
        }
...
int AhBot::AddAuction(int auction, Category* category, ItemPrototype const* proto)
{
    ...
    sLog.outDetail("AhBot %d added %d of %s to auction %d for %d..%d", owner, stackCount, proto->Name1.c_str(), auctionIds[auction], bidPrice, buyoutPrice);
    return 1;
}
...
void AhBot::HandleCommand(string command)
{
    ...
    for (int i = 0; i < CategoryList::instance.size(); i++)
    {
        ...
        if (category->Contains(proto))
        {
            ...
            ostringstream out;
            out << proto->Name1 << " (" << category->GetDisplayName() << "), " << category->GetMaxAllowedAuctionCount() << "x" << category->GetMaxAllowedItemAuctionCount(proto) << "x" << category->GetStackCount(proto) << " max"
                << "\n";
            for (int auction = 0; auction < MAX_AUCTIONS; auction++)
            {
                const AuctionHouseEntry* ahEntry = sAuctionHouseStore.LookupEntry(auctionIds[auction]);
                out << "--- auction house " << auctionIds[auction] << "(faction: " << factions[auctionIds[auction]] << ", money: " << GetAvailableMoney(auctionIds[auction]) << ") ---\n";
...
void AhBot::AddToHistory(AuctionEntry* entry, uint32 won)
{
    ...
    updateMarketPrice(proto->ItemId, entry->buyout, entry->auctionHouseEntry->houseId);

    uint32 now = time(0);
    CharacterDatabase.PExecute("INSERT INTO `ahbot_history` (`buytime`, `item`, `bid`, `buyout`, `category`, `won`, `auction_house`) "
                               "VALUES ('%u', '%u', '%u', '%u', '%s', '%u', '%u')",
                               now, entry->itemTemplate, entry->bid ? entry->bid : entry->startbid, entry->buyout, category.c_str(), won, factions[entry->auctionHouseEntry->houseId]);
}
...
uint32 AhBot::GetAvailableMoney(uint32 auctionHouse)
{
    ...
    QueryResult* results = CharacterDatabase.PQuery("SELECT `won`, SUM(`bid`) FROM `ahbot_history` WHERE `auction_house` = '%u' GROUP BY `won` HAVING `won` > 0 ORDER BY `won`", factions[auctionHouse]);
    if (results)
    {
        do
        {
    ...
    results = CharacterDatabase.PQuery("SELECT max(`buytime`) FROM `ahbot_history` WHERE `auction_house` = '%u' AND `won` = '2'", factions[auctionHouse]);
    if (results)
    {
        Field* fields = results->Fetch();
...
bool AhBot::IsBotAuction(uint32 bidder) { return allBidders.find(bidder) != allBidders.end(); }
