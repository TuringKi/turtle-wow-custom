...
uint32 PlayerbotFactory::tradeSkills[] = {SKILL_ALCHEMY, SKILL_ENCHANTING, SKILL_SKINNING, SKILL_TAILORING, SKILL_LEATHERWORKING, SKILL_ENGINEERING, SKILL_HERBALISM, SKILL_MINING, SKILL_BLACKSMITHING, SKILL_COOKING, SKILL_FIRST_AID, SKILL_FISHING};

void PlayerbotFactory::Randomize() { Randomize(true); }
 ...
 */
void PlayerbotFactory::CleanRandomize() { Randomize(false); }
...
void PlayerbotFactory::InitPet()
{
    ...
    if (!pet)
    {
        ...
        for (const auto& [id, co] : sObjectMgr.GetCreatureInfoMap())
        {
            if (!co || !co->isTameable())
            {
                continue;
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            const auto& co = sObjectMgr.GetCreatureInfoMap().at(ids[index]);
            ...
            if (!pet->Create(guid, pos, co.get(), 0))
            {
                delete pet;
                pet = NULL;
                continue;
...
private:
    bool CanKeep(uint32 id)
    {
        ...
        if (proto->Class == ITEM_CLASS_JUNK && proto->SubClass == ITEM_SUBCLASS_JUNK)
        {
            return true;
        }
...
bool PlayerbotFactory::CanEquipWeapon(ItemPrototype const* proto)
{
    ...
    case CLASS_WARLOCK:
        ...
        break;
    case CLASS_WARRIOR:
        if (proto->SubClass != ITEM_SUBCLASS_WEAPON_MACE2 && proto->SubClass != ITEM_SUBCLASS_WEAPON_SWORD2 && proto->SubClass != ITEM_SUBCLASS_WEAPON_MACE && proto->SubClass != ITEM_SUBCLASS_WEAPON_SWORD && proto->SubClass != ITEM_SUBCLASS_WEAPON_GUN && proto->SubClass != ITEM_SUBCLASS_WEAPON_CROSSBOW && proto->SubClass != ITEM_SUBCLASS_WEAPON_BOW && proto->SubClass != ITEM_SUBCLASS_WEAPON_THROWN)
            return false;
        break;
    case CLASS_PALADIN:
    ...
    case CLASS_DRUID:
        ...
        break;
    case CLASS_HUNTER:
        if (proto->SubClass != ITEM_SUBCLASS_WEAPON_AXE2 && proto->SubClass != ITEM_SUBCLASS_WEAPON_SWORD2 && proto->SubClass != ITEM_SUBCLASS_WEAPON_GUN && proto->SubClass != ITEM_SUBCLASS_WEAPON_CROSSBOW && proto->SubClass != ITEM_SUBCLASS_WEAPON_BOW)
            return false;
        break;
    case CLASS_ROGUE:
        if (proto->SubClass != ITEM_SUBCLASS_WEAPON_DAGGER && proto->SubClass != ITEM_SUBCLASS_WEAPON_SWORD && proto->SubClass != ITEM_SUBCLASS_WEAPON_MACE && proto->SubClass != ITEM_SUBCLASS_WEAPON_GUN && proto->SubClass != ITEM_SUBCLASS_WEAPON_CROSSBOW && proto->SubClass != ITEM_SUBCLASS_WEAPON_BOW && proto->SubClass != ITEM_SUBCLASS_WEAPON_THROWN)
            return false;
        break;
    }
...
void PlayerbotFactory::InitEquipment(bool incremental)
{
    ...
    for (uint8 slot = 0; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        ...
        do
        {
            ...
            for (const auto [itemId, proto] : sObjectMgr.GetItemPrototypeMap())
            {
                ...
                if (proto.Class == ITEM_CLASS_ARMOR && (slot == EQUIPMENT_SLOT_HEAD || slot == EQUIPMENT_SLOT_SHOULDERS || slot == EQUIPMENT_SLOT_CHEST || slot == EQUIPMENT_SLOT_WAIST || slot == EQUIPMENT_SLOT_LEGS || slot == EQUIPMENT_SLOT_FEET || slot == EQUIPMENT_SLOT_WRISTS || slot == EQUIPMENT_SLOT_HANDS) && !CanEquipArmor(&proto))
                {
                    continue;
                }
            ...
            if (proto.Class == ITEM_CLASS_WEAPON)
            {
                ...
                if (existingItem)
                {
                    ...
                    case ITEM_SUBCLASS_WEAPON_SWORD:
                        if (proto.SubClass == ITEM_SUBCLASS_WEAPON_AXE || proto.SubClass == ITEM_SUBCLASS_WEAPON_DAGGER || proto.SubClass == ITEM_SUBCLASS_WEAPON_FIST || proto.SubClass == ITEM_SUBCLASS_WEAPON_MACE || proto.SubClass == ITEM_SUBCLASS_WEAPON_SWORD)
                            continue;
                        break;
                    default:
                        if (proto.SubClass != ITEM_SUBCLASS_WEAPON_AXE && proto.SubClass != ITEM_SUBCLASS_WEAPON_DAGGER && proto.SubClass != ITEM_SUBCLASS_WEAPON_FIST && proto.SubClass != ITEM_SUBCLASS_WEAPON_MACE && proto.SubClass != ITEM_SUBCLASS_WEAPON_SWORD)
...
void PlayerbotFactory::InitBags()
{
    ...
    for (const auto& [itemId, protoV] : sObjectMgr.GetItemPrototypeMap())
    {
        const auto* proto = &protoV;
...
void PlayerbotFactory::InitAvailableSpells()
{
    ...
    for (const auto& [id, co] : sObjectMgr.GetCreatureInfoMap())
    {
        ...
        if (co->trainer_type == TRAINER_TYPE_CLASS && co->trainer_type != bot->GetClass())
        {
            continue;
        }
...
{
    QueryResult* results = WorldDatabase.PQuery("SELECT `entry`, `RequiredClasses`, `RequiredRaces` FROM `quest_template` WHERE `QuestLevel` = -1 and `MinLevel` <= '%u'", bot->GetLevel());
    if (!results)
    {
        return;
    }
    ...
    for (int i = 0; i < 15; i++)
    {
        for (list<uint32>::iterator i = ids.begin(); i != ids.end(); ++i)
        {
            ...
            if (!bot->SatisfyQuestClass(quest, false) || !bot->SatisfyQuestRace(quest, false) || !bot->SatisfyQuestStatus(quest, false))
                continue;
...
void PlayerbotFactory::InitMounts()
{
    ...
    for (size_t spellId = 0; spellId < sSpellMgr.GetMaxSpellId(); spellId++)
    {
        ...
        if (spellInfo->GetCastTime(bot) < 500 || spellInfo->GetDuration() != -1)
        {
            continue;
        }
