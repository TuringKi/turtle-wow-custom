...
{
    ostringstream out;
    out << "|cFFFFFF00|Hfound:" << go->GetObjectGuid().GetRawValue() << ":" << go->GetEntry() << ":"
        << "|h[" << go->GetGOInfo()->name << "]|h|r";
    return out.str();
}
...
{
    ostringstream out;
    out << "|cFFFFFFFF" << name << (available >= required ? "|c0000FF00: " : "|c00FF0000: ") << available << "/" << required << "|r";
...
bool ChatHelper::parseable(string text) { return text.find("|H") != string::npos || text == "questitem" || substrContainsInMap<uint32>(text, consumableSubClasses) || substrContainsInMap<uint32>(text, tradeSubClasses) || substrContainsInMap<uint32>(text, itemQualities) || substrContainsInMap<uint32>(text, slots) || substrContainsInMap<ChatMsg>(text, chats) || parseMoney(text) > 0; }
...
string ChatHelper::formatClass(uint8 cls) { return classes[cls]; }

string ChatHelper::formatRace(uint8 race) { return races[race]; }
