 ...
 */
void PacketHandlingHelper::AddHandler(uint16 opcode, string handler) { handlers[opcode] = handler; }
 ...
 */
PlayerbotAI::PlayerbotAI() : PlayerbotAIBase(), bot(NULL), aiObjectContext(NULL), currentEngine(NULL), chatHelper(this), chatFilter(this), accountId(0), security(NULL), master(NULL), currentState(BOT_STATE_NON_COMBAT)
{
...
void PlayerbotAI::UpdateAI(uint32 elapsed)
{
    ...
    if (nextAICheckDelay > sPlayerbotAIConfig.globalCoolDown && bot->IsNonMeleeSpellCasted(true, true, false) && *GetAiObjectContext()->GetValue<bool>("invalid target", "current target"))
    {
 ...
 * @param packet The packet to handle.
 */
void PlayerbotAI::HandleMasterIncomingPacket(const WorldPacket& packet) { masterIncomingPacketHandlers.AddPacket(packet); }

/**
 * Handles outgoing packets to the master.
 * @param packet The packet to handle.
 */
void PlayerbotAI::HandleMasterOutgoingPacket(const WorldPacket& packet) { masterOutgoingPacketHandlers.AddPacket(packet); }
 ...
 */
bool PlayerbotAI::HasStrategy(string name, BotState type) { return engines[type]->HasStrategy(name); }
...
namespace MaNGOS
{
    ...
    public:
        ...
        bool operator()(Unit* u) { return u->GetObjectGuid() == i_guid && i_obj->IsWithinDistInMap(u, i_range); }
...
bool PlayerbotAI::TellMasterNoFacing(string text, PlayerbotSecurityLevel securityLevel)
{
    ...
    if (sPlayerbotAIConfig.whisperDistance && !bot->GetGroup() && sRandomPlayerbotMgr.IsRandomBot(bot) && master->GetSession()->GetSecurity() < SEC_ADMINISTRATOR && (bot->GetMapId() != master->GetMapId() || bot->GetDistance(master) > sPlayerbotAIConfig.whisperDistance))
    {
        return false;
    }
 ...
 */
bool PlayerbotAI::CanCastSpell(string name, Unit* target) { return CanCastSpell(aiObjectContext->GetValue<uint32>("spell id", name)->Get(), target); }
...
bool PlayerbotAI::CanCastSpell(uint32 spellid, Unit* target, bool checkHasSpell)
{
    ...
    bool positiveSpell = Spells::IsPositiveSpell(spellid);
...
bool PlayerbotAI::CastSpell(uint32 spellId, Unit* target)
{
    ...
    if (bot->IsMoving() && pSpellInfo->GetCastTime(bot))
    {
        return false;
    }
    ...
    if (pSpellInfo->Effect[0] == SPELL_EFFECT_OPEN_LOCK || pSpellInfo->Effect[0] == SPELL_EFFECT_SKINNING)
    {
...
void PlayerbotAI::WaitForSpellCast(uint32 spellId)
{
    ...
    float castTime = pSpellInfo->GetCastTime(bot) + sPlayerbotAIConfig.reactDelay;
...
bool PlayerbotAI::IsInterruptableSpellCasting(Unit* target, string spell)
{
    ...
    for (int32 i = EFFECT_INDEX_0; i <= EFFECT_INDEX_2; i++)
    {
        ...
        if ((spellInfo->Effect[i] == SPELL_EFFECT_INTERRUPT_CAST) && !target->IsImmuneToSpellEffect(spellInfo, (SpellEffectIndex)i, true))
        {
            return true;
        }
    }
...
bool PlayerbotAI::canDispel(const SpellEntry* entry, uint32 dispelType)
{
    ...
    return entry->SpellName[0].empty() || (strcmpi(entry->SpellName[0].c_str(), "demon skin") && strcmpi(entry->SpellName[0].c_str(), "mage armor") && strcmpi(entry->SpellName[0].c_str(), "frost armor") && strcmpi(entry->SpellName[0].c_str(), "wavering will") && strcmpi(entry->SpellName[0].c_str(), "chilled") && strcmpi(entry->SpellName[0].c_str(), "ice armor"));
}
 ...
 */
bool IsAlliance(uint8 race) { return race == RACE_HUMAN || race == RACE_DWARF || race == RACE_NIGHTELF || race == RACE_GNOME; }
 ...
 */
bool PlayerbotAI::IsOpposing(uint8 race1, uint8 race2) { return (IsAlliance(race1) && !IsAlliance(race2)) || (!IsAlliance(race1) && IsAlliance(race2)); }
