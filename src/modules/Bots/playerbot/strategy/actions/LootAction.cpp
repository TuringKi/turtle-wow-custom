...
uint32 OpenLootAction::GetOpeningSpell(LootObject& lootObject, GameObject* go)
{
    ...
    for (uint32 spellId = 0; spellId < sSpellMgr.GetMaxSpellId(); spellId++)
    {
        if (spellId == MINING || spellId == HERB_GATHERING)
        {
            continue;
...
bool StoreLootAction::IsLootAllowed(uint32 itemid)
{
    ...
    if (proto->StartQuest || proto->Bonding == BIND_QUEST_ITEM || proto->Bonding == BIND_QUEST_ITEM1 || proto->Class == ITEM_CLASS_QUEST)
        return true;
