...
bool WhoAction::Execute(Event event)
{
    ...
    bot->GetSession()->GetMasterPlayer()->Whisper(tell, LANG_UNIVERSAL, owner->GetSession()->GetMasterPlayer(), true);
    return true;
}
...
string WhoAction::QuerySkill(string text)
{
    ...
    for (map<uint32, string>::iterator i = skills.begin(); i != skills.end(); ++i)
    {
        ...
        if (!strcmpi(text.c_str(), name.c_str()) && bot->HasSkill(skill))
        {
            ...
            string data = "0";
            out << "|cFFFFFF00|Htrade:" << spellId << ":" << value << ":" << maxSkill << ":" << std::hex << std::uppercase << guid.GetRawValue() << std::nouppercase << std::dec << ":" << data << "|h[" << skills[skill] << "]|h|r"
