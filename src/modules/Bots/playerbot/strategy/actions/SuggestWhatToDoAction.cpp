...
void SuggestWhatToDoAction::specificQuest()
{
    ...
    ostringstream out;
    out << "We could do some quest, for instance " << chat->formatQuest(quest);
    spam(out.str());
}
...
void SuggestWhatToDoAction::nothing() { ai->TellMasterNoFacing("I don't want to do anything", PLAYERBOT_SECURITY_ALLOW_ALL); }

void SuggestWhatToDoAction::relax() { ai->TellMasterNoFacing("It is so boring... We could relax a bit", PLAYERBOT_SECURITY_ALLOW_ALL); }
...
void SuggestWhatToDoAction::trade()
{
    ...
    ostringstream out;
    out << "Selling " << chat->formatItem(proto, count) << " for " << chat->formatMoney(price);
    spam(out.str());
}
