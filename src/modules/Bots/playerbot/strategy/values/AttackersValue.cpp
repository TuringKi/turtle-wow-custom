...
list<ObjectGuid> AttackersValue::Calculate()
{
    ...
    {
        result.push_back(bot->m_duel->opponent);
    }
...
void AttackersValue::AddAttackersOf(Player* player, set<Unit*>& targets)
{
    ...
    list<Unit*> units;
    MaNGOS::AnyUnfriendlyUnitInObjectRangeCheck u_check(player, ai->GetBot(), sPlayerbotAIConfig.sightDistance);
...
bool AttackersValue::hasRealThreat(Unit* attacker) { return attacker && attacker->IsInWorld() && attacker->IsAlive() && !attacker->IsPolymorphed() && !attacker->IsInRoots() && !attacker->IsFriendlyTo(bot) && (attacker->GetThreatManager().getCurrentVictim() || attacker->GetObjectGuid().IsPlayer()); }
