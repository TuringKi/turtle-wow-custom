...
SpellIdValue::SpellIdValue(PlayerbotAI* ai) : CalculatedValue<uint32>(ai, "spell id", 5) {}
...
uint32 SpellIdValue::Calculate()
{
    ...
    for (PlayerSpellMap::iterator itr = bot->GetSpellMap().begin(); itr != bot->GetSpellMap().end(); ++itr)
    {
        ...
        auto spellName = pSpellInfo->SpellName[loc];
        if (tolower(spellName[0]) != firstSymbol || strlen(spellName.c_str()) != spellLength || !Utf8FitTo(spellName, wnamepart))
        {
            continue;
        }
