    ...
    do
    {
        ...
        if (basket)
        {
            ...
            else if (action->isUseful())
            {
                ...
                {
                    if ((!skipPrerequisites || lastRelevance - relevance > 0.04) && MultiplyAndPush(actionNode->getPrerequisites(), relevance + 0.02, false, event))
                    {
...
bool Engine::HasStrategy(string name) { return strategies.find(name) != strategies.end(); }
