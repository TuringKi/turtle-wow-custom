...
namespace ai
{
    ...
    {
    public:
        FindUsableItemVisitor(Player* bot) : FindItemVisitor() { this->bot = bot; }
    ...
    {
    public:
        FindItemsToTradeByClassVisitor(uint32 itemClass, uint32 itemSubClass, int count) : IterateItemsVisitor(), count(count), itemClass(itemClass), itemSubClass(itemSubClass) {}
    ...
    {
    public:
        QueryNamedItemCountVisitor(string name) : QueryItemCountVisitor(0) { this->name = name; }

        virtual bool Visit(Item* item)
        {
            const ItemPrototype* proto = item->GetProto();
            if (proto && strstri(proto->Name1.c_str(), name.c_str()))
            {
                count += item->GetCount();
            }
    ...
    {
    public:
        FindUsableNamedItemVisitor(Player* bot, string name) : FindUsableItemVisitor(bot) { this->name = name; }

        virtual bool Accept(const ItemPrototype* proto) { return proto && strstri(proto->Name1.c_str(), name.c_str()); }
    ...
    {
    public:
        FindItemByIdVisitor(uint32 id) : FindItemVisitor() { this->id = id; }

        virtual bool Accept(const ItemPrototype* proto) { return proto->ItemId == id; }
