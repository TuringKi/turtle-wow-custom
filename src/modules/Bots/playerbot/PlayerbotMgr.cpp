 ...
 */
PlayerbotHolder::~PlayerbotHolder() { LogoutAllBots(); }
...
list<string> PlayerbotHolder::HandlePlayerbotCommand(char* args, Player* master)
{
    ...
    if (charnameStr == "!" && master && master->GetSession()->GetSecurity() > SEC_PLAYER)
    {
    ...
    for (set<string>::iterator i = bots.begin(); i != bots.end(); ++i)
    {
        ...
        {
            result = ProcessBotCommand(cmdStr, member, master->GetSession()->GetSecurity() >= SEC_MODERATOR, master->GetSession()->GetAccountId());
        }
        else if (!master)
        {
 ...
 * @param elapsed Time elapsed since the last update.
 */
void PlayerbotMgr::UpdateAIInternal(uint32 elapsed) { SetNextCheckDelay(sPlayerbotAIConfig.reactDelay); }
