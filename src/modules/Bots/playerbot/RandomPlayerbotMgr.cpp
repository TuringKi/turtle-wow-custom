...
void RandomPlayerbotMgr::UpdateAIInternal(uint32 elapsed)
{
    ...
    sLog.outString("%d bots processed. %d alliance and %d horde bots added. %d bots online. Next check in %d seconds", botProcessed, allianceNewBots, hordeNewBots, playerBots.size(), sPlayerbotAIConfig.randomBotUpdateInterval);
...
void RandomPlayerbotMgr::RandomTeleport(Player* bot, vector<WorldLocation>& locs)
{
    ...
    for (int attemtps = 0; attemtps < 10; ++attemtps)
    {
        ...
        float x = loc.x + urand(0, sPlayerbotAIConfig.grindDistance) - sPlayerbotAIConfig.grindDistance / 2;
        float y = loc.y + urand(0, sPlayerbotAIConfig.grindDistance) - sPlayerbotAIConfig.grindDistance / 2;
        float z = loc.z;
...
{
    vector<WorldLocation> locs;
    QueryResult* results = WorldDatabase.PQuery("SELECT `position_x`, `position_y`, `position_z` FROM `creature` WHERE `map` = '%u' AND ABS(`position_x` - '%f') < '%u' AND ABS(`position_y` - '%f') < '%u'", mapId, teleX, sPlayerbotAIConfig.randomBotTeleportDistance / 2, teleY, sPlayerbotAIConfig.randomBotTeleportDistance / 2);
    if (results)
    {
        do
        {
...
void RandomPlayerbotMgr::RandomizeFirst(Player* bot)
{
    ...
    for (int attempt = 0; attempt < 100; ++attempt)
    {
        ...
        uint32 level = GetZoneLevel(tele->mapId, tele->x, tele->y, tele->z);
        if (level > maxLevel + 5)
        {
            continue;
        }
        ...
        RandomTeleport(bot, tele->mapId, tele->z, tele->y, tele->z);
        break;
    }
}
...
bool RandomPlayerbotMgr::IsRandomBot(Player* bot) { return IsRandomBot(bot->GetObjectGuid()); }
...
uint32 RandomPlayerbotMgr::GetEventValue(uint32 bot, string event)
{
    ...
    QueryResult* results = CharacterDatabase.PQuery("SELECT `value`, `time`, `validIn` FROM `ai_playerbot_random_bots` WHERE `owner` = 0 AND `bot` = '%u' AND `event` = '%s'", bot, event.c_str());
...
uint32 RandomPlayerbotMgr::SetEventValue(uint32 bot, string event, uint32 value, uint32 validIn)
{
    ...
    if (value)
    {
        ...
        CharacterDatabase.PExecute("INSERT INTO `ai_playerbot_random_bots` (`owner`, `bot`, `time`, `validIn`, `event`, `value`) VALUES ('%u', '%u', '%u', '%u', '%s', '%u')", 0, bot, (uint32)time(0), validIn, event.c_str(), value);
    }
        ...
        for (list<uint32>::iterator i = sPlayerbotAIConfig.randomBotAccounts.begin(); i != sPlayerbotAIConfig.randomBotAccounts.end(); ++i)
        {
            ...
            if (QueryResult* results = CharacterDatabase.PQuery("SELECT `guid` FROM `characters` where `account` = '%u'", account))
            {
                do
                {
                    ...
                    uint32 randomTime = urand(sPlayerbotAIConfig.minRandomBotRandomizeTime, sPlayerbotAIConfig.maxRandomBotRandomizeTime);
                    CharacterDatabase.PExecute("UPDATE `ai_playerbot_random_bots` SET `validIn` = '%u' WHERE `event` = 'randomize' AND `bot` = '%u'", randomTime, bot->GetGUIDLow());
                    CharacterDatabase.PExecute("UPDATE `ai_playerbot_random_bots` SET `validIn` = '%u' WHERE `event` = 'logout' AND `bot` = '%u'", sPlayerbotAIConfig.maxRandomBotInWorldTime, bot->GetGUIDLow());
                }
                while (results->NextRow());
