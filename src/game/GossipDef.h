...
inline bool IsValidGossipOptionIconForBuild(uint8 icon) { return true; }
...
public:
    ...
    bool Empty() const { return m_gItems.empty(); }

    GossipMenuItem const& GetItem(unsigned int Id) { return m_gItems[Id]; }

    GossipMenuItemData const& GetItemData(unsigned int indexId) { return m_gItemsData[indexId]; }
...
public:
    ...
    uint8 MenuItemCount() const { return m_qItems.size(); }

    bool Empty() const { return m_qItems.empty(); }

    bool HasItem(uint32 questid);

    QuestMenuItem const& GetItem(uint16 Id) { return m_qItems[Id]; }
