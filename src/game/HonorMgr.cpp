...
void HonorMaintenancer::LoadStandingLists()
{
    ...
    sLog.outHonor("[MAINTENANCE] Alliance: %u, Horde: %u, Inactive: %u", m_allianceStandingList.size(), m_hordeStandingList.size(), m_inactiveStandingList.size());
}
...
void HonorMaintenancer::SetCityRanks()
{
    ...
    std::map<uint8, std::pair<uint32, uint32>> highestStandingInRace = {
        {RACE_HUMAN, {0, 0}}, {RACE_ORC, {0, 0}}, {RACE_DWARF, {0, 0}}, {RACE_NIGHTELF, {0, 0}}, {RACE_UNDEAD, {0, 0}}, {RACE_TAUREN, {0, 0}}, {RACE_GNOME, {0, 0}}, {RACE_TROLL, {0, 0}}, {RACE_GOBLIN, {0, 0}}, {RACE_HIGH_ELF, {0, 0}},
    };
...
void HonorMaintenancer::FlushRankPoints()
{
    ...
    for (auto& pair : m_weeklyScores)
    {
        ...
        CharacterDatabase.PExecute("UPDATE `characters` SET `honorHighestRank` = %u, `honorRankPoints` = %.1f, `honorStanding` = %u, "
                                   ...
                                   highestRank.rank, finiteAlways(weeklyScore.newRp), weeklyScore.standing, weeklyScore.hk, weeklyScore.hk, weeklyScore.dk, finiteAlways(weeklyScore.cp), pair.first);
    }
...
void HonorMaintenancer::CreateCalculationReport()
{
    ...
    if (!m_allianceStandingList.empty())
    {
        ...
        for (auto& st : m_allianceStandingList)
        {
            ...
            ofs << "Guid: " << st.guid << ", HK: " << ws.hk << ", DK: " << ws.dk << ", CP: " << ws.cp << ", oldRp: " << ws.oldRp << ", earning: " << ws.earning << ", newRp: " << ws.newRp << ", capRp: " << MaximumRpAtLevel(ws.level) << ", standing: " << ws.standing << std::endl << std::flush;
        }
    }
    ...
    if (!m_inactiveStandingList.empty())
    {
        ...
        for (auto& st : m_inactiveStandingList)
        {
            ...
            ofs << "Guid: " << st.guid << ", HK: " << ws.hk << ", DK: " << ws.dk << ", CP: " << ws.cp << ", oldRp: " << ws.oldRp << ", newRp: " << ws.newRp << ", capRp: " << MaximumRpAtLevel(ws.level) << ", standing: " << ws.standing << std::endl << std::flush;
        }
    }
...
float HonorMaintenancer::CalculateRpDecay(float rpEarning, const WeeklyScore& wk)
{
    ...
    static const uint32 RankMinRP[] = {2000, 5000, 10000, 15000, 20000, 25000, 30000, 35000, 40000, 45000, 50000, 55000, 60000};
...
void HonorMgr::ClearHonorCP() { m_honorCP.clear(); }
    ...
    for (auto& honorCP : m_honorCP)
    {
        ...
        case STATE_NEW:
            CharacterDatabase.PExecute("INSERT INTO `character_honor_cp` (`guid`, `victimType`, `victim`, `cp`, `date`, `type`) "
                                       " VALUES (%u, %u, %u, %.1f, %u, %u)",
                                       m_owner->GetGUIDLow(), honorCP.victimType, honorCP.victimId, finiteAlways(honorCP.cp), honorCP.date, honorCP.type);
...
void HonorMgr::SaveStoredData()
{
    ...
    CharacterDatabase.PExecute("UPDATE `characters` SET `honorRankPoints` = %.1f, `honorStanding` = %u, `honorHighestRank` = %u, "
                               ...
                               finiteAlways(m_rankPoints), m_standing, m_highestRank.rank, m_lastWeekHK, finiteAlways(m_lastWeekCP), m_storedHK, m_storedDK, m_owner->GetGUIDLow());
}
...
bool HonorMgr::Add(float cp, uint8 type, Unit* source)
{
    ...
    if (m_owner->GetMap()->IsBattleGround())
        sLog.outHonor("[BATTLEGROUND]: Player %s (account: %u) got %f honor for type %u, source %s %s (IP: %s)", m_owner->GetSession()->GetPlayerName(), m_owner->GetSession()->GetAccountId(), honor, type, plr ? "player" : "unit", source->GetName(), ip.c_str());
    else
        sLog.outHonor("[OPEN WORLD]: Player %s (account: %u) got %f honor for type %u, source %s %s (IP: %s)", m_owner->GetSession()->GetPlayerName(), m_owner->GetSession()->GetAccountId(), honor, type, plr ? "player" : "unit", source->GetName(), ip.c_str());
