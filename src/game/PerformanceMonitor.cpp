...
PerformanceMonitor::PerformanceMonitor() { gPerfMonitorInterface = this; }
...
void PerformanceMonitor::SetReportInterval(uint32 IntervalInSeconds) { IntervalReport.SetInterval(IntervalInSeconds * 1000); }
...
void PerformanceMonitor::ReportCPU(ChatHandler& Handler)
{
    ...
    auto GetPercentOfLambda = [](XStatTimer Local, XStatTimer Global) -> float { return 100.0f * float(Local.result) / float(Global.result); };
    ...
    auto ReportStatWithParentLambda = [&Handler, &GetPercentOfLambda](const char* Name, const XStatTimer& Timer, const XStatTimer& Parent, int32 Level = 1)
    {
        ...
        Handler.PSendSysMessage(" %s %s: [%u] %2.2fms, %2.1f%%, min: %2.2fms, max: %2.2fms", BeginShit, Name, Timer.count, Timer.result, GetPercentOfLambda(Timer, Parent), Timer.MinResult, Timer.MaxResult);
    };
