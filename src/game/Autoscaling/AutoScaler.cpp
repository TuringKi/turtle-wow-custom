...
{
public:
    explicit Read_Mutex_Guard(std::shared_mutex& mut) : mut(mut)
    { // construct and lock
        mut.lock_shared();
    }
...
static uint32 disabledMapIds[] = {807};
...
void AutoScaler::ScaleCreature(Creature* creature, uint32 playerCount, uint32 maxCount, Map* map)
{
    ...
    auto ScaleHp = [hpPercentage](float value) { return value / 100 * hpPercentage; };

    const float dmgPercentage = (static_cast<float>(playerCount) / static_cast<float>(maxCount) * 100.f) + (maxCount - playerCount);
    auto ScaleDamage = [dmgPercentage](float value) { return value / 100 * dmgPercentage; };
    ...
    {
        // store base vals.
        auto tup = std::make_tuple(std::make_pair(creature->GetWeaponDamageRange(BASE_ATTACK, MINDAMAGE), creature->GetWeaponDamageRange(BASE_ATTACK, MAXDAMAGE)), std::make_pair(creature->GetFloatValue(UNIT_FIELD_MINRANGEDDAMAGE), creature->GetFloatValue(UNIT_FIELD_MAXRANGEDDAMAGE)), creature->GetInt32Value(UNIT_FIELD_ATTACK_POWER));
...
void AutoScaler::GenerateScaledMoneyLoot(Creature* creature, Loot* loot)
{
    ...
    loot->GenerateMoneyLoot(static_cast<uint32>(creature->GetGoldMin() * gold_factor), static_cast<uint32>(creature->GetGoldMax() * gold_factor));
}
