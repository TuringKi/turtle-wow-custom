...
TransmogMgr::TransmogMgr(Player* owner) : _owner(owner) { prefix = "TW_TRANSMOG"; }
...
void TransmogMgr::HandleAddonMessages(std::string msg)
{
    ...
    if (strstr(msg.c_str(), "ResetTransmog:"))
    {
        ...
        result = "ResetResult:" + std::to_string(slot) + ":" + std::to_string(slotId) + ":" + std::to_string(ApplyTransmog(slot, 0, 0));
...
void TransmogMgr::ApplyTransmog(std::string msg)
{
    ...
    result = "TransmogResult:" + std::to_string(slot) + ":" + std::to_string(slotId) + ":" + std::to_string(ApplyTransmog(slot, itemID, slotId));
...
uint8 TransmogMgr::ApplyTransmog(uint8 slot, uint32 sourceItemID, uint32 slotId)
{
    ...
    if (sourceItemID)
    {
        ...
        if (STUPID_RESTRICTIONS)
        {
            ...
            // cloth = leather * cloth
            if (srcItemProto->Class == ITEM_CLASS_ARMOR && destItemProto->Class == ITEM_CLASS_ARMOR && srcItemProto->InventoryType == destItemProto->InventoryType)
            {
    ...
    for (auto& item : _transmogs)
    {
        if (ItemPrototype const* proto = sObjectMgr.GetItemPrototype(item))
        {
            ...
            if (STUPID_RESTRICTIONS)
            {
                ...
                // leather = mail & leather & cloth
                // cloth = leather * cloth
                if (proto->Class == ITEM_CLASS_ARMOR && destItemProto->Class == ITEM_CLASS_ARMOR && proto->InventoryType == destItemProto->InventoryType)
                {
