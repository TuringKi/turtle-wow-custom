...
public:
    ...
    bool IsNextMeleeSwingSpell() const { return m_spellInfo->Attributes & (SPELL_ATTR_ON_NEXT_SWING_1 | SPELL_ATTR_ON_NEXT_SWING_2); }
    bool IsRangedSpell() const { return m_spellInfo->Attributes & SPELL_ATTR_RANGED; }
...
protected:
    ...
    void AddExecuteLogInfo(SpellEffectIndex i, ExecuteLogInfo info) { m_executeLogInfo[i].push_back(info); }
};
...
namespace MaNGOS
{
    struct SpellNotifierPlayer
    {
        ...
        SpellNotifierPlayer(Spell& spell, Spell::UnitList& data, const uint32& i, float radius) : i_data(data), i_spell(spell), i_index(i), i_radius(radius) { i_originalCaster = i_spell.GetAffectiveCasterObject(); }
