...
public:
    ...
    void BroadcastEvent(GuildEvents event, char const* str1 = nullptr, char const* str2 = nullptr, char const* str3 = nullptr) { BroadcastEvent(event, ObjectGuid(), str1, str2, str3); }
    ...
    bool HasRankRight(uint32 rankId, uint32 right) { return ((GetRankRights(rankId) & right) != GR_RIGHT_EMPTY) ? true : false; }
    ...
    void SendRoster(WorldSession* session = nullptr) { Roster(session); }
    ...
    void AddToCache(uint32 guidLow) { m_onlineMemberCache.insert(guidLow); }

    void RemoveFromCache(uint32 guidLow) { m_onlineMemberCache.erase(guidLow); }

    bool IsMemberCacheEnabled() const { return members.size() >= OnlineMemberCache; }

    bool IsRosterCacheEnabled() const { return members.size() >= RosterCache; }
