...
namespace Anticheat
{
    bool SessionAnticheat::ReadAddonInfo(WorldPacket* authSession, WorldPacket& out)
    {
        ...
        if (!!_fingerprint)
        {
            sLog.out(LOG_ANTICHEAT_BASIC, "ADDON: Received addon information when fingerprint is already known (0x%lx) account %u ip %s.  This may be an attempt to crash the server", _session->GetAccountId(), _session->GetRemoteAddress().c_str());
        ...
        {
            sLog.out(LOG_ANTICHEAT_BASIC, "ADDON: Addon info too big, size %u.  Account %s (id %u) IP %s", addonSize, _session->GetUsername().c_str(), _session->GetAccountId(), _session->GetRemoteAddress().c_str());
        ...
        if (uncompress(const_cast<Bytef*>(clientAddonData.contents()), &addonSize, reinterpret_cast<const Bytef*>(authSession->contents() + authSession->rpos()), static_cast<uLongf>(authSession->size() - authSession->rpos())) != Z_OK)
        {
            sLog.out(LOG_ANTICHEAT_BASIC, "ADDON: Addon information failed to compress.  Account %s (id %u) IP %s", _session->GetUsername().c_str(), _session->GetAccountId(), _session->GetRemoteAddress().c_str());
        ...
        while (clientAddonData.rpos() < clientAddonData.size())
        {
            ...
            sLog.out(LOG_ANTICHEAT_DEBUG, "ADDON: %s flags 0x%02x modulus crc 0x%08x url crc 0x%08x", info.name.c_str(), info.flags, info.moduluscrc, info.urlcrc);
        ...
        if (fingerprintFound && FingerprintValid(fingerprint))
            sLog.out(LOG_ANTICHEAT_DEBUG, "ADDON: Found fingerprint: 0x%08x.  Account %s (id %u) IP %s", fingerprint, _session->GetUsername().c_str(), _session->GetAccountId(), _session->GetRemoteAddress().c_str());
        else
        {
            // if it was found, but we reach here, it must not have been valid?
            if (fingerprintFound)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "ADDON: Found invalid fingerprint 0x%08x.  Account %s (id %u) IP %s.  Generating new...", fingerprint, _session->GetUsername().c_str(), _session->GetAccountId(), _session->GetRemoteAddress().c_str());

                fingerprintFound = false;
            }

            fingerprint = GenerateFingerprint();
            sLog.out(LOG_ANTICHEAT_BASIC, "ADDON: Generated new fingerprint: 0x%08x.  Account %s (id %u) IP %s local IP %s", fingerprint, _session->GetUsername().c_str(), _session->GetAccountId(), _session->GetRemoteAddress().c_str(), _session->GetRemoteAddress().c_str());
        }
