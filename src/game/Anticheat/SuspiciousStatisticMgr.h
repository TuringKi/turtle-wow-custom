...
struct FishingTracker
{
    ...
    FishingTracker() : StartTimestamp(0), LastAttemptTimestamp(0) {}

    FishingTracker(uint32 InStartTimestamp, uint32 InLastAttemptTimestamp) : StartTimestamp(InStartTimestamp), LastAttemptTimestamp(InLastAttemptTimestamp) {}

    FishingTracker(const FishingTracker& other) : StartTimestamp(other.StartTimestamp), LastAttemptTimestamp(other.LastAttemptTimestamp) {}
};
...
public:
    ...
    void SetMovementEnable(bool bInEnable) { bMovementEnable = bInEnable; }

    const bool IsMovementEnabled() const { return bMovementEnable; }

    void SetFishingEnable(bool bInEnable) { bFishingEnable = bInEnable; }

    const bool IsFishingEnabled() const { return bFishingEnable; }

    void SetNpcKilledEnable(bool bInEnable) { bNpcKilledEnable = bInEnable; }

    const bool IsNpcKilledEnabled() const { return bNpcKilledEnable; }

    void SetMovementDetectValue(float InDetectValue) { MovementDetectValue = InDetectValue; }

    const float GetMovementDetectValue() const { return MovementDetectValue; }
