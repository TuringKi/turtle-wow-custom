...
namespace
{
    static constexpr char* sCheatTypeNames[] = {"WallClimb", "WaterWalk", "SlowFall", "FlyHack", "Forbidden", "MultiJump", "TimeBack", "FastJump", "JumpSpeedChange", "NullClientTime", "RootMove", "Explore", "ExploreHigh", "OverspeedZ", "HeartbeatSkip", "ClockDesync", "FakeTransport", "TeleToTransport", "Teleport", "TeleportFar", "FixedZ", "BadOrderAck", "Warden"};

    static_assert(sizeof(sCheatTypeNames) / sizeof(sCheatTypeNames[0]) == Anticheat::CheatType::CHEATS_COUNT, "sCheatTypeNames has the wrong number of strings");
} // namespace
...
namespace Anticheat
{
    ...
    void AnticheatConfig::setConfig(AnticheatConfigUInt32Values index, char const* fieldname, uint32 defvalue) { setConfig(index, GetIntDefault(fieldname, defvalue)); }

    void AnticheatConfig::setConfig(AnticheatConfigBoolValues index, char const* fieldname, bool defvalue) { setConfig(index, GetBoolDefault(fieldname, defvalue)); }

    void AnticheatConfig::setConfig(AnticheatConfigFloatValues index, char const* fieldname, float defvalue) { setConfig(index, GetFloatDefault(fieldname, defvalue)); }
    ...
    void AnticheatConfig::loadConfigSettings()
    {
        ...
        for (auto i = 0; i < CHEATS_COUNT; ++i)
        {
            ...
            _responseRules[i] = {static_cast<uint32>(GetIntDefault(tickCount.str().c_str(), 0)), static_cast<uint32>(GetIntDefault(tickAction.str().c_str(), 0)), static_cast<uint32>(GetIntDefault(totalCount.str().c_str(), 0)), static_cast<uint32>(GetIntDefault(totalAction.str().c_str(), 0))};
        }
