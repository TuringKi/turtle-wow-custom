...
WindowsModuleScan::WindowsModuleScan(const std::string& module, bool wanted, const std::string& comment, uint32 flags) :
    _module(module), _wanted(wanted), WindowsScan(
                                          // builder
...
WindowsMemoryScan::WindowsMemoryScan(uint32 offset, const void* expected, size_t length, const std::string& comment, uint32 flags) :
    _expected(length), _offset(offset), WindowsScan(
                                            // builder
                                            [this](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
                                            {
                                                auto const winWarden = reinterpret_cast<const WardenWin*>(warden);

                                                scan << static_cast<uint8>(winWarden->GetModule()->opcodes[READ_MEMORY] ^ winWarden->GetXor()) << static_cast<uint8>(0) // no string associated with this form of the constructor
                                                     << this->_offset << static_cast<uint8>(this->_expected.size());
                                            },
                                            // checker
                                            [this](const Warden*, ByteBuffer& buff)
                                            {
...
WindowsMemoryScan::WindowsMemoryScan(const std::string& module, uint32 offset, const void* expected, size_t length, const std::string& comment, uint32 flags) :
    _expected(length), _offset(offset), _module(module), WindowsScan(
                                                             // builder
                                                             [this](const Warden* warden, std::vector<std::string>& strings, ByteBuffer& scan)
                                                             {
                                                                 ...
                                                                 scan << static_cast<uint8>(winWarden->GetModule()->opcodes[READ_MEMORY] ^ winWarden->GetXor()) << static_cast<uint8>(strings.size()) << this->_offset << static_cast<uint8>(this->_expected.size());
                                                             },
                                                             // checker
                                                             [this](const Warden*, ByteBuffer& buff)
                                                             {
...
WindowsCodeScan::WindowsCodeScan(uint32 offset, const std::vector<uint8>& pattern, bool memImageOnly, bool wanted, const std::string& comment, uint32 flags) :
    _offset(offset), _pattern(pattern), _memImageOnly(memImageOnly), _wanted(wanted), WindowsScan(
                                                                                          // builder
                                                                                          [this](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
                                                                                          {
                                                                                              auto const winWarden = reinterpret_cast<const WardenWin*>(warden);
                                                                                              ...
                                                                                              scan << static_cast<uint8>(winWarden->GetModule()->opcodes[this->_memImageOnly ? FIND_MEM_IMAGE_CODE_BY_HASH : FIND_CODE_BY_HASH] ^ winWarden->GetXor()) << seed;
...
WindowsFileHashScan::WindowsFileHashScan(const std::string& file, const void* expected, bool wanted, const std::string& comment, uint32 flags) :
    _file(file), _wanted(wanted), _hashMatch(!!expected), WindowsScan(
                                                              // builder
                                                              [this](const Warden* warden, std::vector<std::string>& strings, ByteBuffer& scan)
                                                              {
                                                                  MANGOS_ASSERT(strings.size() < 0xFF);
                                                                  ...
                                                                  scan << static_cast<uint8>(winWarden->GetModule()->opcodes[HASH_CLIENT_FILE] ^ winWarden->GetXor()) << static_cast<uint8>(strings.size());
                                                              },
                                                              // checker
                                                              [this](const Warden*, ByteBuffer& buff)
                                                              {
...
WindowsLuaScan::WindowsLuaScan(const std::string& lua, bool wanted, const std::string& comment, uint32 flags) :
    _lua(lua), _wanted(wanted), WindowsScan(
                                    // builder
                                    [this](const Warden* warden, std::vector<std::string>& strings, ByteBuffer& scan)
                                    {
                                        MANGOS_ASSERT(strings.size() < 0xFF);
                                        ...
                                        scan << static_cast<uint8>(winWarden->GetModule()->opcodes[GET_LUA_VARIABLE] ^ winWarden->GetXor()) << static_cast<uint8>(strings.size());
                                    },
                                    // checker
                                    [this](const Warden*, ByteBuffer& buff)
                                    {
...
WindowsLuaScan::WindowsLuaScan(const std::string& lua, const std::string& expectedValue, const std::string& comment, uint32 flags) :
    _lua(lua), _expectedValue(expectedValue), WindowsScan(
                                                  // builder
                                                  [this](const Warden* warden, std::vector<std::string>& strings, ByteBuffer& scan)
                                                  {
                                                      MANGOS_ASSERT(strings.size() < 0xFF);
...
WindowsHookScan::WindowsHookScan(const std::string& module, const std::string& proc, const void* hash, uint32 offset, size_t length, const std::string& comment, uint32 flags) :
    _module(module), _proc(proc), _offset(offset), _length(length), WindowsScan(
                                                                        // builder
                                                                        [this](const Warden* warden, std::vector<std::string>& strings, ByteBuffer& scan)
                                                                        {
                                                                            ...
                                                                            scan << static_cast<uint8>(strings.size() - 1) << static_cast<uint8>(strings.size()) << this->_offset << static_cast<uint8>(this->_length);
                                                                        },
                                                                        // checker
                                                                        [](const Warden*, ByteBuffer& buff) { return buff.read<uint8>() == Detoured; }, sizeof(uint8) + sizeof(uint32) + SHA_DIGEST_LENGTH + sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8), comment, flags)
{
...
WindowsDriverScan::WindowsDriverScan(const std::string& name, const std::string& targetPath, bool wanted, const std::string& comment, uint32 flags) :
    _name(name), _wanted(wanted), _targetPath(targetPath), WindowsScan(
                                                               // builder
                                                               [this](const Warden* warden, std::vector<std::string>& strings, ByteBuffer& scan)
                                                               {
                                                                   MANGOS_ASSERT(strings.size() < 0xFF);
