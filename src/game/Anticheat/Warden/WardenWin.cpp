...
struct AIT_SAMPLING_PART<Version, typename std::enable_if_t<((Version) >= OsVersion::WindowsVista)>>
{
...
template <OsVersion Version>
struct BUILDNUMBER_PART<Version, typename std::enable_if_t<((Version) == OsVersion::WindowsXP)>>
{
    ULONG Reserved2[8];
};

template <OsVersion Version>
struct BUILDNUMBER_PART<Version, typename std::enable_if_t<((Version) == OsVersion::WindowsVista || Version == OsVersion::Windows7)>>
{
    ULONG Reserved2[7];
};

template <OsVersion Version>
struct BUILDNUMBER_PART<Version, typename std::enable_if_t<((Version) == OsVersion::Windows8)>>
{
    ULONG Reserved2;
};

template <OsVersion Version>
struct BUILDNUMBER_PART<Version, typename std::enable_if_t<((Version) == OsVersion::Windows10AndUp)>>
{
    ULONG NtBuildNumber;
};
...
template <OsVersion Version>
struct TSC_PART<Version, typename std::enable_if_t<(Version == OsVersion::Windows7)>>
{
    union
    {
        UCHAR TscQpcData;
...
template <OsVersion Version>
struct TSC_PART<Version, typename std::enable_if_t<((Version) == OsVersion::Windows10AndUp)>>
{
...
template <OsVersion Version>
struct TSC_PART_2<Version, typename std::enable_if_t<(Version == OsVersion::Windows7 || Version == OsVersion::Windows10AndUp)>>
{
    UCHAR TscQpcPad[2];
};

template <OsVersion Version>
struct TSC_PART_2<Version, typename std::enable_if_t<((Version) == OsVersion::Windows8)>>
{
    UCHAR Reserved12[3];
};
...
template <OsVersion Version>
struct TRACELOG_PART<Version, typename std::enable_if_t<(Version == OsVersion::WindowsXP)>>
{
    ULONG TraceLogging;
};

template <OsVersion Version>
struct TRACELOG_PART<Version, typename std::enable_if_t<((Version) >= OsVersion::WindowsVista)>>
{
...
template <OsVersion Version>
struct PAD_PART_1<Version, typename std::enable_if_t<(Version >= OsVersion::Windows7)>>
{
...
template <OsVersion Version>
struct QPC_SYSTEM_TIME<Version, typename std::enable_if_t<(Version == OsVersion::Windows8)>>
{
...
#define LSTRIP(c, l)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
    ...
    while (false)
        ...
        do
        {
            ...
            else if (code[0] == 0x70 || code[0] == 0x71 || code[0] == 0x74 || code[0] == 0x75 || code[0] == 0x78 || code[0] == 0x7A || code[0] == 0x7B || code[0] == 0x7C || code[0] == 0x7D || code[0] == 0x7E || code[0] == 0x7F)
            {
...
std::shared_ptr<WindowsScan> WardenWin::MakeDynamicDataScan(WardenWin* warden, uint32& offset, uint32& sizeLeft, turtle_vector<uint8, Category_Anticheat>& output)
{
    Scan::CheckT callback = [&offset, &sizeLeft, &output](const Warden* warden, ByteBuffer& buff)
    {
        ...
        if (!!result)
        {
            sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read dynamic data from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
...
void WardenWin::LoadScriptedScans()
{
    ...
    auto const wardenSysInfo2 = std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor()) << static_cast<uint8>(0) << wardenWin->_sysInfo.dwOemId + sOfsWardenWinSysInfo << static_cast<uint8>(sizeof(wardenWin->_sysInfo));
        },
        // checker
        [](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read SYSTEM_INFO from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
            ...
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Incorrect architecture reported (%u) for account %u ip %s", wardenWin->_sysInfo.wProcessorArchitecture, wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());

                return true;
            }

            // for classic, tbc, and wotlk, the cpu type should never be anything other than i386, i486, or pentium (i586)
            if (wardenWin->_sysInfo.dwProcessorType != 386 && wardenWin->_sysInfo.dwProcessorType != 486 && wardenWin->_sysInfo.dwProcessorType != 586)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Incorrect processor type: %u for account %u ip %s", wardenWin->_sysInfo.dwProcessorType, wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + sizeof(WIN_SYSTEM_INFO), "Sysinfo locate", None);
    ...
    auto const wardenSysInfo1 = std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor()) << static_cast<uint8>(0) << wardenWin->_wardenAddress + sOfsWardenSysInfo << static_cast<uint8>(sizeof(wardenWin->_sysInfo.dwOemId));
        },
        // checker
        [wardenSysInfo2](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read warden->SysInfo from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + sizeof(uint32), "Intermediate sysinfo locate", None);
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor()) << static_cast<uint8>(0) << sOfsWardenModule << static_cast<uint8>(sizeof(wardenWin->_wardenAddress));
        },
        // checker
        [wardenSysInfo1](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read s_moduleInterface from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + sizeof(uint32), "Warden locate", InitialLogin | WinAllBuild));

    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsMemoryScan>(
        sOfsCWorld__enables, sizeof(uint32),
        [](const Warden* warden, ByteBuffer& buff)
        {
            auto const wardenWin = const_cast<WardenWin*>(reinterpret_cast<const WardenWin*>(warden));
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read CWorld::enables from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
            ...
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: CWorld::enables expected 0x%lx prohibited 0x%lx received 0x%lx", (uint32)Required, (uint32)Prohibited, val);
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor()) << static_cast<uint8>(0) << sOfsCSimpleTop__m_eventTime << static_cast<uint8>(sizeof(uint32));
        ...
        [](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!buff.read<uint8>())
            {
                sLog.out(LOG_ANTICHEAT_DEBUG, "WARDEN: Account %u timing check failed to read CSimpleTop::m_eventTime", session->GetLatency());
                return true;
            }
            ...
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Account: %u Current time: %u Last hardware action: %u (last hardware action in the future)", session->GetAccountId(), currentTime, lastHardwareAction);
                return true;
            }
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsScan>(
        ...
        (sizeof(uint8) + sizeof(uint32) + SHA_DIGEST_LENGTH + sizeof(uint8)) * HypervisorCount + 21, sizeof(uint8) * HypervisorCount, "Hypervisor check", WinAllBuild | InitialLogin));
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>& strings, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(warden->GetModule()->opcodes[FIND_CODE_BY_HASH] ^ warden->GetXor()) << seed;

            static constexpr std::uint8_t pattern[] = {0x56, 0x57, 0xFC, 0x8B, 0x54, 0x24, 0x14, 0x8B, 0x74, 0x24, 0x10, 0x8B, 0x44, 0x24, 0x0C, 0x8B, 0xCA, 0x8B, 0xF8, 0xC1, 0xE9, 0x02, 0x74, 0x02, 0xF3, 0xA5, 0xB1, 0x03, 0x23, 0xCA, 0x74, 0x02, 0xF3, 0xA4, 0x5F, 0x5E, 0xC3};
        ...
        sizeof(uint8) + sizeof(uint32) + SHA_DIGEST_LENGTH + sizeof(uint32) + sizeof(uint8), sizeof(uint8), "Warden Memory Read check", WinAllBuild | AdvancedScan));
    ...
    auto const endSceneHookCheck1 = std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor()) << static_cast<uint8>(0) << wardenWin->_endSceneAddress << endSceneReadSize;
        },
        // checker
        [](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read EndScene (hook check stage 1) from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + endSceneReadSize, "EndScene hook check stage 1", WinAllBuild | AdvancedScan);
    ...
    auto const endSceneLocate4 = std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor()) << static_cast<uint8>(0) << wardenWin->_endSceneAddress + sOfsDevice4 << static_cast<uint8>(sizeof(uint32));
        },
        // checker
        [endSceneHookCheck1](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read EndScene (stage 4) from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + sizeof(uint32), "EndScene locate stage 4", None);
    ...
    auto const endSceneLocate3 = std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor()) << static_cast<uint8>(0) << wardenWin->_endSceneAddress + sOfsDevice3 << static_cast<uint8>(sizeof(uint32));
        },
        // checker
        [endSceneLocate4](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read EndScene (stage 3) from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + sizeof(uint32), "EndScene locate stage 3", None);
    ...
    auto const endSceneLocate2 = std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor()) << static_cast<uint8>(0) << wardenWin->_endSceneAddress + sOfsDevice2 << static_cast<uint8>(sizeof(uint32));
        },
        // checker
        [endSceneLocate3](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read EndScene (stage 2) from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + sizeof(uint32), "EndScene locate stage 2", None);

    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsMemoryScan>(
        sOfsg_theGxDevicePtr, sizeof(uint32),
        // checker
        [endSceneLocate2](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read g_theGxDevicePtr from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
            ...
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: g_theGxDevicePtr is nullptr for account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
                return true;
            }
                                                                      ...
                                                                      Scan::CheckT(
                                                                          [](const Warden* warden, ByteBuffer& buff)
                                                                          {
                                                                              if (buff.read<uint8>() == WindowsModuleScan::ModuleFound)
                                                                              {
                                                                                  ...
                                                                                  sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Proxifier found on account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
                                                                              }
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsMemoryScan>(
        sOfsSharedData + sOfsSharedDataMajorVersion, sOfsSharedDataMinorVersion - sOfsSharedDataMajorVersion + sizeof(ULONG),
        [](const Warden* warden, ByteBuffer& buff)
        {
            auto const wardenWin = const_cast<WardenWin*>(reinterpret_cast<const WardenWin*>(warden));
            auto const result = buff.read<uint8>();
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read preBuiltData1Offset from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsScan>(
        ...
        [](const Warden* warden, std::vector<std::string>&, ByteBuffer& scan)
        {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor()) << static_cast<uint8>(0) << sOfsClickToMovePosition << static_cast<uint8>(sizeof(float) * 3);
        },
        // checker
        [](const Warden* warden, ByteBuffer& buff)
        {
            ...
            if (!!result)
            {
                sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read click to move position from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + sizeof(float) + sizeof(float) + sizeof(float), "Click To Move Position", WinAllBuild));
}
...
void WardenWin::BuildLuaInit(const std::string& module, bool fastcall, uint32 offset, ByteBuffer& out) const
{
    ...
    out << static_cast<uint8>(WARDEN_SMSG_MODULE_INITIALIZE) << static_cast<uint16>(len) << static_cast<uint32>(0) // checksum placeholder
        << static_cast<uint8>(4) << static_cast<uint8>(0) << static_cast<uint8>(module.length());

    // avoid writing null terminator
    out.append(module.c_str(), module.length());

    out << offset << static_cast<uint8>(fastcall ? 0 : 1);
...
void WardenWin::BuildFileHashInit(const std::string& module, bool asyncparam, uint32 openOffset, uint32 sizeOffset, uint32 readOffset, uint32 closeOffset, ByteBuffer& out) const
{
    ...
    out << static_cast<uint8>(WARDEN_SMSG_MODULE_INITIALIZE) << static_cast<uint16>(len) << static_cast<uint32>(0) // checksum placeholder
        << static_cast<uint8>(1) << static_cast<uint8>(0) << static_cast<uint8>(asyncparam ? 2 : 1) << static_cast<uint8>(module.length());

    // avoid writing null terminator
    out.append(module.c_str(), module.length());

    out << openOffset << sizeOffset << readOffset << closeOffset;
...
void WardenWin::BuildTimingInit(const std::string& module, uint32 offset, bool set, ByteBuffer& out) const
{
    ...
    out << static_cast<uint8>(WARDEN_SMSG_MODULE_INITIALIZE) << static_cast<uint16>(len) << static_cast<uint32>(0) // checksum placeholder
        << static_cast<uint8>(1) << static_cast<uint8>(1) << static_cast<uint8>(module.length());
...
WardenWin::WardenWin(WorldSession* session, const BigNumber& K, SessionAnticheatInterface* anticheat) : _wardenAddress(0), Warden(session, sWardenModuleMgr.GetWindowsModule(), K, anticheat), _lastClientTime(0), _lastHardwareActionTime(0), _lastTimeCheckServer(0), _sysInfoSaved(false), _proxifierFound(false), _hypervisors(""), _endSceneFound(false), _endSceneAddress(0)
{
...
void WardenWin::ValidateEndScene(const std::vector<uint8>& code)
{
    ...
    if (*p == 0xCC)
    {
        sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Detected INT3 EndScene hook for account %u IP %s (NOP count = %d)", _session->GetAccountId(), _session->GetRemoteAddress().c_str(), nopCount);
    }
    // JMP hook
    else if (*p == 0xE9)
    {
        ...
        sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Detected JMP EndScene hook for account %u IP %s (NOP count = %d)", _session->GetAccountId(), _session->GetRemoteAddress().c_str(), nopCount);

        // request a custom scan just to check the JMP destination
        EnqueueScans({std::make_shared<WindowsMemoryScan>(
            absoluteDest, codeRequestLength,
            // checker
            [](const Warden* warden, ByteBuffer& buff)
            {
                auto const wardenWin = const_cast<WardenWin*>(reinterpret_cast<const WardenWin*>(warden));
                ...
                if (!!result)
                {
                    sLog.out(LOG_ANTICHEAT_BASIC, "WARDEN: Failed to read EndScene hook code from account %u ip %s", wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
                ...
                if (ValidateEndSceneHook(code) && !!sAnticheatConfig.GetWardenSuspiciousEndSceneHookAction())
                    wardenWin->_anticheat->RecordCheat(sAnticheatConfig.GetWardenSuspiciousEndSceneHookAction(), "Warden", "Suspicious client heuristic results.  Probable bot.");
...
{
    uint32 result = _session->GetAccountMaxLevel() >= sAnticheatConfig.GetWardenMinimumAdvancedLevel() ? ScanFlags::AdvancedScan : ScanFlags::None;
...
void WardenWin::SendPlayerInfo(ChatHandler* handler, bool includeFingerprint) const
{
    ...
    {
        handler->PSendSysMessage("Last hardware action: %u client time: %u idle time: %u seconds info age: %u seconds", _lastHardwareActionTime, _lastClientTime, (_lastClientTime - _lastHardwareActionTime) / 1000, WorldTimer::getMSTimeDiffToNow(_lastTimeCheckServer) / 1000);
    }
    ...
    if (includeFingerprint && !!_sysInfo.lpMaximumApplicationAddress)
    {
        ...
        str << "Architecture: " << ArchitectureString(_sysInfo.wProcessorArchitecture) << " CPU Type: " << CPUTypeAndRevision(_sysInfo.dwProcessorType, _sysInfo.wProcessorRevision) << " Page Size: 0x" << std::hex << std::uppercase << _sysInfo.dwPageSize << std::dec;
