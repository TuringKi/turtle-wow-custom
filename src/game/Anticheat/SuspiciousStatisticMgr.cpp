...
std::string FormatPlayerNameToLink(const char* InPlayerName) { return string_format("|cffffc0c0|Hplayer:{}|h[{}]|h|r", InPlayerName, InPlayerName); }
...
void SuspiciousStatisticMgr::PrintAllActiveFishers(Player* receiver)
{
    ...
    for (auto iter = FishingTimeCounterMap.begin(); iter != FishingTimeCounterMap.end(); ++iter)
    {
        if (Player* fisher = sObjectMgr.GetPlayer(ObjectGuid(iter->first)))
        {
            ...
            std::stringstream ss;
            ss << "Player:" << PlayerLink << " Start fishing " << flDeltaSinceStart << " Last attempt " << flDeltaSinceLastFishAttempt;
...
void SuspiciousStatisticMgr::RecordSuspiciousActivity(SuspiciousType::Value type, Player* player, uint32 Param1, uint32 Param2, const char* OptDescMessage /*= nullptr*/)
{
    ...
    message << "Player:" << PlayerLink << " IP: " << player->GetSession()->GetRemoteAddress() << " Account ID: " << player->GetSession()->GetAccountId() << " Action: " << SuspiciousType::ToString(type) << " Param 1: " << Param1 << " Param 2: " << Param2;
    ...
    sDiscordBot->SendMessageToChannel(1083954369503973416, Message, DiscordBot::MessagePriority::Requeue);
#endif
}
    ...
    if (Group* group = player->GetGroup())
    {
        ...
        for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            if (Player* groupMember = itr->getSource())
            {
                ...
                if (groupMember->FindMap() != player->FindMap())
                    continue;
