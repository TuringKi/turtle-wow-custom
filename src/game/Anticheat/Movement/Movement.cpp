...
namespace Anticheat
{
    Movement::Movement(Player* me) : _me(me), _jumpInitialSpeed(0.f), _inKnockBack(false), _anticheat(reinterpret_cast<SessionAnticheat*>(me->GetSession()->GetAntiCheat())), _serverInitTime(0), _clientInitTime(0), _justTeleported(false), _totalDistanceTraveled(0.f), overSpeedDistanceTick(0.f), overSpeedDistanceTotal(0.f), _wasMovingOther(false)
    {
    ...
    void Movement::VerifyMovementFlags(uint32 flags, uint32& removeFlags, bool strict) const
    {
        ...
        {
            static constexpr uint32 flyHackFlags1 = (MOVEFLAG_WALK_MODE | MOVEFLAG_FIXED_Z | MOVEFLAG_SWIMMING | MOVEFLAG_HOVER | MOVEFLAG_FALLINGFAR | MOVEFLAG_FLYING | MOVEFLAG_ONTRANSPORT);
            static constexpr uint32 flyHackFlags2 = (MOVEFLAG_SWIMMING | MOVEFLAG_FLYING);

            if ((flags & flyHackFlags1) == flyHackFlags1)
            {
                _anticheat->RecordCheatInternal(CHEAT_TYPE_FLY_HACK, "MovementFlags hack: Player %s had old flyhack moveFlags mask (flags: 0x%lx)", _me->GetName(), flags);
                removeFlags |= flags;
            }
            ...
            {
                _anticheat->RecordCheatInternal(CHEAT_TYPE_FLY_HACK, "MovementFlags hack: Player %s had MOVEFLAG_SWIMMING and MOVEFLAG_FLYING (flags: 0x%lx)", _me->GetName(), flags);
                removeFlags |= MOVEFLAG_SWIMMING | MOVEFLAG_FLYING;
            }

            // no need to check pending orders.  players should never this.
            if (flags & MOVEFLAG_LEVITATING)
            {
                _anticheat->RecordCheatInternal(CHEAT_TYPE_FLY_HACK, "MovementFlags hack: Player %s had MOVEFLAG_LEVITATING (only for creatures!) (flags: 0x%lx)", _me->GetName(), flags);
            ...
            {
                _anticheat->RecordCheatInternal(CHEAT_TYPE_FLY_HACK, "MovementFlags hack: Player %s had MOVEFLAG_CAN_FLY (flags: 0x%lx)", _me->GetName(), flags);
                removeFlags |= MOVEFLAG_CAN_FLY;
            }
        }

        // no need to check pending orders.  players should never this.
        if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_FIXED_Z) && flags & MOVEFLAG_FIXED_Z)
        {
            _anticheat->RecordCheatInternal(CHEAT_TYPE_FIXED_Z, "MovementFlags hack: Player %s had MOVEFLAG_FIXED_Z (flags: 0x%lx)", _me->GetName(), flags);
            removeFlags |= MOVEFLAG_FIXED_Z;
        }
        ...
        if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_ROOT_MOVE) && !(flags & MOVEFLAG_ROOT) && !!(_me->GetUnitState() & (UNIT_STAT_STUNNED | UNIT_STAT_ROOT)) && !pendingRoot)
        ...
        if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_WATER_WALK) && !!(flags & MOVEFLAG_WATERWALKING) && !_me->HasAuraType(SPELL_AURA_WATER_WALK) && !_me->HasAuraType(SPELL_AURA_GHOST) && !pendingWaterWalkRemoval)
        {
        ...
        if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_SLOW_FALL) && !!(flags & MOVEFLAG_SAFE_FALL) && !_me->HasAuraType(SPELL_AURA_FEATHER_FALL) && !pendingSlowFallRemoval)
        {
            _anticheat->RecordCheatInternal(CHEAT_TYPE_SLOW_FALL, "MovementFlags hack: Player %s had MOVEFLAG_SLOW_FALL with no slow fall aura and no pending orders", _me->GetName());
    ...
    void Movement::OrderSent(uint16 opcode, uint32 counter)
    {
        ...
        // there is currently a bug (reported here: https://github.com/elysium-project/bug-tracker/issues/18) which causes the client to
        // ignore the the enabling of water-walking (it is unaware of the unit referenced in the guid field).  therefore, we will never
        // receive an acknowledgement of that order.  once this issue is fixed, this if () block can be removed.
        if (opcode == SMSG_MOVE_WATER_WALK || opcode == SMSG_MOVE_LAND_WALK)
            return;

        _orders.emplace_back<PendingOrder>({opcode, counter, WorldTimer::getMSTime()});

        sLog.out(LOG_ANTICHEAT_DEBUG, "ORDER: %s (%u) sent to %s (counter = %u)", LookupOpcodeName(opcode), opcode, _me->GetName(), counter);
    }
    ...
    void Movement::OrderAck(uint16 opcode, uint32 counter)
    {
        ...
        // see above comment about https://github.com/elysium-project/bug-tracker/issues/18
        if (opcode == CMSG_MOVE_WATER_WALK_ACK)
            return;

        sLog.out(LOG_ANTICHEAT_DEBUG, "ORDER: %s (%u) ACK %s (counter = %u)", LookupOpcodeName(opcode), opcode, _me->GetName(), counter);

        if (_orders.empty())
        {
            if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_BAD_ORDER_ACK))
                _anticheat->RecordCheatInternal(CHEAT_TYPE_BAD_ORDER_ACK, "Received ACK %s (%u) but no orders are pending", LookupOpcodeName(opcode), opcode);
        ...
        _anticheat->RecordCheatInternal(CHEAT_TYPE_BAD_ORDER_ACK, "Received ACK %s (%u) counter = %u which was not pending", LookupOpcodeName(opcode), opcode, counter);
    }
    ...
    void Movement::CheckExpiredOrders(uint32 latency)
    {
        ...
        for (auto i = _orders.begin(); i != _orders.end(); ++i)
        {
            ...
            if (expected < now)
            {
                //_anticheat->RecordCheatInternal(CHEAT_TYPE_BAD_ORDER_ACK, "Expired order received no ACK for %s (%u) counter %u sent %u expected %u latency %u now %u",
    ...
    bool Movement::HandleAnticheatTests(MovementInfo& movementInfo, WorldSession* session, const WorldPacket& packet)
    {
        ...
        auto const dist2d = (movementInfo.pos.x - GetLastMovementInfo().pos.x) * (movementInfo.pos.x - GetLastMovementInfo().pos.x) + (movementInfo.pos.y - GetLastMovementInfo().pos.y) * (movementInfo.pos.y - GetLastMovementInfo().pos.y);
        ...
        if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_MULTIJUMP) && opcode == MSG_MOVE_JUMP && GetLastMovementInfo().moveFlags & MOVEFLAG_JUMPING)
        {
            ...
            sLog.out(LOG_ANTICHEAT_BASIC, "Multijump debug output:\n%s", str.c_str());
        }
        ...
        if (opcode == MSG_MOVE_JUMP && sAnticheatConfig.IsEnabled(CHEAT_TYPE_OVERSPEED_JUMP) && !IsInKnockBack())
        {
            ...
            auto const expected = !!(GetLastMovementInfo().moveFlags & (MOVEFLAG_JUMPING | MOVEFLAG_SWIMMING)) ? GetXZFlagBasedClientSpeed(GetLastMovementInfo().moveFlags) : GetXZFlagBasedClientSpeed(movementInfo.moveFlags);

            // XXX: these should all be temporary until the above logic is perfected
            auto const oldServerSpeed = _me->GetXZFlagBasedSpeed(GetLastMovementInfo().moveFlags);
            auto const oldClientSpeed = GetXZFlagBasedClientSpeed(GetLastMovementInfo().moveFlags);
        ...
        if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_JUMP_SPEED_CHANGE) && !IsInKnockBack() && opcode != CMSG_MOVE_KNOCK_BACK_ACK && opcode != MSG_MOVE_STOP && (movementInfo.moveFlags & (MOVEFLAG_JUMPING | MOVEFLAG_FALLINGFAR)) && (GetLastMovementInfo().moveFlags & (MOVEFLAG_JUMPING | MOVEFLAG_FALLINGFAR)) && movementInfo.jump.xyspeed > GetLastMovementInfo().jump.xyspeed + 0.0001f && GetLastMovementInfo().jump.xyspeed > 0.0001f && !(!!((movementInfo.moveFlags | GetLastMovementInfo().moveFlags) & MOVEFLAG_WATERWALKING) && GetLastMovementInfo().jump.xyspeed < 0.001f)) // true when entering water with waterwalking
        {
            _anticheat->RecordCheatInternal(CHEAT_TYPE_JUMP_SPEED_CHANGE, "Lateral jump speed changed from %f to %f flags 0x%lx -> 0x%lx opcode %s jump velocity: %f cos: %f sin: %f fall time: %d", GetLastMovementInfo().jump.xyspeed, movementInfo.jump.xyspeed, GetLastMovementInfo().moveFlags, movementInfo.moveFlags, LookupOpcodeName(opcode), movementInfo.jump.zspeed, movementInfo.jump.cosAngle, movementInfo.jump.sinAngle, movementInfo.fallTime);

            std::string str;
            DumpMovement(str);
            sLog.out(LOG_ANTICHEAT_BASIC, "Lateral check recent movements:\n%s", str.c_str());
        }
        ...
        else
        {
            if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_NUM_DESYNC) && !_anticheat->IsKickTimerActive() && !_anticheat->IsBanTimerActive())
            {
                ...
                {
                    _anticheat->RecordCheatInternal(CHEAT_TYPE_NUM_DESYNC, "Client desync exceeded tolerance.  Desync: %d server diff: %d client diff: %d tolerance: %f", clientDiff - serverDiff, serverDiff, clientDiff, tolerance);
                }
            }
            ...
            if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_TIME_BACK) && movementInfo.ctime < GetLastMovementInfo().ctime)
            {
                ...
                {
                    _anticheat->RecordCheatInternal(CHEAT_TYPE_TIME_BACK, "Clock moved in reverse from %u to %u, possible bot!", GetLastMovementInfo().ctime, movementInfo.ctime);
                }
                else
                {
                    std::string log;
                    ...
                    sLog.out(LOG_ANTICHEAT_BASIC, "TimeBack debug output.\n%s", log.c_str());
                }
            }
        }
            ...
            if (BattleGround* bg = _me->GetBattleGround())
            {
                if (bg->GetStatus() == STATUS_WAIT_JOIN)
                {
                    ...
                    if (_me->GetTeam() == ALLIANCE && movementInfo.pos.x < 1490.0f)
                        _anticheat->RecordCheatInternal(CHEAT_TYPE_FORBIDDEN, "Outside alliance starting area (%f, %f, %f)", movementInfo.pos.x, movementInfo.pos.y, movementInfo.pos.z);
                    if (_me->GetTeam() == HORDE && movementInfo.pos.x > 957.0f)
                        _anticheat->RecordCheatInternal(CHEAT_TYPE_FORBIDDEN, "Outside horde starting area (%f, %f, %f)", movementInfo.pos.x, movementInfo.pos.y, movementInfo.pos.z);
                }
            }
        }

        // Minimal checks on transports
        if (movementInfo.moveFlags & MOVEFLAG_ONTRANSPORT && sAnticheatConfig.IsEnabled(CHEAT_TYPE_TELE_TO_TRANSPORT) && GetLastMovementInfo().ctime && !(GetLastMovementInfo().moveFlags & MOVEFLAG_ONTRANSPORT))
        {
        ...
        if (!_me->IsTaxiFlying() && !(movementInfo.moveFlags & MOVEFLAG_ONTRANSPORT) && sAnticheatConfig.EnableAntiSpeedHack() && !_justTeleported)
        {
                    ...
                    if (theirDist2 > extrapDist2)
                    {
                        ...
                        if (delta >= minErr)
                            if (auto const anticheat = dynamic_cast<AnticheatLib*>(GetAnticheatLib()))
                                anticheat->OfferExtrapolationData(GetLastMovementInfo(), clientSpeeds[GetMoveType(GetLastMovementInfo().moveFlags)], clientSpeeds[GetMoveType(movementInfo.moveFlags)], movementInfo, extrap, delta);
                    }
                }
            }
            else if (GetMaxAllowedDist(GetLastMovementInfo(), dt, allowedDXY, allowedDZ))
            {
                ...
                if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_OVERSPEED_Z) && fabs(movementInfo.pos.z - GetLastMovementInfo().pos.z) > allowedDZ)
                    _anticheat->RecordCheatInternal(CHEAT_TYPE_OVERSPEED_Z, "z changed from %f to %f, difference of %f, max allowed: %f", GetLastMovementInfo().pos.z, movementInfo.pos.z, fabs(movementInfo.pos.z - GetLastMovementInfo().pos.z), allowedDZ);
            }
            ...
            if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_SKIPPED_HEARTBEATS) && dt > 1000 && GetLastMovementInfo().ctime && GetLastMovementInfo().moveFlags & MOVEFLAG_MASK_MOVING)
        ...
        _moveHistory.push_back({packet.GetOpcode(), movementInfo});
    ...
    bool Movement::HandleSpeedChangeAck(MovementInfo& movementInfo, WorldSession* session, const WorldPacket& packet, float newSpeed)
    {
        ...
        {
            DETAIL_LOG("%sSpeedChange player %s incorrect. %f -> %f (instead of %f)", move_type_name[moveType], _me->GetName(), clientSpeeds[moveType], newSpeed, _me->GetSpeed(moveType));
            _me->SetSpeedRate(moveType, _me->GetSpeedRate(moveType));
        }
    ...
    void Movement::TimeSkipped(const ObjectGuid& mover, uint32 ms)
    {
        ...
        sLog.out(LOG_ANTICHEAT_DEBUG, "Time skipped: Player %s (GUID 0x%lx) mover 0x%lx ms %u", _me ? _me->GetName() : "<none>", _me ? _me->GetGUIDLow() : 0, mover.GetRawValue(), ms);
    }

    MovementInfo& Movement::GetLastMovementInfo() const { return _me->m_movementInfo; }

    float Movement::GetXZFlagBasedClientSpeed(uint32 moveFlags) const
    {
    ...
    bool Movement::ExtrapolateMovement(MovementInfo const& mi, uint32 diffMs, Position& pos) const
    {
        ...
        return _me->GetMap()->isInLineOfSight(mi.pos.x, mi.pos.y, mi.pos.z + 0.5f, pos.x, pos.y, pos.z + 0.5f);
    }
    ...
    void Movement::OnExplore(AreaEntry const* p)
    {
        if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_EXPLORE))
            _anticheat->RecordCheatInternal(CHEAT_TYPE_EXPLORE);

        if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_EXPLORE_HIGH_LEVEL) && static_cast<int32>(_me->GetLevel() + 10) < p->AreaLevel)
            _anticheat->RecordCheatInternal(CHEAT_TYPE_EXPLORE_HIGH_LEVEL);
    }

    void Movement::HandleTeleport(const Position&) { _justTeleported = true; }
    ...
    void Movement::OnTransport(Player* plMover, ObjectGuid transportGuid)
    {
        ...
        if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_FAKE_TRANSPORT) && (!transportGobj || !transportGobj->IsTransport() || !transportGobj->IsWithinDist(plMover, maxDist2d, false)))
    ...
    bool Movement::CheckTeleport(uint16 opcode, MovementInfo& movementInfo)
    {
        ...
        if (!mover->m_transport && !mover->HasMovementFlag(MOVEFLAG_ONTRANSPORT) && !mover->IsTaxiFlying())
        {
            ...
            if (!IsTeleportAllowed(movementInfo, distance))
            {
                ...
                if (sAnticheatConfig.IsEnabled(cheatType))
                    _anticheat->RecordCheatInternal(cheatType,
                                                    "Teleport hack: player %s, %s, %.2f yd\n"
                                                    "    map %u \"%s\"\n"
                                                    "    source: zone %u \"%s\" area %u \"%s\" %.2f, %.2f, %.2f\n"
                                                    "    dest:   zone %u \"%s\" area %u \"%s\" %.2f, %.2f, %.2f",
                                                    mover->GetName(), LookupOpcodeName(opcode), distance, mover->GetMapId(), mapName, mover->GetZoneId(), srcZoneName, mover->GetAreaId(), srcAreaName, mover->GetPositionX(), mover->GetPositionY(), mover->GetPositionZ(), destZoneId, destZoneName, destAreaId, destAreaName, movementInfo.pos.x, movementInfo.pos.y, movementInfo.pos.z);
        ...
        if (!mover->IsLaunched() && !mover->IsFalling() && (!mover->movespline || mover->movespline->Finalized()))
        {
            ...
            if (!!removeFlags)
            {
                ...
                sLog.out(LOG_ANTICHEAT_BASIC, "ANTICHEAT: Player %s sent bad movement flags (0x%lx) with opcode %s (%u)", mover->GetName(), (uint32)moveFlags, LookupOpcodeName(opcode), opcode);
    ...
    {
        handler->PSendSysMessage("Order ACKs pending: %u Current time: %u", _orders.size(), WorldTimer::getMSTime());
        ...
        if (!_orders.empty())
        {
            ...
            for (auto const& order : _orders)
            {
                handler->PSendSysMessage("Next: %s Age: %ums", LookupOpcodeName(order.opcode), WorldTimer::getMSTimeDiffToNow(order.time));
    ...
    size_t Movement::DumpOrders(std::string& out) const
    {
        ...
        {
            str << LookupOpcodeName(order.opcode) << " at " << order.time << " (" << WorldTimer::getMSTimeDiff(order.time, now) << "ms ago)\n";
            ++result;
        }
    ...
    size_t Movement::DumpAcks(std::string& out) const
    {
        ...
        {
            str << LookupOpcodeName(ack.opcode) << " at " << ack.time << " (" << WorldTimer::getMSTimeDiff(ack.time, now) << "ms ago)\n";
            ++result;
        }
    ...
    size_t Movement::DumpTimeSkips(std::string& out) const
    {
        ...
        {
            str << skip.mover.GetCounter() << ": Skipped " << skip.ms << " ms at time " << skip.time << " (" << WorldTimer::getMSTimeDiff(skip.time, now) << "ms ago)\n";
            ++result;
        }
    ...
    size_t Movement::DumpMovement(std::string& out) const
    {
        ...
        {
            str << move.movementInfo.ctime << ": age " << (now - move.movementInfo.stime) << " flags 0x" << std::hex << move.movementInfo.moveFlags << std::dec << " at (" << move.movementInfo.pos.x << ", " << move.movementInfo.pos.y << ", " << move.movementInfo.pos.z << ")";

            if (move.movementInfo.HasMovementFlag(MOVEFLAG_JUMPING))
                str << " jump xy speed " << move.movementInfo.jump.xyspeed << " start ctime " << move.movementInfo.jump.startClientTime;
