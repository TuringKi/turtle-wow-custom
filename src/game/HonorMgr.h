...
{
    HonorStanding() : guid(0), cp(0.0f) {}

    uint32 guid;
    float cp;

    // create the standing order
    bool operator<(const HonorStanding& hs) const { return cp > hs.cp; }
};
...
public:
    ...
    static uint32 GetMostHkYesterdayPlayerGuid() { return m_mostHkYesterdayGuid; }
    static uint32 GetMostDkYesterdayPlayerGuid() { return m_mostDkYesterdayGuid; }
