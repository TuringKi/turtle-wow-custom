...
auto NoMovementAreas = make_array<restricted_movement_t>(std::make_tuple(1u, 5122u, 5121u), // The Shallow Strand, Tel'Abim
                                                         std::make_tuple(1u, 5128u, 5121u), // Jagged Isles, Tel'Abim
                                                         std::make_tuple(1u, 5129u, 5121u) // The Washing Shore, Tel'Abim
);

////////////////// PathInfo //////////////////
PathInfo::PathInfo(const Unit* owner) : m_polyLength(0), m_type(PATHFIND_BLANK), m_useStraightPath(false), m_forceDestination(false), m_pointPathLimit(MAX_POINT_PATH_LENGTH), m_transport(nullptr), m_sourceUnit(owner), m_navMesh(nullptr), m_navMeshQuery(nullptr), m_targetAllowedFlags(0)
{
...
void PathInfo::setPathLengthLimit(float dist) { m_pointPathLimit = std::min<uint32>(MAX_POINT_PATH_LENGTH, uint32(dist / SMOOTH_PATH_STEP_SIZE)); }
...
bool PathInfo::calculate(Vector3 const& start, Vector3 dest, bool forceDest, bool offsets)
{
    ...
    if (!m_navMesh || !m_navMeshQuery || m_sourceUnit->HasUnitState(UNIT_STAT_IGNORE_PATHFINDING) || !HaveTiles(start) || !HaveTiles(dest))
    {
        BuildShortcut();
    ...
    else
    {
        ...
        if (m_type & PATHFIND_NOPATH)
        {
            ...
            bool ignoreMmaps = std::find_if(std::begin(NoMovementAreas), std::end(NoMovementAreas),
                                            [owner](const restricted_movement_t& t)
                                            {
...
void PathInfo::BuildPolyPath(const Vector3& startPos, const Vector3& endPos)
{
    ...
    bool const canSwimToDestination = m_sourceUnit->CanSwim() && m_sourceUnit->GetTerrain()->IsSwimmable(startPos.x, startPos.y, startPos.z) && m_sourceUnit->GetTerrain()->IsSwimmable(endPos.x, endPos.y, endPos.z);
    ...
    if (startPoly == INVALID_POLYREF || endPoly == INVALID_POLYREF)
    {
        ...
        if ((startPoly == INVALID_POLYREF && m_sourceUnit->GetTerrain()->IsSwimmable(startPos.x, startPos.y, startPos.z)) || (endPoly == INVALID_POLYREF && m_sourceUnit->GetTerrain()->IsSwimmable(endPos.x, endPos.y, endPos.z)))
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        dtStatus dtResult = m_navMeshQuery->findPath(suffixStartPoly, // start polygon
                                                     endPoly, // end polygon
    ...
    else
    {
        ...
        dtStatus dtResult = m_navMeshQuery->findPath(startPoly, // start polygon
                                                     endPoly, // end polygon
...
void PathInfo::BuildPointPath(const float* startPoint, const float* endPoint, float distToStartPoly, float distToEndPoly)
{
    ...
    {
        dtResult = m_navMeshQuery->findStraightPath(startPoint, // start position
                                                    endPoint, // end position
...
float Distance2DPointToLineYZX(float* lineA, float* lineB, float* point) { return std::abs(CrossProduct(lineA, lineB, point) / Distance(lineA, lineB)); }

dtStatus PathInfo::findSmoothPath(const float* startPos, const float* endPos, const dtPolyRef* polyPath, uint32 polyPathSize, float* smoothPath, int* smoothPathSize, uint32 maxSmoothPathSize)
{
...
bool PathInfo::UpdateForCaster(Unit* pTarget, float castRange)
{
    ...
    if (pTarget->IsWithinDist3d(m_sourceUnit->GetPositionX(), m_sourceUnit->GetPositionY(), m_sourceUnit->GetPositionZ(), castRange) && pTarget->IsWithinLOS(m_sourceUnit->GetPositionX(), m_sourceUnit->GetPositionY(), m_sourceUnit->GetPositionZ()))
    {
        clear();
    ...
    {
        if (pTarget->IsWithinDist3d(m_pathPoints[i].x, m_pathPoints[i].y, m_pathPoints[i].z, castRange) && pTarget->IsWithinLOS(m_pathPoints[i].x, m_pathPoints[i].y, m_pathPoints[i].z))
        {
            ...
            if (pTarget->IsWithinDist3d(startPoint.x, startPoint.y, startPoint.z, castRange) && pTarget->IsWithinLOS(startPoint.x, startPoint.y, startPoint.z))
...
float PathInfo::dist3DSqr(const Vector3& p1, const Vector3& p2) { return (p1 - p2).squaredLength(); }
