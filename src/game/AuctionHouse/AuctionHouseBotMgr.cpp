...
void AuctionHouseBotMgr::Load()
{
    ...
    if (!result)
    {
        ...
        sLog.outBasic("");
        sLog.outBasic(">> Loaded 0 AuctionHouseBot items");
        return;
    }
    ...
    sLog.outBasic("");
    sLog.outBasic(">> Loaded %u AuctionHouseBot items", count);
    ...
    {
        sLog.outBasic("AHBot::Load() : No auction house for faction %u.", m_config->ahfid);
        return;
    }
    m_loaded = true;
}
...
void AuctionHouseBotMgr::AddItem(AuctionHouseBotEntry e, AuctionHouseObject* auctionHouse)
{
    ...
    {
        sLog.outBasic("AHBot::AddItem() : Item %u does not exist.", e.item);
        return;
    }
    ...
    if (!item)
    {
        sLog.outBasic("AHBot::AddItem() : Cannot create item.");
        return;
    }

    sLog.outBasic("AHBot::AddItem() : Adding item %u.", e.item);
