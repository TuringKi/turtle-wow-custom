...
struct SpellModifier
{
    ...
    SpellModifier(SpellModOp _op, SpellModType _type, int32 _value, uint32 _spellId, uint64 _mask, int16 _charges = 0) : op(_op), type(_type), charges(_charges), value(_value), mask(_mask), spellId(_spellId), ownerAura(nullptr) {}
...
{
public: // constructors
    TradeData(Player* player, Player* trader) : m_player(player), m_trader(trader), m_accepted(false), m_acceptProccess(false), m_money(0), m_spell(0), m_lastModificationTime(0), m_scamPreventionDelay(0) {}
...
struct ScheduledTeleportData
{
    ...
    ScheduledTeleportData(uint32 mapid, float x, float y, float z, float o, uint32 options) : targetMapId(mapid), x(x), y(y), z(z), orientation(o), options(options){};
...
public:
    ...
    void SetHCChat(bool on)
    {
        if (!on)
            m_ExtraFlags |= PLAYER_EXTRA_DISABLE_HC_CHAT;
        else
            m_ExtraFlags &= ~PLAYER_EXTRA_DISABLE_HC_CHAT;
    }
    ...
    void SetPlayerVariable(PlayerVariables variable, std::string value) { m_variables[variable] = value; }
...
public:
    ...
    InventoryResult CanStoreNewItem(uint8 bag, uint8 slot, ItemPosCountVec& dest, uint32 item, uint32 count, uint32* no_space_count = nullptr) const { return _CanStoreItem(bag, slot, dest, item, count, nullptr, false, no_space_count); }
    ...
    Item* BankItem(ItemPosCountVec const& dest, Item* pItem, bool update) { return StoreItem(dest, pItem, update); }
...
public:
    ...
    template <class T>
    T ApplySpellMod(uint32 spellId, SpellModOp op, T& basevalue, Spell* spell = nullptr);
...
public:
    ...
    void SetFallInformation(float fallStartZ) { m_fallStartZ = fallStartZ; }
    ...
    void DoIgnoreRelocation()
    {
        if (m_bNextRelocationsIgnored)
            --m_bNextRelocationsIgnored;
    }
...
public:
    ...
    bool HasCustomFlag(CustomPlayerFlags flag) const { return (customFlags & flag) == flag; }

    void SetCustomFlag(CustomPlayerFlags flags) { customFlags |= flags; }

    void RemoveCustomFlag(CustomPlayerFlags flags) { customFlags &= ~flags; }
    ...
    void SetHCImmunityTimer(uint32 secs) { m_hardcoreImmunityTime = time(nullptr) + secs; }

    void ScheduleStandStateChange(uint8 state);
    void ClearScheduledStandState()
    {
    ...
    void RemoveDelayedOperation(uint32 operation) { m_DelayedOperations &= ~operation; }
...
public:
    ...
    bool HasChallenge(Challenges challenge) const
    {
        static std::unordered_map<uint32, uint32> challenge_spells{
            {CHALLENGE_SLOW_AND_STEADY, SPELL_SLOW_AND_STEADY}, {CHALLENGE_EXHAUSTION_MODE, SPELL_EXHAUSTION_MODE}, {CHALLENGE_WAR_MODE, SPELL_WAR_MODE}, {CHALLENGE_HARDCORE, SPELL_HARDCORE}, {CHALLENGE_VAGRANT_MODE, SPELL_VARGANT_MODE}, {CHALLENGE_BOARING_MODE, SPELL_BOARING_MODE},
        };
    ...
    ObjectGuid GetChampionGUID() const { return m_currentChampionGuid; }

    void SetChampion(ObjectGuid champion) { m_currentChampionGuid = champion; }
...
public:
    ...
    {
        if (on)
            m_ExtraFlags |= PLAYER_EXTRA_WHISP_RESTRICTION;
        else
            m_ExtraFlags &= ~PLAYER_EXTRA_WHISP_RESTRICTION;
    }
    ...
    // Returns:   uint32
    //************************************
    uint32 GetLastMovementTime() const { return m_lastMovementTimer; }

    //************************************
    // Desc:    Last time when player casted spell in milliseconds. Call WorldTimer::getMSTime() - GetLastSpellCastedTime() to get delta
    // Returns:   uint32
    //************************************
    uint32 GetLastSpellCastedTime() const { return m_lastSpellTimer; }
    ...
    // Returns:   uint32
    //************************************
    uint32 GetLastChatMessageTime() const { return m_lastChatMessageTimer; }
    ...
    // Returns:   float
    //************************************
    float GetVelocityPerSecond() const { return m_velocity; }
    ...
    // Returns:   float
    //************************************
    float GetVelocityPer3Min() const { return m_velocityPer3Min; }
...
public:
    ...
    bool InBattleGroundQueueForBattleGroundQueueType(BattleGroundQueueTypeId bgQueueTypeId) const { return GetBattleGroundQueueIndex(bgQueueTypeId) < PLAYER_MAX_BATTLEGROUND_QUEUES; }
    ...
    void SetSoulShardCountBeforeBgJoin(uint32 count) { m_bgData.soulShardCount = count; }
    uint32 GetSoulShardCountBeforeBgJoin() const { return m_bgData.soulShardCount; }
...
template <class T>
T Player::ApplySpellMod(uint32 spellId, SpellModOp op, T& basevalue, Spell* spell)
{
