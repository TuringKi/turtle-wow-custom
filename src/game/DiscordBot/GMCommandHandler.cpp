...
void GMCommandHandler::ExecuteGMCommand(const std::string& command, const dpp::parameter_list_t& parameters, dpp::command_source src)
{
    ...
    sLog.outDiscord("%s", string_format_depr("Executing command %s for Discord user %s (%llu). Account (%s / %u)", command.c_str(), src.issuer.format_username().c_str(), static_cast<uint64>(src.issuer.id), authinfo->gameAccountName.c_str(), authinfo->gameAccountId).c_str());
...
{
    dpp::message msg;
    msg.add_component(dpp::component().add_component(dpp::component().set_type(dpp::cot_selectmenu).set_placeholder("Filter on").add_select_option(dpp::select_option("Player name", "playername", "The name (or part of it) of the player")).add_select_option(dpp::select_option("Player level minimum", "playerlevelmin", "The minimum level of the player")).add_select_option(dpp::select_option("Player level maximum", "playerlevelmax", "The maximum level of the player")).set_id("filterselect")));
