...
ChannelBroadcaster::ChannelBroadcaster() : MessageQueue(15) { StartThread(); }

ChannelBroadcaster::~ChannelBroadcaster() { Stop(); }

void ChannelBroadcaster::StartThread()
{
    Worker = new std::thread([this]() { ThreadProc(); });
}
...
void ChannelBroadcaster::EnqueueMessage(std::string&& Message, const std::string& ChannelName, ObjectGuid PlayerGuid, uint32 Language, Team ChannelTeam, bool bSkipChecks) { MessageQueue.enqueue(ChannelMessage{std::move(Message), ChannelName, PlayerGuid, Language, ChannelTeam, bSkipChecks}); }
