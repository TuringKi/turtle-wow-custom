...
void WorldSession::HandleDestroyItemOpcode(WorldPacket& recv_data)
{
    ...
    // don't recover these.
    if (pProto->Duration || hasSpellCharges || pProto->IsQuestItem || pProto->DisplayInfoID == 33537) // Signet Ring of the Bronze Dragonflight
        save = false;

    // Turtle: save destroyed items so they can be restored
    if (save && (count <= pItem->GetCount()) && !pProto->Duration && !pProto->Map && !pProto->Area && (pProto->Quality >= ITEM_QUALITY_RARE || pProto->StartQuest))
...
void WorldSession::SendListInventory(ObjectGuid vendorguid, uint8 menu_type)
{
    ...
    if (!pCreature)
    {
        ...
        {
            CharacterDatabase.AsyncPQuery(&WorldSession::HandleListRestoreItemsCallBack, GetAccountId(), std::to_string(vendorguid.GetRawValue()), "SELECT `item_entry`, `stack_count` FROM `character_destroyed_items` WHERE `player_guid`=%u ORDER BY `time` DESC LIMIT 128", _player->GetGUIDLow());
            return;
        }
        ...
        if (crItem)
        {
            if (ItemPrototype const* pProto = sObjectMgr.GetItemPrototype(crItem->item))
            {
                if (!_player->IsGameMaster())
                {
                    ...
                    if (!pProto->RequiredReputationFaction && pProto->RequiredReputationRank > 0 && ReputationRank(pProto->RequiredReputationRank) > _player->GetReputationRank(pCreature->GetFactionId()))
                        continue;
