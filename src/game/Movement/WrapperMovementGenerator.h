...
{
public:
    AbstractWrapperMovementGenerator(Movement::MoveSplineInit& spline, uint32 id = 0, bool delayed = false, bool resident = true, ObjectGuid guid = ObjectGuid(), uint32 relayId = 0) : i_spline(spline), i_id(id), i_delayed(delayed), i_resident(resident), i_dispatched(false), i_informed(false), i_useTimer(false), i_timerSet(false), m_guid(guid), m_relayId(relayId) {}
...
{
public:
    EffectMovementGenerator(Movement::MoveSplineInit& spline, uint32 id = 0, bool delayed = false, ObjectGuid guid = ObjectGuid(), uint32 relayId = 0) : AbstractWrapperMovementGenerator(spline, id, delayed, false, guid, relayId) {}
