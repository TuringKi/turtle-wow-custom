...
{
protected:
    TargetedMovementGeneratorMedium(Unit& target, float offset, float angle) : TargetedMovementGeneratorBase(target), m_checkDistanceTimer(0), m_fOffset(offset), m_fAngle(angle), m_bRecalculateTravel(false), m_bTargetReached(false), m_bReachable(true), m_fTargetLastX(0), m_fTargetLastY(0), m_fTargetLastZ(0), m_bTargetOnTransport(false) {}
...
{
public:
    explicit ChaseMovementGenerator(Unit& target) : TargetedMovementGeneratorMedium<T, ChaseMovementGenerator<T>>(target) {}
...
{
public:
    explicit FollowMovementGenerator(Unit& target) : TargetedMovementGeneratorMedium<T, FollowMovementGenerator<T>>(target) {}
