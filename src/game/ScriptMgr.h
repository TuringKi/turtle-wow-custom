...
enum eScriptCommand
{
    ...
    SCRIPT_COMMAND_SEND_SCRIPT_EVENT = 85, // source = Cgeneric_script idreature
                                           // target = WorldObject
                                           // datalong = event_id
...
struct ScriptInfo
{
    ...
    ScriptInfo() : id(0), delay(0), command(0), condition(0), target_param1(0), target_param2(0), target_type(0), x(0), y(0), z(0), o(0) { memset(raw.data, 0, sizeof(raw.data)); }
    ...
    uint32 GetGOGuid() const
    {
        ...
        case SCRIPT_COMMAND_LOAD_GAMEOBJECT_SPAWN:
            return loadGo.goGuid;
        case SCRIPT_COMMAND_OPEN_DOOR:
            return openDoor.goGuid;
        case SCRIPT_COMMAND_CLOSE_DOOR:
            return closeDoor.goGuid;
        default:
            return 0;
        }
    }
...
struct ScriptAction
{
    ...
    bool IsSameScript(uint32 id, ObjectGuid sourceGuid, ObjectGuid targetGuid) const { return id == script->id && (sourceGuid == this->sourceGuid || !sourceGuid) && (targetGuid == this->targetGuid || !targetGuid); }
};
...
enum ScriptTarget
{
    ...
    TARGET_T_NEAREST_CREATURE_WITH_ENTRY = 8, // Searches for closest creature with the given entry.
    ...
    TARGET_T_NEAREST_GAMEOBJECT_WITH_ENTRY = 11, // Searches for closest gameobject with the given entry.
...
void DoScriptText(int32 textEntry, WorldObject* pSource, Unit* target = nullptr, int32 chatTypeOverride = -1, float rangeOverride = 0.0f);
...
public:
    ...
    uint32 GetQuestStage() const { return QuestStage; }

    Player* GetPlayer() const;

    const Quest* GetQuestTemplate() const { return QuestTemplate; }

    const uint32 GetQuestId() const { return QuestID; }
...
protected:
    ...
    void SetQuestStage(uint32 newStage) { QuestStage = newStage; }
...
struct Script
{
    Script() : Name(""), QuestID(0), pGossipHello(nullptr), pGOGossipHello(nullptr), pQuestAcceptNPC(nullptr), pGossipSelect(nullptr), pGOGossipSelect(nullptr), pGossipSelectWithCode(nullptr), pGOGossipSelectWithCode(nullptr), pQuestComplete(nullptr), pNPCDialogStatus(nullptr), pGODialogStatus(nullptr), pQuestRewardedNPC(nullptr), pQuestRewardedGO(nullptr), pItemHello(nullptr), pGOHello(nullptr), pAreaTrigger(nullptr), pProcessEventId(nullptr), pItemQuestAccept(nullptr), pGOQuestAccept(nullptr), pItemUse(nullptr), pItemUseSpell(nullptr), pEffectDummyCreature(nullptr), pEffectDummyGameObj(nullptr), pEffectAuraDummy(nullptr), GOOpen(nullptr), GOGetAI(nullptr), GetAI(nullptr), GetQuestInstance(nullptr), GetInstanceData(nullptr) {}
