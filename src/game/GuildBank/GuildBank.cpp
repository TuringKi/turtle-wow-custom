...
GuildBank::GuildBank(bool isInfenoBank) { b_infernoBank = isInfenoBank; }
...
void GuildBank::SaveToDB()
{
    ...
    if (b_money_changed)
    {
        CharacterDatabase.PExecute("UPDATE guild_bank_money SET money = '%u' WHERE guildid = '%u' AND isInferno = '%u'", b_money, guildid, b_infernoBank);
    ...
    if (b_tabs_changed)
    {
        ...
        stmt.PExecute(b_tabs, b_tabInfo[1].name.c_str(), b_tabInfo[2].name.c_str(), b_tabInfo[3].name.c_str(), b_tabInfo[4].name.c_str(), b_tabInfo[5].name.c_str(), b_tabInfo[1].icon.c_str(), b_tabInfo[2].icon.c_str(), b_tabInfo[3].icon.c_str(), b_tabInfo[4].icon.c_str(), b_tabInfo[5].icon.c_str(), b_tabInfo[1].withdrawals, b_tabInfo[2].withdrawals, b_tabInfo[3].withdrawals, b_tabInfo[4].withdrawals, b_tabInfo[5].withdrawals, b_tabInfo[1].minrank, b_tabInfo[2].minrank, b_tabInfo[3].minrank, b_tabInfo[4].minrank, b_tabInfo[5].minrank, guildid, b_infernoBank ? (uint8)1 : (uint8)0);
        ...
        for (uint32 i = 0; i < b_moneyLog.size(); ++i)
            if (b_moneyLog[i].state == ITEM_NEW)
            {
                CharacterDatabase.PExecute("INSERT INTO guild_bank_log "
                                           ...
                                           guildid, b_infernoBank, b_moneyLog[i].player, b_moneyLog[i].action, 0, 0, 0, 0, b_moneyLog[i].money, b_moneyLog[i].stamp);
            ...
            for (uint32 j = 0; j < b_log[i].size(); ++j)
                if (b_log[i][j].state == ITEM_NEW)
                {
                    CharacterDatabase.PExecute("INSERT INTO guild_bank_log "
                                               "(guildid, isInferno, player, action, tab, item, randomPropertyId, enchant, count, stamp) "
                                               "VALUES ('%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u')",
                                               guildid, b_infernoBank, b_log[i][j].player, b_log[i][j].action, b_log[i][j].tab, b_log[i][j].item, b_log[i][j].randomPropertyId, b_log[i][j].enchant, b_log[i][j].count, b_log[i][j].stamp);
...
void GuildBank::UpdateMinranks(uint32 rank)
{
    for (uint32 tab = 1; tab <= MAX_TABS; ++tab)
    {
        if (b_tabInfo[tab].minrank == rank)
        {
            ...
            SendGuildMessage("GUpdateTab:" + std::to_string(tab) + ":" + b_tabInfo[tab].name + ":" + b_tabInfo[tab].icon + ":" + std::to_string(b_tabInfo[tab].withdrawals) + ":" + std::to_string(b_tabInfo[tab].minrank));
        }
    }
}
...
void GuildBank::LoadBank()
{
    ...
    QueryResult* guildBankTabs = CharacterDatabase.PQuery("SELECT `tabs`, `money`, "
                                                          ...
                                                          "WHERE `bt`.`guildid` = '%u' AND `bt`.`isInferno` = '%u'",
...
void GuildBank::UnlockGuildBank(std::string msg)
{
    ...
    if (query == "Ok")
    {
        ...
        // insert tabs
        CharacterDatabase.DirectPExecute("INSERT INTO guild_bank_tabs (guildid, isInferno, tabs) VALUES ('%u', '%u', '%u')", guildid, b_infernoBank, 1);

        // insert money
        CharacterDatabase.DirectPExecute("INSERT INTO guild_bank_money (guildid, isInferno, money) VALUES ('%u', '%u', '%u')", guildid, b_infernoBank, 0);
...
void GuildBank::SendBankMoney() { _player->SendAddonMessage(prefix, "BankInfo:money:" + std::to_string(b_money)); }
...
void GuildBank::SendTabItems(std::string msg)
{
    ...
    for (uint32 i = 1; i <= BANK_TAB_SIZE; i++)
    {
        ...
        {
            message += std::to_string(bItem->tab) + ":" + std::to_string(bItem->slot) + ":" + std::to_string(bItem->guid) + ":" + std::to_string(bItem->item_template) + ":" + std::to_string(bItem->count) + ":" + (bItem->randomPropertyId > 0 ? bItem->Name : "0") + ":" + std::to_string(bItem->randomPropertyId) + ":" + std::to_string(bItem->enchant) + ";";
...
void GuildBank::DepositItem(std::string msg)
{
    ...
    else
    {
        ...
        if (dItem)
        {
            ...
            if (dItem->item_template == pItem->GetProto()->ItemId && dItem->count == pItem->GetCount() && dItem->count == dItem->maxStackSize && (dItem->randomPropertyId == pItem->GetItemRandomPropertyId()))
            {
...
void GuildBank::DepositItemInSlot(uint32 bankTab, uint32 bankSlot, Item* pItem, uint32 count, uint32 remainer, ItemUpdateState state, bool stack)
{
    ...
    // Update player view
    SendGuildMessage("GDeposit:" + std::to_string(bItem->tab) + ":" + std::to_string(bItem->slot) + ":" + std::to_string(bItem->guid) + ":" + std::to_string(bItem->item_template) + ":" + std::to_string(bItem->count) + ":" + (bItem->randomPropertyId > 0 ? bItem->Name : "0") + ":" + std::to_string(bItem->randomPropertyId) + ":" + std::to_string(bItem->enchant));
...
void GuildBank::WithdrawItem(uint32 bankTab, uint32 bankSlot, uint32 count, uint32 playerBag, uint32 playerSlot, BankItem* bItem, Item* pItem, uint32 remainer, bool swap)
{
    ...
    if (pItem)
    {
        ...
        else
        {
            ...
            SendGuildMessage("GPartialWithdraw:" + std::to_string(bItem->tab) + ":" + std::to_string(bItem->slot) + ":" + std::to_string(bItem->guid) + ":" + std::to_string(bItem->item_template) + ":" + std::to_string(bItem->count) + ":" + "0:0:0");
        }
...
void GuildBank::MoveItem(BankItem* sItem, BankItem* dItem, uint8 bankTab, uint8 fromSlot, uint8 toSlot)
{
    ...
    if (!dItem)
    {
        ...
        SendGuildMessage("GMoveItem:from:" + std::to_string(bankTab) + ":" + std::to_string(fromSlot) + ":" + "0:" + "0:" + "0:" + "0:" + "0:" + "0:" + "to:" + std::to_string(sItem->tab) + ":" + std::to_string(sItem->slot) + ":" + std::to_string(sItem->guid) + ":" + std::to_string(sItem->item_template) + ":" + std::to_string(sItem->count) + ":" + (sItem->randomPropertyId > 0 ? sItem->Name : "0") + ":" + std::to_string(sItem->randomPropertyId) + ":" + std::to_string(sItem->enchant));
        return;
    }
    else
    {
        ...
        if (sItem->item_template == dItem->item_template)
        {
            ...
            else if ((sItem->count + dItem->count) <= sItem->maxStackSize)
            {
                ...
                SendGuildMessage("GMoveItem:from:" + std::to_string(bankTab) + ":" + std::to_string(fromSlot) + ":" + "0:" + "0:" + "0:" + "0:" + "0:" + "0:" + "to:" + std::to_string(dItem->tab) + ":" + std::to_string(dItem->slot) + ":" + std::to_string(dItem->guid) + ":" + std::to_string(dItem->item_template) + ":" + std::to_string(dItem->count) + ":" + (dItem->randomPropertyId > 0 ? dItem->Name : "0") + ":" + std::to_string(dItem->randomPropertyId) + ":" + std::to_string(dItem->enchant));
                return;
            }
            else
            {
                ...
                SendGuildMessage("GMoveItem:from:" + std::to_string(sItem->tab) + ":" + std::to_string(sItem->slot) + ":" + std::to_string(sItem->guid) + ":" + std::to_string(sItem->item_template) + ":" + std::to_string(sItem->count) + ":" + (sItem->randomPropertyId > 0 ? sItem->Name : "0") + ":" + std::to_string(sItem->randomPropertyId) + ":" + std::to_string(sItem->enchant) + ":to:" + std::to_string(dItem->tab) + ":" + std::to_string(dItem->slot) + ":" + std::to_string(dItem->guid) + ":" + std::to_string(dItem->item_template) + ":" + std::to_string(dItem->count) + ":" + (dItem->randomPropertyId > 0 ? dItem->Name : "0") + ":" + std::to_string(dItem->randomPropertyId) + ":" + std::to_string(dItem->enchant));
                return;
            }
        }
        else
...
void GuildBank::SplitItem(std::string msg)
{
    ...
    if (!dItem)
    {
        ...
        // update player view
        SendGuildMessage("GSplitItem:from:" + std::to_string(sItem->tab) + ":" + std::to_string(sItem->slot) + ":" + std::to_string(sItem->guid) + ":" + std::to_string(sItem->item_template) + ":" + std::to_string(sItem->count) + ":" + "0:" + "0:" + "0:" + "to:" + std::to_string(dItem->tab) + ":" + std::to_string(dItem->slot) + ":" + std::to_string(dItem->guid) + ":" + std::to_string(dItem->item_template) + ":" + std::to_string(dItem->count) + ":" + "0:" + "0:" + "0");
    }
    else
    {
        // destination not empty
...
void GuildBank::DestroyItem(std::string msg)
{
    ...
    if (count == 0 || count == sItem->count)
    {
        ...
        SendGuildMessage("GDestroy:" + std::to_string(bankTab) + ":" + std::to_string(bankSlot) + ":" + "0" + ":" + std::to_string(sItem->item_template) + ":" + std::to_string(sItem->count));
    ...
    else
    {
        ...
        // decrease count in player view
        SendGuildMessage("GPartialDestroy:" + std::to_string(sItem->tab) + ":" + std::to_string(sItem->slot) + ":" + std::to_string(sItem->guid) + ":" + std::to_string(sItem->item_template) + ":" + std::to_string(sItem->count));
...
void GuildBank::SendTabWithdrawalsLeft(uint8 tab)
{
    ...
    if (b_tabInfo[tab].withdrawals == 0 || playerRankIndex < b_tabInfo[tab].minrank || playerRankIndex == 0)
        ...
        _player->SendAddonMessage(prefix, "TabWithdrawalsLeft:" + std::to_string(tab) + ":" + std::to_string(GetTabWithdrawalsLeft(tab)));
}
...
void GuildBank::SendTabLog(std::string msg)
{
    ...
    {
        message += std::to_string(b_log[tab][i].stamp) + ";" + b_log[tab][i].playerName + ";" + std::to_string(b_log[tab][i].action) + ";" + std::to_string(b_log[tab][i].item) + ";" + b_log[tab][i].itemName + ";" + b_log[tab][i].itemQuality + ";" + std::to_string(b_log[tab][i].count) + ";" + std::to_string(b_log[tab][i].randomPropertyId) + ";" + std::to_string(b_log[tab][i].enchant) + "=";
...
void GuildBank::SendMoneyLog(std::string msg)
{
    ...
    {
        message += std::to_string(b_moneyLog[i].stamp) + ";" + b_moneyLog[i].playerName + ";" + std::to_string(b_moneyLog[i].action) + ";" + std::to_string(b_moneyLog[i].money) + "=";
...
void GuildBank::LogAction(uint8 action, uint32 tab, uint32 itemID, uint32 count, BankItem* bItem)
{
    ...
    if (tab == TAB_MONEY)
    {
        ...
        SendGuildMessage("GMoneyLog:0:" + std::to_string(b_moneyLog[index].stamp) + ";" + b_moneyLog[index].playerName + ";" + std::to_string(b_moneyLog[index].action) + ";" + std::to_string(b_moneyLog[index].money) + ";" + std::to_string(index));
    ...
    else
    {
        ...
        // for real time log update
        SendGuildMessage("GTabLog:" + std::to_string(tab) + ":" + std::to_string(b_log[tab][index].stamp) + ";" + b_log[tab][index].playerName + ";" + std::to_string(action) + ";" + std::to_string(itemID) + ";" + b_log[tab][index].itemName + ";" + b_log[tab][index].itemQuality + ";" + std::to_string(count) + ";" + std::to_string(b_log[tab][index].randomPropertyId) + ";" + std::to_string(b_log[tab][index].enchant));
...
void GuildBank::SendGuildMessage(std::string message) { _guild->BroadcastToGuild(_player->GetSession(), prefix + "\t" + message, LANG_ADDON); }
...
void GuildBank::AddItem(BankItem* pItem) { b_items.push_back(pItem); }
...
void GuildBank::CloneItem(Item* item, BankItem* bItem)
{
    ...
    {
        item->SetUInt32Value(ITEM_FIELD_ENCHANTMENT + i * MAX_ENCHANTMENT_OFFSET + ENCHANTMENT_ID_OFFSET, strtoul(ench[i * MAX_ENCHANTMENT_OFFSET + ENCHANTMENT_ID_OFFSET], nullptr, 10));
        item->SetUInt32Value(ITEM_FIELD_ENCHANTMENT + i * MAX_ENCHANTMENT_OFFSET + ENCHANTMENT_DURATION_OFFSET, strtoul(ench[i * MAX_ENCHANTMENT_OFFSET + ENCHANTMENT_DURATION_OFFSET], nullptr, 10));
        item->SetUInt32Value(ITEM_FIELD_ENCHANTMENT + i * MAX_ENCHANTMENT_OFFSET + ENCHANTMENT_CHARGES_OFFSET, strtoul(ench[i * MAX_ENCHANTMENT_OFFSET + ENCHANTMENT_CHARGES_OFFSET], nullptr, 10));
    }
