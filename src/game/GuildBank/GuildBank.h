...
struct BankItem
{
    ...
    bool AddToDB()
    {
        return CharacterDatabase.PExecute("INSERT INTO guild_bank (guildid, guid, isInferno, tab, slot, item_template, "
                                          ...
                                          "'%s', '%u', '%s', '%u', '%u', "
                                          "'%u', '%u', '%u')",
                                          guildid, guid, is_inferno ? 1 : 0, tab, slot, item_template, creatorGuid, giftCreatorGuid, count, duration, charges.c_str(), flags, enchantments.c_str(), randomPropertyId, transmogrifyId, durability, text, generated_loot);
    }
    ...
    bool SaveToDB()
    {
        return CharacterDatabase.PExecute("UPDATE guild_bank SET "
                                          ...
                                          tab, is_inferno ? 1 : 0, slot, item_template, creatorGuid, giftCreatorGuid, count, duration, charges.c_str(), flags, enchantments.c_str(), randomPropertyId, transmogrifyId, durability, text, generated_loot, guildid, guid);
    }

    bool DeleteFromDB() { return CharacterDatabase.PExecute("DELETE FROM guild_bank WHERE guildid = '%u' and guid = '%u' AND isInferno = '%u'", guildid, guid, is_inferno ? 1 : 0); }
};
