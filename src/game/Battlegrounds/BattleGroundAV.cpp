...
{
    ASSERT(faction_id < BG_TEAMS_COUNT);
    if (aerial_id == BG_AV_AIR_ASSAULT_BEACON_SOLDIER || aerial_id == BG_AV_AIR_ASSAULT_GLOBAL_SOLDIER)
    {
        return (m_challengeStatus[faction_id][BG_AV_SOLDIER_AIR_ASSAULT] >= m_challengeGoals[faction_id][BG_AV_SOLDIER_AIR_ASSAULT]);
    }
    else if (aerial_id == BG_AV_AIR_ASSAULT_BEACON_LIEUTENANT || aerial_id == BG_AV_AIR_ASSAULT_GLOBAL_LIEUTENANT)
    {
        return (m_challengeStatus[faction_id][BG_AV_LIEUTENANT_AIR_ASSAULT] >= m_challengeGoals[faction_id][BG_AV_LIEUTENANT_AIR_ASSAULT]);
    }
    else if (aerial_id == BG_AV_AIR_ASSAULT_BEACON_COMMANDER || aerial_id == BG_AV_AIR_ASSAULT_GLOBAL_COMMANDER)
    {
        return (m_challengeStatus[faction_id][BG_AV_COMMANDER_AIR_ASSAULT] >= m_challengeGoals[faction_id][BG_AV_COMMANDER_AIR_ASSAULT]);
    }
...
{
    ASSERT(faction_id < BG_TEAMS_COUNT);
    return ((m_challengeStatus[faction_id][BG_AV_HIDE_CAVALRY_ASSAULT] >= m_challengeGoals[faction_id][BG_AV_HIDE_CAVALRY_ASSAULT]) && (m_challengeStatus[faction_id][BG_AV_TAMED_CAVALRY_ASSAULT] >= m_challengeGoals[faction_id][BG_AV_TAMED_CAVALRY_ASSAULT]));
}
...
{
    ASSERT(faction_id < BG_TEAMS_COUNT);
    return ((m_challengeStatus[faction_id][BG_AV_IRONDEEP_GROUND_ASSAULT] >= m_challengeGoals[faction_id][BG_AV_IRONDEEP_GROUND_ASSAULT]) || (m_challengeStatus[faction_id][BG_AV_COLDTOOTH_GROUND_ASSAULT] >= m_challengeGoals[faction_id][BG_AV_COLDTOOTH_GROUND_ASSAULT]));
}
...
{
    ASSERT(faction_id < BG_TEAMS_COUNT);
    return m_challengeStatus[faction_id][BG_AV_BLOOD_WORLDBOSS_ASSAULT] >= m_challengeGoals[faction_id][BG_AV_BLOOD_WORLDBOSS_ASSAULT];
}
/*********/
...
void BattleGroundAV::StartingEventCloseDoors() { DEBUG_LOG("BattleGroundAV: entering state STATUS_WAIT_JOIN ..."); }

void BattleGroundAV::StartingEventOpenDoors() { OpenDoorEvent(BG_EVENT_DOOR); }
...
void BattleGroundAV::EndBattleGround(Team winner)
{
    ...
    for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
        if (m_Nodes[i].State == POINT_CONTROLLED && m_Nodes[i].Owner != BG_AV_TEAM_NEUTRAL && i != BG_AV_NODES_FROSTWOLF_HUT && i != BG_AV_NODES_FIRSTAID_STATION)
...
void BattleGroundAV::ChangeMineOwner(uint8 mine, BattleGroundAVTeamIndex teamIdx)
{
    ...
    if (teamIdx != BG_AV_TEAM_NEUTRAL)
    {
        ...
        SendYell2ToAll(LANG_BG_AV_MINE_TAKEN, LANG_UNIVERSAL, GetSingleCreatureGuid(BG_AV_HERALD, 0), (teamIdx == BG_AV_TEAM_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE, (mine == BG_AV_NORTH_MINE) ? LANG_BG_AV_MINE_NORTH : LANG_BG_AV_MINE_SOUTH);
    }
...
void BattleGroundAV::EventPlayerDefendsPoint(Player* player, BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        SendYell2ToAll(LANG_BG_AV_TOWER_DEFENDED, LANG_UNIVERSAL, GetSingleCreatureGuid(BG_AV_HERALD, 0), GetNodeName(node), (teamIdx == BG_TEAM_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE);
        UpdatePlayerScore(player, SCORE_TOWERS_DEFENDED, 1);
        PlaySoundToAll(BG_AV_SOUND_BOTH_TOWER_DEFEND);
    }
    else
    {
        SendYell2ToAll(LANG_BG_AV_GRAVE_DEFENDED, LANG_UNIVERSAL, GetSingleCreatureGuid(BG_AV_HERALD, 0), GetNodeName(node), (teamIdx == BG_TEAM_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE);
...
void BattleGroundAV::EventPlayerAssaultsPoint(Player* player, BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        SendYell2ToAll(LANG_BG_AV_TOWER_ASSAULTED, LANG_UNIVERSAL, GetSingleCreatureGuid(BG_AV_HERALD, 0), GetNodeName(node), (teamIdx == BG_TEAM_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE);
        UpdatePlayerScore(player, SCORE_TOWERS_ASSAULTED, 1);
    }
    else
    {
        SendYell2ToAll(LANG_BG_AV_GRAVE_ASSAULTED, LANG_UNIVERSAL, GetSingleCreatureGuid(BG_AV_HERALD, 0), GetNodeName(node), (teamIdx == BG_TEAM_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE);
...
void BattleGroundAV::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    ...
    for (uint32 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
    {
        for (uint8 j = 0; j < BG_AV_MAX_STATES; j++)
        {
            ...
            FillInitialWorldState(data, count, BG_AV_NodeWorldStates[i][GetWorldStateType(j, BG_AV_TEAM_ALLIANCE)], m_Nodes[i].Owner == BG_AV_TEAM_ALLIANCE && stateok);
            FillInitialWorldState(data, count, BG_AV_NodeWorldStates[i][GetWorldStateType(j, BG_AV_TEAM_HORDE)], m_Nodes[i].Owner == BG_AV_TEAM_HORDE && stateok);
        }
    }
