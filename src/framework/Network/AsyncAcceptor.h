...
public:
    ...
    AsyncAcceptor(asio::io_context ioContext, std::string const& bindIp, uint16 port) : _acceptor(ioContext), _endpoint(asio::ip::make_address_v4(bindIp.c_str()), port), _socket(ioContext), _closed(false), _socketFactory(std::bind(&AsyncAcceptor::DefeaultSocketFactory, this)) {}
