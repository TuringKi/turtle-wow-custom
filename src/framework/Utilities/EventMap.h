...
public:
     ...
     */
    uint8 GetPhaseMask() const { return _phase; }
     ...
     */
    void ScheduleEvent(uint32 eventId, Milliseconds const& time, uint32 group = 0, uint8 phase = 0) { ScheduleEvent(eventId, uint32(time.count()), group, phase); }
     ...
     * @param phase The phase in which the event can occur. Has to be between 1 and 8. 0 means it can occur in all phases.
     */
    void ScheduleEvent(uint32 eventId, Milliseconds const& minTime, Milliseconds const& maxTime, uint32 group = 0, uint32 phase = 0) { ScheduleEvent(eventId, urand(uint32(minTime.count()), uint32(maxTime.count())), group, phase); }
     ...
     * @param phase The phase in which the event can occur. Has to be between 1 and 8. 0 means it can occur in all phases.
     */
    void RescheduleEvent(uint32 eventId, Milliseconds const& time, uint32 group = 0, uint8 phase = 0) { RescheduleEvent(eventId, uint32(time.count()), group, phase); }
     ...
     * @param phase The phase in which the event can occur. Has to be between 1 and 8. 0 means it can occur in all phases.
     */
    void RescheduleEvent(uint32 eventId, Milliseconds const& minTime, Milliseconds const& maxTime, uint32 group = 0, uint32 phase = 0) { RescheduleEvent(eventId, urand(uint32(minTime.count()), uint32(maxTime.count())), group, phase); }
     ...
     */
    void Repeat(Milliseconds const& time) { Repeat(uint32(time.count())); }
     ...
     */
    void Repeat(uint32 time) { _eventMap.insert(EventStore::value_type(_time + time, _lastEvent)); }
     ...
     */
    void Repeat(Milliseconds const& minTime, Milliseconds const& maxTime) { Repeat(uint32(minTime.count()), uint32(maxTime.count())); }
     ...
     */
    void Repeat(uint32 minTime, uint32 maxTime) { Repeat(urand(minTime, maxTime)); }
     ...
     */
    void DelayEvents(Milliseconds const& delay) { DelayEvents(uint32(delay.count())); }
     ...
     */
    void DelayEvents(uint32 delay) { _time = delay < _time ? _time - delay : 0; }
     ...
     */
    void DelayEvents(Milliseconds const& delay, uint32 group) { DelayEvents(uint32(delay.count()), group); }
    ...
    /**
     * @name CancelEventsByGCD
     * @brief Cancels events by GCD
     * @param global cooldown
     */
     ...
     */
    uint32 GetNextEventTime() const { return Empty() ? 0 : _eventMap.begin()->first; }
     ...
     */
    bool IsInPhase(uint8 phase) const { return phase <= 8 && (!phase || _phase & (1 << (phase - 1))); }
